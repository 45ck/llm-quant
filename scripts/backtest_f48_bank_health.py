#!/usr/bin/env python3
"""Backtest F48: Financial Sector Health (KRE/XLF).

Self-contained backtest script. Does NOT use the strategy registry.

Signal: Compute KRE/XLF ratio, 20-day ratio momentum, and 40-day SMA.
Three regimes based on momentum direction + ratio vs SMA:
  - BANK_HEALTH (mom > 0 AND ratio > SMA): 70% SPY + 15% IWM + 15% SHY
  - BANK_STRESS (mom < 0 AND ratio < SMA): 20% SPY + 30% TLT + 30% GLD + 20% SHY
  - NEUTRAL (one condition): 50% SPY + 30% TLT + 10% SHY + 10% GLD
Rebalance every 5 trading days.

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_f48_bank_health.py
"""
...
SLUG = "bank-health-v1"
SIGNAL_SYMBOLS: list[str] = ["KRE", "XLF"]
...
class BacktestParams:
    """Strategy parameters for the bank health regime backtest."""

    mom_lookback: int = 60
    sma_period: int = 120
    rebalance_frequency_days: int = 10
    # Bank-health regime (KRE outperforming = risk-on, healthy lending)
    spy_health: float = 0.60
    iwm_health: float = 0.00
    gld_health: float = 0.10
    shy_health: float = 0.20
    tlt_health: float = 0.10
    # Bank-stress regime (KRE underperforming = risk-off, credit tightening)
    spy_stress: float = 0.00
    iwm_stress: float = 0.00
    gld_stress: float = 0.40
    shy_stress: float = 0.30
    tlt_stress: float = 0.30
    # Neutral regime (mixed signals)
    spy_neutral: float = 0.30
    iwm_neutral: float = 0.00
    gld_neutral: float = 0.15
    shy_neutral: float = 0.25
    tlt_neutral: float = 0.30
    round_trip_cost_bps: float = ROUND_TRIP_COST_BPS

    def get_regime_weights(self, regime: str) -> dict[str, float]:
        """Return target weights for a given regime."""
        if regime == "BANK_HEALTH":
            return {
                "SPY": self.spy_health,
                "IWM": self.iwm_health,
                "GLD": self.gld_health,
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "spy_health": self.spy_health,
            "iwm_health": self.iwm_health,
            "gld_health": self.gld_health,
            "shy_health": self.shy_health,
            "tlt_health": self.tlt_health,
            "spy_stress": self.spy_stress,
            "iwm_stress": self.iwm_stress,
            "gld_stress": self.gld_stress,
            "shy_stress": self.shy_stress,
            "tlt_stress": self.tlt_stress,
...
def run_backtest(
    ...
    """Run the bank health regime backtest.
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    # -- Precompute KRE/XLF ratio, momentum, and SMA for all dates -----------
    kre_closes = [close_by_symbol["KRE"][d] for d in common_dates]
    xlf_closes = [close_by_symbol["XLF"][d] for d in common_dates]

    # Compute ratio
    ratio: list[float] = []
    for i in range(len(common_dates)):
        if xlf_closes[i] > 0:
            ratio.append(kre_closes[i] / xlf_closes[i])
        else:
            ratio.append(0.0)
...
def main() -> None:
    """Run the F48 backtest and save results."""
    result = run_backtest()
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  F48 Bank Health Regime Backtest: {SLUG}")
    print("=" * 60)
    ...
    if result["sharpe_ratio"] < 0.70:
        print("  --> Sharpe < 0.70: SKIP robustness (signal too weak)")
    elif not all_basic:
        print("  --> Some gates failed but Sharpe >= 0.70: proceed to robustness")
    print()
