...
def _compute_breakeven_inflation_returns(data_dir: Path) -> dict | None:
    """Breakeven inflation: TIP/IEF ratio momentum → DBA+GLD / SPY allocation."""
    try:
        import polars as pl
        ...
        # Build TIP/IEF ratio series
        ratio_series = []
        for i in range(n):
            d = dates[i]
            tip = sym_data["TIP"].get(d, 0.0)
            ief = sym_data["IEF"].get(d, 0.0)
            ratio_series.append(tip / ief if tip > 0 and ief > 0 else 0.0)
        ...
        for i in range(warmup, n):
            ...
            if ratio_mom > 0:
                # Inflation expectations rising: commodities + gold
                regime = "inflation_rising"
                day_ret = (
                    _asset_ret("DBA", i) * 0.40
                    + _asset_ret("GLD", i) * 0.30
                    + spy_rets[i] * 0.10
                )
            else:
...
def _compute_tlt_gld_disinflation_returns(data_dir: Path) -> dict | None:
    """TLT/GLD disinflation v1: TLT/GLD ratio momentum → SPY / GLD+DBA allocation."""
    try:
        import polars as pl
        ...
        # Build TLT/GLD ratio series
        ratio_series = []
        for i in range(n):
            d = dates[i]
            tlt = sym_data["TLT"].get(d, 0.0)
            gld = sym_data["GLD"].get(d, 0.0)
            ratio_series.append(tlt / gld if tlt > 0 and gld > 0 else 0.0)
        ...
        for i in range(warmup, n):
            ...
            if ratio_mom > 0:
                # Disinflation (TLT outperforming GLD): equity benefits
                regime = "disinflation"
                day_ret = spy_rets[i] * 0.80
            else:
                # Inflation (GLD outperforming TLT): gold + commodities
                regime = "inflation"
                day_ret = (
                    _asset_ret("GLD", i) * 0.50
                    + _asset_ret("DBA", i) * 0.20
                    + spy_rets[i] * 0.10
                )
