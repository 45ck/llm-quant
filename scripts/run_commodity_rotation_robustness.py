#!/usr/bin/env python3
"""Robustness analysis for commodity-rotation-v1 (F20 Commodity Rotation).

Mechanism: Relative momentum rotation across GLD, DBA, USO.
  - Compute 30-day momentum for each commodity ETF: (price_t-1 / price_t-31 - 1)
  - Rank by momentum
  - Hold top-1 commodity at 70% weight, top-2 at 20% weight
  - If ALL momentums are negative (commodity bear market): hold 50% SHY (cash proxy)

ZERO equity exposure. Commodity-only strategy. Near-zero correlation with equity-timing.
Cost = 3 bps per portfolio switch (rebalance only when ranking changes).

Base parameters: lookback=30, top1_weight=0.70, top2_weight=0.20, cash_weight=0.50.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_commodity_rotation_robustness.py
"""
...
SLUG = "commodity-rotation-v1"
SYMBOLS = ["GLD", "DBA", "USO", "SHY"]
COMMODITY_SYMBOLS = ["GLD", "DBA", "USO"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
# Build price series — GLD as the date spine (no SPY in this strategy)
...
# We need a benchmark for shuffled signal test — use equal-weight commodity basket
# (since this is a commodity-only strategy, SPY would be wrong benchmark)
eq_commodity_rets = [0.0]
for i in range(1, n):
    ...
    for sym in COMMODITY_SYMBOLS:
        if d in sym_data[sym] and dp in sym_data[sym] and sym_data[sym][dp] > 0:
            ...
            count += 1
    eq_commodity_rets.append(day_ret / count if count > 0 else 0.0)
...
def run_single(params):
    ...
    top1_weight = float(params.get("top1_weight", 0.70))
    top2_weight = float(params.get("top2_weight", 0.20))
    cash_weight = float(params.get("cash_weight", 0.50))

    daily_returns = []
    cost_per_switch = 0.0003  # 3 bps per switch

    prev_allocation = None  # track (top1_sym, top2_sym) or "cash"
    for i in range(WARMUP, n):
        if i < lookback + 1:
            daily_returns.append(0.0)
            continue

        # Compute momentum for each commodity at close i-1 (lag-1, no look-ahead)
        momentums = {}
        valid = True
        for sym in COMMODITY_SYMBOLS:
            d_now = dates[i - 1]
        ...
        # Check if ALL momentums are negative -> commodity bear market
        all_negative = all(m < 0 for m in momentums.values())

        if all_negative:
            # Commodity bear: hold SHY at cash_weight
            allocation = "cash"
            day_ret = asset_ret("SHY", i) * cash_weight
        else:
            # Rank by momentum (descending), hold top-1 and top-2
            ranked = sorted(momentums.items(), key=lambda x: x[1], reverse=True)
            top1_sym = ranked[0][0]
            top2_sym = ranked[1][0]
            allocation = (top1_sym, top2_sym)

            day_ret = (
                asset_ret(top1_sym, i) * top1_weight
                + asset_ret(top2_sym, i) * top2_weight
            )

        # Apply switching cost only when allocation changes
...
perturbations = [
    ...
    ("top1_weight=0.60", {**BASE_PARAMS, "top1_weight": 0.60}),
    ("top1_weight=0.80", {**BASE_PARAMS, "top1_weight": 0.80}),
    ("top2_weight=0.10", {**BASE_PARAMS, "top2_weight": 0.10}),
    ("top2_weight=0.30", {**BASE_PARAMS, "top2_weight": 0.30}),
]
...
# ==============================================================================
print("\n--- SHUFFLED SIGNAL TEST ---")
# Benchmark = equal-weight commodity basket (GLD+DBA+USO), NOT SPY
# This is a commodity-only strategy; SPY is the wrong benchmark
benchmark_rets = eq_commodity_rets[WARMUP:]
...
print(f"  Benchmark (EW commodity): {len(aligned_bench)} days")
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "commodity_rotation",
    "mechanism": "F20 Commodity Rotation — relative momentum ranking across GLD/DBA/USO",
