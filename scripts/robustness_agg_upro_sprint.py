#!/usr/bin/env python3
"""Robustness analysis for agg-upro-sprint (Track D -- Sprint Alpha).

Mechanism: AGG (aggregate bonds) return as lead signal
-> UPRO (3x leveraged S&P 500) follower. AGG reacts to credit/rate
   shifts before equity markets price them in, creating a lag signal.

Signal logic:
  - AGG 10-day return >= entry_threshold (1.0%) -> entry (buy UPRO at target_weight)
  - AGG 10-day return <= exit_threshold (-0.5%) -> exit to SHY (cash proxy)
  - Use signal from lag_days (3) ago -- no look-ahead
  - Daily rebalancing (rebalance_frequency_days=1)
  - VIX crash filter: 100% SHY when VIX > vix_crash_threshold (default 30)
...
Run: cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_agg_upro_sprint.py
"""
...
SYMBOLS = ["AGG", "UPRO", "SPY", "SHY", "VIX"]
DD_THRESHOLD = 0.40  # Track D: 40% max drawdown
LOOKBACK_DAYS = 5 * 365
WARMUP = 60

BASE_PARAMS = {
    "entry_threshold": 0.01,  # AGG 10-day return >= 1.0% -> buy UPRO
    ...
    "rebalance_frequency_days": 1,  # Daily rebalancing
...
def run_single(params: dict) -> dict:
    ...
    - Rebalance every rebalance_frequency_days trading days
    - VIX crash filter: if VIX > vix_crash_threshold, go 100% SHY regardless
    - If AGG return >= entry_threshold -> hold UPRO at target_weight, rest in SHY
    ...
    rebal_freq = int(params.get("rebalance_frequency_days", 1))
    vix_crash = float(params.get("vix_crash_threshold", 30.0))
    ...
    days_since_rebalance = rebal_freq  # Force first rebalance
    ...
    for i in range(WARMUP, n):
        ...
        is_rebal_day = days_since_rebalance >= rebal_freq

        if is_rebal_day:
            days_since_rebalance = 0

            # VIX crash filter: if VIX > threshold, force exit to SHY
            d_today = dates[i]
            vix_level = sym_data["VIX"].get(d_today, 0.0)
            if vix_level > vix_crash:
                in_position = False
            else:
...
perturbations = [
    ...
    ("lag=4", {**BASE_PARAMS, "lag_days": 4}),
    ("lag=5", {**BASE_PARAMS, "lag_days": 5}),
    # signal_window: [5, 7, 10, 15, 20]
    ...
    # rebalance_frequency: [1, 3, 5, 7]
    ("rebalance=3", {**BASE_PARAMS, "rebalance_frequency_days": 3}),
    ("rebalance=5", {**BASE_PARAMS, "rebalance_frequency_days": 5}),
    ("rebalance=7", {**BASE_PARAMS, "rebalance_frequency_days": 7}),
...
output = {
    ...
    "track": "D",
    "mechanism": "AGG (aggregate bonds) 10-day return -> UPRO (3x leveraged S&P 500)",
    "key_differentiator": "AGG credit/rate signal leads equity via bond-equity lead-lag; VIX crash filter for tail risk",
