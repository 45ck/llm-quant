#!/usr/bin/env python3
"""Backtest F44: Semiconductor Capex Cycle.

Self-contained backtest script. Does NOT use the strategy registry.

Signal: Compute SMH/SPY ratio, 20-day momentum, and 40-day SMA.
Three regimes based on momentum direction + ratio vs SMA:
  - CAPEX_EXPANSION (mom > 0 AND ratio > SMA): 70% SPY + 15% QQQ + 15% SHY
  - CAPEX_CONTRACTION (mom < 0 AND ratio < SMA): 30% SPY + 30% GLD + 20% SHY + 20% TLT
  - NEUTRAL: 50% SPY + 30% SHY + 10% GLD + 10% TLT
Rebalance every 5 trading days.

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_f44_semi_capex.py
"""
...
SLUG = "semi-capex-cycle-v1"
SIGNAL_SYMBOLS: list[str] = ["SMH"]
...
class BacktestParams:
    """Strategy parameters for the semiconductor capex cycle backtest."""

    mom_lookback: int = 20
    sma_period: int = 40
    rebalance_frequency_days: int = 5
    # Capex expansion regime (SMH outperforming = risk-on/early-cycle)
    spy_expansion: float = 0.70
    qqq_expansion: float = 0.15
    gld_expansion: float = 0.00
    shy_expansion: float = 0.15
    tlt_expansion: float = 0.00
    # Capex contraction regime (SMH underperforming = defensive/late-cycle)
    spy_contraction: float = 0.30
    qqq_contraction: float = 0.00
    gld_contraction: float = 0.30
    shy_contraction: float = 0.20
    tlt_contraction: float = 0.20
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "qqq_expansion": self.qqq_expansion,
            ...
            "qqq_contraction": self.qqq_contraction,
...
def run_backtest(
    ...
    """Run the semiconductor capex cycle backtest.
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    all_needed = [*TRADEABLE_SYMBOLS, "SMH"]
    ...
    # ── Precompute SMH/SPY ratio, momentum, and SMA for all dates ────────
    smh_closes = [close_by_symbol["SMH"][d] for d in common_dates]
    ...
    for i in range(len(common_dates)):
        if spy_closes[i] > 0:
            ratio.append(smh_closes[i] / spy_closes[i])
        else:
            ratio.append(0.0)
    ...
    for i, date_str in enumerate(common_dates):
        ...
        if mom > 0 and current_ratio > sma:
            regime = "CAPEX_EXPANSION"
        elif mom < 0 and current_ratio < sma:
            regime = "CAPEX_CONTRACTION"
        else:
            regime = "NEUTRAL"
...
def main() -> None:
    """Run the F44 backtest and save results."""
    result = run_backtest()
    ...
    entry = {
        ...
        "slug": SLUG,
        "strategy_type": "semi_capex_cycle",
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  F44 Semiconductor Capex Cycle Backtest: {SLUG}")
    print("=" * 60)
