#!/usr/bin/env python3
"""Robustness analysis for sector-crowding-v1 (M4, Sector Crowding).

Mechanism: Intra-sector correlation rise predicts sector crowding and mean-reversion.
  - Compute 40-day rolling correlation between XLK and SPY (tech crowding proxy)
  - Compute 120-day percentile rank of that correlation
  - High crowding (pct > 85%): rotate AWAY from tech into energy/commodities
    50% XLE + 20% GLD + 10% SHY
  - Low crowding (pct < 30%): tech decoupled, potential alpha
    70% XLK + 10% SPY
  - Middle zone: 40% SPY + 20% XLK + 10% GLD
Base parameters: corr_window=40, pctile_window=120, high_pct=85, low_pct=30.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_sector_crowding_robustness.py
"""
...
SLUG = "sector-crowding-v1"
SYMBOLS = ["XLK", "XLF", "XLE", "XLV", "XLP", "XLU", "XLI", "XLY", "SPY", "SHY", "GLD"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 130
...
def _rolling_corr(series_a, series_b, window):
    """Compute rolling Pearson correlation between two lists of floats."""
    n_pts = len(series_a)
    corrs = [float("nan")] * n_pts
    for i in range(window - 1, n_pts):
        a_win = series_a[i - window + 1 : i + 1]
        b_win = series_b[i - window + 1 : i + 1]
        mean_a = sum(a_win) / window
        mean_b = sum(b_win) / window
        cov = (
            sum((a_win[j] - mean_a) * (b_win[j] - mean_b) for j in range(window))
            / window
        )
        std_a = (sum((x - mean_a) ** 2 for x in a_win) / window) ** 0.5
        std_b = (sum((x - mean_b) ** 2 for x in b_win) / window) ** 0.5
        if std_a > 0 and std_b > 0:
            corrs[i] = cov / (std_a * std_b)
        else:
            corrs[i] = 0.0
    return corrs


def _percentile_rank(values, idx, pctile_window):
    """Compute percentile rank of values[idx] in the window ending at idx."""
    start = max(0, idx - pctile_window + 1)
    window_vals = [v for v in values[start : idx + 1] if not math.isnan(v)]
    if len(window_vals) < 10:
        return float("nan")
    current = values[idx]
    if math.isnan(current):
        return float("nan")
    count_below = sum(1 for v in window_vals if v <= current)
    return count_below / len(window_vals) * 100.0


def run_single(params):
    """Run a single backtest with the given parameters."""
    corr_window = int(params.get("corr_window", 40))
    pctile_window = int(params.get("pctile_window", 120))
    high_pct = float(params.get("high_pct", 85))
    low_pct = float(params.get("low_pct", 30))

    # Compute daily returns for XLK and SPY
    xlk_rets = []
    spy_daily = []
    for i in range(n):
        if i == 0:
            ...
            xlk_rets.append(asset_ret("XLK", i))
            spy_daily.append(spy_rets[i])

    # Compute rolling correlation between XLK and SPY returns
    corrs = _rolling_corr(xlk_rets, spy_daily, corr_window)

    # Compute percentile ranks
    pctile_ranks = [float("nan")] * n
    for i in range(n):
        if not math.isnan(corrs[i]):
            pctile_ranks[i] = _percentile_rank(corrs, i, pctile_window)
    ...
    for i in range(WARMUP, n):
        # Use lag-1: signal from day i-1 applied to day i returns
        pct_rank = pctile_ranks[i - 1]

        if math.isnan(pct_rank):
            daily_returns.append(0.0)
            continue

        # Determine regime based on correlation percentile rank
        if pct_rank > high_pct:
            # Extreme crowding: tech moves lockstep with market
            # Rotate AWAY from tech into energy/commodities
            regime = "high_crowding"
            day_ret = (
                asset_ret("XLE", i) * 0.50
                + asset_ret("GLD", i) * 0.20
                + asset_ret("SHY", i) * 0.10
            )
        elif pct_rank < low_pct:
            # Tech decoupled, potential alpha
            regime = "low_crowding"
            day_ret = asset_ret("XLK", i) * 0.70 + asset_ret("SPY", i) * 0.10
        else:
            # Middle zone
            regime = "neutral"
            day_ret = (
                asset_ret("SPY", i) * 0.40
                + asset_ret("XLK", i) * 0.20
...
perturbations = [
    ...
    ("corr_window=60", {**BASE_PARAMS, "corr_window": 60}),
    ("high_pct=75", {**BASE_PARAMS, "high_pct": 75}),
