#!/usr/bin/env python3
"""Robustness analysis for momentum-dispersion-v1 (M5, Multi-Asset Momentum Dispersion).

Mechanism: Cross-asset momentum dispersion signals alpha opportunity windows.
  - Compute 60-day momentum for 8 assets (SPY, TLT, GLD, EFA, DBA, USO, EEM, IEF)
  - Dispersion = std deviation of all 8 assets' 60-day momentum
  - 120-day percentile rank of current dispersion
  - High dispersion (>70th pct): allocate 40% to top-2 momentum assets (20% each)
  - Low dispersion (<30th pct): allocate 60% SHY + 20% SPY (sit out)
  - Middle zone: 50% SPY + 15% GLD + 15% TLT
Base parameters: mom_window=60, pct_window=120, high_pct=70, low_pct=30.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_momentum_dispersion_robustness.py
"""
...
SLUG = "momentum-dispersion-v1"
MOMENTUM_SYMBOLS = ["SPY", "TLT", "GLD", "EFA", "DBA", "USO", "EEM", "IEF"]
ALLOCATION_SYMBOLS = [*MOMENTUM_SYMBOLS, "SHY"]  # SHY needed for low-dispersion regime
ALL_SYMBOLS = list({*MOMENTUM_SYMBOLS, "SHY"})
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 130  # 120 for percentile + 60 for momentum
...
def run_single(params):
    ...
    mom_window = int(params.get("mom_window", 60))
    ...
    for i in range(WARMUP, n):
        if i < mom_window + pct_window + 1:
            daily_returns.append(0.0)
            continue

        # Compute 60-day momentum for each asset at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_mom = dates[i - 1 - mom_window]

        momentums = {}
        valid = True
        for sym in MOMENTUM_SYMBOLS:
            p_now = sym_data[sym].get(d_now, 0.0)
            p_mom = sym_data[sym].get(d_mom, 0.0)
            if p_now <= 0 or p_mom <= 0:
                valid = False
                break
            momentums[sym] = p_now / p_mom - 1

        if not valid:
            daily_returns.append(0.0)
            continue

        # Compute cross-asset momentum dispersion (std dev of 8 momentums)
        mom_values = list(momentums.values())
        mom_mean = sum(mom_values) / len(mom_values)
        dispersion = (
            sum((m - mom_mean) ** 2 for m in mom_values) / len(mom_values)
        ) ** 0.5

        # Compute 120-day rolling percentile rank of dispersion
        # Look back pct_window days and compute dispersion for each
        hist_dispersions = []
        for j in range(1, pct_window + 1):
            hist_idx = i - 1 - j
            if hist_idx < mom_window:
                continue
            d_hist = dates[hist_idx]
            d_hist_mom = dates[hist_idx - mom_window]

            hist_moms = []
            hist_valid = True
            for sym in MOMENTUM_SYMBOLS:
                p_h = sym_data[sym].get(d_hist, 0.0)
                p_hm = sym_data[sym].get(d_hist_mom, 0.0)
                if p_h <= 0 or p_hm <= 0:
                    hist_valid = False
                    break
                hist_moms.append(p_h / p_hm - 1)

            if not hist_valid:
                continue

            h_mean = sum(hist_moms) / len(hist_moms)
            h_disp = (sum((m - h_mean) ** 2 for m in hist_moms) / len(hist_moms)) ** 0.5
            hist_dispersions.append(h_disp)

        if len(hist_dispersions) < 20:
            daily_returns.append(0.0)
            continue

        # Percentile rank: what fraction of historical dispersions is <= current
        pct_rank = (
            sum(1 for d in hist_dispersions if d <= dispersion)
            / len(hist_dispersions)
            * 100
        )

        # Determine regime and allocate
        if pct_rank > high_pct:
            # High dispersion: strong trends, allocate to top-2 momentum assets
            regime = "high_dispersion"
            sorted_syms = sorted(momentums, key=momentums.get, reverse=True)
            top2 = sorted_syms[:2]
            day_ret = asset_ret(top2[0], i) * 0.20 + asset_ret(top2[1], i) * 0.20
        elif pct_rank < low_pct:
            # Low dispersion: no clear trends, sit out
            regime = "low_dispersion"
            day_ret = asset_ret("SHY", i) * 0.60 + spy_rets[i] * 0.20
        else:
...
# ==============================================================================
perturbations = [
    ("mom_window=40", {**BASE_PARAMS, "mom_window": 40}),
    ("mom_window=90", {**BASE_PARAMS, "mom_window": 90}),
    ("high_pct=60", {**BASE_PARAMS, "high_pct": 60}),
    ("high_pct=80", {**BASE_PARAMS, "high_pct": 80}),
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "momentum_dispersion",
