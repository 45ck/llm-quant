#!/usr/bin/env python3
"""Robustness analysis for agg-tqqq-sprint (Track D -- Sprint Alpha).

Mechanism: AGG (aggregate bond ETF) 10-day return as lead signal
-> TQQQ (3x leveraged QQQ) follower. AGG captures broad fixed income
sentiment -- when aggregate bonds rise, credit conditions are easing,
which historically precedes equity rallies (especially growth/tech).

Signal logic:
  - AGG 10-day return >= entry_threshold (1%) -> entry (buy TQQQ at target_weight)
  ...
  - VIX crash filter: go 100% SHY when VIX > vix_crash_threshold (default 30)
...
Run: cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_agg_tqqq_sprint.py
"""
...
SYMBOLS = ["AGG", "TQQQ", "QQQ", "SHY", "VIX"]
DD_THRESHOLD = 0.40  # Track D: 40% max drawdown
LOOKBACK_DAYS = 5 * 365
WARMUP = 60

BASE_PARAMS = {
    "entry_threshold": 0.01,  # AGG 10-day return >= 1% -> buy TQQQ
    ...
    "vix_crash_threshold": 30,  # Go 100% SHY when VIX > 30
}
...
def run_single(params: dict) -> dict:
    ...
    Signal logic (with lag + daily rebalancing + VIX crash filter):
    - Compute AGG return over signal_window days
    - Use the signal from lag_days ago (causal)
    - Rebalance every rebalance_frequency days
    - If VIX > vix_crash_threshold -> override to SHY (100%)
    - If AGG return >= entry_threshold -> hold TQQQ at target_weight, rest in SHY
    ...
    vix_crash = float(params.get("vix_crash_threshold", 30))
    ...
    for i in range(WARMUP, n):
        ...
        # VIX crash filter -- override everything
        vix_level = sym_data["VIX"].get(dates[i], 0.0)
        if vix_level > vix_crash and vix_level > 0:
            in_position = False
        elif days_since_rebal >= rebal_freq:
...
# ==============================================================================
# PERTURBATION TESTS (22 variants as specified)
# ==============================================================================
perturbations = [
    # lag_days: [1, 2, 5, 7]
...
output = {
    ...
    "track": "D",
    "mechanism": "AGG (aggregate bond) 10-day return -> TQQQ (3x leveraged QQQ)",
    "key_differentiator": "AGG captures broad fixed income sentiment; credit easing -> equity rally",
    "vix_crash_filter": "Go 100% SHY when VIX > threshold (default 30)",
