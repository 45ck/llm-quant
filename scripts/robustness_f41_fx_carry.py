#!/usr/bin/env python3
"""Robustness analysis for F41: FX Carry Risk Appetite Signal.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f41_fx_carry.py
"""
...
from backtest_f41_fx_carry import (  # noqa: E402
    ALL_SYMBOLS,
    LOOKBACK_DAYS,
    SLUG,
    TRADEABLE_SYMBOLS,
...
def run_shuffled_signal_test(
    ...
    we randomly assign one of {risk_on, risk_off, neutral}. If the real signal
    is genuine, the shuffled Sharpe distribution should be lower.
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    needed = [*["FXA", "FXY"], *TRADEABLE_SYMBOLS]
    ...
    from backtest_f41_fx_carry import INITIAL_CAPITAL
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    # lookback
    ("lookback=7", {"lookback": 7}),
    ("lookback=15", {"lookback": 15}),
    # entry_threshold
    ("entry_threshold=0.01", {"entry_threshold": 0.01}),
    ("entry_threshold=0.03", {"entry_threshold": 0.03}),
    # spy_risk_on
    ("spy_risk_on=0.70", {"spy_risk_on": 0.70}),
    ("spy_risk_on=0.90", {"spy_risk_on": 0.90}),
    # gld_risk_off
    ("gld_risk_off=0.40", {"gld_risk_off": 0.40}),
    ("gld_risk_off=0.60", {"gld_risk_off": 0.60}),
    # shy_risk_off
    ("shy_risk_off=0.20", {"shy_risk_off": 0.20}),
    ("shy_risk_off=0.40", {"shy_risk_off": 0.40}),
    # spy_neutral
...
def main() -> None:
    """Run full robustness analysis for F41."""
    import yaml
    ...
    # -- Final gate summary --
    print("\n" + "=" * 60)
    print("  F41 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
