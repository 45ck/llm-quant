...
"""Robustness analysis for F4 pre-fomc-tlt-drift strategy."""
...
# Perturbations
perturbations = [
    ("pre_days=2", {**BASE_PARAMS, "pre_days": 2}),
    ("pre_days=4", {**BASE_PARAMS, "pre_days": 4}),
    ("pre_days=1", {**BASE_PARAMS, "pre_days": 1}),
    ("weight=0.80", {**BASE_PARAMS, "target_weight": 0.80}),
    ("weight=0.60", {**BASE_PARAMS, "target_weight": 0.60}),
]
perturb_results = []
...
perturb_pass = n_stable / len(perturb_results) >= 0.60
dd_pass = base["max_dd"] < 12.0
...
print(
    f"Perturbation:   {'PASS' if perturb_pass else 'FAIL'} ({n_stable}/{len(perturb_results)})"
)
print(f"Max DD < 12%:   {'PASS' if dd_pass else 'FAIL'} ({base['max_dd']:.2f}%)")
