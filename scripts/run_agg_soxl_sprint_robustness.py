#!/usr/bin/env python3
"""Robustness analysis for agg-soxl-credit-lead-lag (Track D).

Mechanism: AGG (IG aggregate) 5-day return as lead signal -> SOXL (3x semiconductors) follower.
Novel combination: aggregate credit signal + most volatile 3x sector ETF.

Signal logic:
  - AGG 5-day return >= entry_threshold (0.5%) -> entry (buy SOXL at target_weight)
  - AGG 5-day return <= exit_threshold (-0.5%) -> exit to SHY (cash proxy)
...
Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_agg_soxl_sprint_robustness.py
"""
...
SLUG = "agg-soxl-credit-lead-lag"
SYMBOLS = ["AGG", "SOXL", "SHY"]
DD_THRESHOLD = 0.40  # Track D: 40% max drawdown
LOOKBACK_DAYS = 5 * 365
WARMUP = 60

BASE_PARAMS = {
    "entry_threshold": 0.005,  # AGG 5-day return >= 0.5% -> buy SOXL
    "exit_threshold": -0.005,  # AGG 5-day return <= -0.5% -> exit
    "lag_days": 5,  # Use signal from 5 days ago
    "signal_window": 5,  # 5-day return lookback for AGG
...
follower_df = prices.filter(pl.col("symbol") == "SOXL").sort("date")
...
print(f"  SOXL returns:     {len(aligned_follower)} days")
...
output = {
    ...
    "track": "D",
    "mechanism": "AGG (IG aggregate) 5-day return -> SOXL (3x semiconductors)",
