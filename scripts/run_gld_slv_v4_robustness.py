#!/usr/bin/env python3
"""Robustness analysis for gld-slv-mean-reversion-v4 (consensus windows 60/90/120).

Perturbations test whether the CONSENSUS approach is robust:
  1. Shift all windows -15d: [45, 75, 105] — tests timing robustness
  2. Shift all windows +15d: [75, 105, 135] — tests timing robustness
  3. Narrower bands: bb_std=1.5 — tests entry sensitivity
  4. Wider bands: bb_std=2.5 — tests entry sensitivity
  5. Lower weight: target_weight=0.30 — tests sizing sensitivity

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_gld_slv_v4_robustness.py
"""
...
def run_single(params):
    config = StrategyConfig(
        ...
        max_positions=2,
        target_position_weight=params.get("target_weight", 0.40),
        stop_loss_pct=0.05,
...
print(f"  consensus_windows = {BASE_PARAMS['consensus_windows']}")
print(f"  bb_std = {BASE_PARAMS['bb_std']}")
print(f"  target_weight = {BASE_PARAMS['target_weight']}")
...
# Perturbations: test robustness of consensus mechanism
perturbations = [
    # Window timing robustness (the key fix in v4)
    (
        "windows=[45,75,105] (shift -15d)",
        {**BASE_PARAMS, "consensus_windows": [45, 75, 105]},
    ),
    (
        "windows=[75,105,135] (shift +15d)",
        {**BASE_PARAMS, "consensus_windows": [75, 105, 135]},
    ),
    # Band sensitivity
