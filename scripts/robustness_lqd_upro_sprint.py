#!/usr/bin/env python3
"""Robustness analysis for lqd-upro-sprint (Track D -- Sprint Alpha).

Mechanism: LQD (investment-grade corporate bonds) return as lead signal
-> UPRO (3x leveraged S&P 500) follower. Credit spreads lead equity moves
because institutional credit desks reprice risk before equity markets react.

Signal logic:
  - LQD 10-day return >= entry_threshold (1.0%) -> entry (buy UPRO at target_weight)
  ...
  - VIX crash filter: 100% SHY when VIX > threshold (default 30)
...
Run: cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_lqd_upro_sprint.py
"""
...
SLUG = "lqd-upro-sprint"
SYMBOLS = ["LQD", "UPRO", "SHY"]
VIX_SYMBOL = "^VIX"
DD_THRESHOLD = 0.40  # Track D: 40% max drawdown
LOOKBACK_DAYS = 5 * 365
WARMUP = 60

# Parameters from experiment registry (experiment 038de384, frozen spec)
BASE_PARAMS = {
    "entry_threshold": 0.01,  # LQD 10-day return >= 1.0% -> buy UPRO
    ...
    "vix_crash_threshold": 30,  # 100% SHY when VIX > 30
}
...
follower_df = prices.filter(pl.col("symbol") == "UPRO").sort("date")
dates = follower_df["date"].to_list()
follower_close = follower_df["close"].to_list()
n = len(dates)
...
# VIX data -- fetcher stores ^VIX as "VIX"
vix_key = "VIX"
...
follower_rets = [0.0] + [
    (follower_close[i] / follower_close[i - 1] - 1) if follower_close[i - 1] > 0 else 0
    for i in range(1, n)
]
...
def run_single(params: dict) -> dict:
    ...
    Signal logic (with lag + rebalance frequency + VIX crash filter):
    - Compute LQD return over signal_window days
    - Use the signal from lag_days ago (causal)
    - Only act on rebalance days (rebalance_frequency=1 means daily)
    - VIX crash filter: if VIX > vix_crash_threshold, go 100% SHY
    - If LQD return >= entry_threshold -> hold UPRO at target_weight, rest in SHY
    ...
    for i in range(WARMUP, n):
        ...
        day_counter += 1
        is_rebal_day = (day_counter % rebal_freq) == 0 if rebal_freq > 1 else True

        prev_position = in_position

        if is_rebal_day:
            # VIX crash filter: override everything if VIX is elevated
            d_today = dates[i]
            vix_level = vix_data.get(d_today, 0.0)
        ...
        if in_position:
            day_ret = follower_rets[i] * tw + asset_ret("SHY", i) * (1.0 - tw)
        else:
...
def cpcv_sharpe(
    ...
    Uses 15 groups, 3 test groups, 5-day purge as specified in research-spec.
    """
...
perturbations = [
    ...
    ("rebalance=3", {**BASE_PARAMS, "rebalance_frequency": 3}),
    ("rebalance=5", {**BASE_PARAMS, "rebalance_frequency": 5}),
    ("rebalance=7", {**BASE_PARAMS, "rebalance_frequency": 7}),
    # vix_crash_threshold: [25, 35, 40]
...
for name, params in perturbations:
    ...
    print(
        f"  {name:<25} {r['sharpe']:>8.4f} {r['max_dd']:>8.4f} {r['cagr']:>8.4f} "
        f"{pct:>+8.1f}% {stable:>10}"
    )
...
follower_daily_rets = follower_rets[WARMUP:]
strat_returns = base["daily_returns"]
n_min = min(len(strat_returns), len(follower_daily_rets))
aligned_strat = strat_returns[-n_min:]
aligned_follower = follower_daily_rets[-n_min:]

print(f"  Strategy returns: {len(aligned_strat)} days")
print(f"  UPRO returns:     {len(aligned_follower)} days")

shuffled_result = shuffled_signal_test(
    daily_returns=aligned_strat,
    asset_returns=aligned_follower,
    n_shuffles=1000,
    seed=42,
)
...
output = {
    ...
    "track": "D",
    "mechanism": "LQD (IG credit) 10-day return -> UPRO (3x leveraged SPY)",
    "key_differentiator": "Credit spreads lead equity: LQD signals risk appetite shift before SPY reacts",
    "vix_crash_filter": "100% SHY when VIX > threshold (default 30)",
    "cost_model": "20 bps per trade round-trip",
