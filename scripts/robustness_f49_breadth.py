#!/usr/bin/env python3
"""Robustness analysis for F49: Market Breadth Regime.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f49_breadth.py
"""
...
from backtest_f49_breadth import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SECTOR_SYMBOLS,
...
def run_shuffled_signal_test(
    ...
    """Test whether the breadth signal is genuine by shuffling regime assignments.

    At each rebalance point, instead of using the real regime, we
    randomly assign one of {STRONG_BREADTH, NEUTRAL, WEAK_BREADTH}.
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    regimes = ["STRONG_BREADTH", "NEUTRAL", "WEAK_BREADTH"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    ...
    # Breadth momentum lookback (+/- 25%: 15 and 25)
    ("breadth_mom=15", {"breadth_mom_lookback": 15}),
    ("breadth_mom=25", {"breadth_mom_lookback": 25}),
    # Breadth thresholds
    ("strong_thresh=7", {"strong_breadth_threshold": 7}),
    ("strong_thresh=9", {"strong_breadth_threshold": 9}),
    ("weak_thresh=3", {"weak_breadth_threshold": 3}),
    ("weak_thresh=5", {"weak_breadth_threshold": 5}),
    # Strong regime SPY weight
    ("spy_strong=0.45", {"spy_strong": 0.45, "shy_strong": 0.15}),
    ("spy_strong=0.55", {"spy_strong": 0.55, "shy_strong": 0.05}),
    # Strong regime GLD weight
    ("gld_strong=0.20", {"gld_strong": 0.20, "shy_strong": 0.15}),
    ("gld_strong=0.30", {"gld_strong": 0.30, "shy_strong": 0.05}),
    # Neutral regime GLD weight
    ("gld_neutral=0.25", {"gld_neutral": 0.25, "shy_neutral": 0.35}),
    ("gld_neutral=0.35", {"gld_neutral": 0.35, "shy_neutral": 0.25}),
    # Weak regime GLD weight
    ("gld_weak=0.25", {"gld_weak": 0.25, "shy_weak": 0.35}),
    ("gld_weak=0.40", {"gld_weak": 0.40, "shy_weak": 0.20}),
    # Rebalance frequency
...
def _adjust_weights_for_perturbation(params: BacktestParams) -> BacktestParams:
    ...
    # Check and fix strong regime weights
    strong_sum = (
        params.spy_strong + params.qqq_strong + params.gld_strong + params.tlt_strong
    )
    if abs(strong_sum + params.shy_strong - 1.0) > 0.001:
        params.shy_strong = max(0.0, 1.0 - strong_sum)

    # Check and fix weak regime weights
    weak_sum = params.spy_weak + params.qqq_weak + params.gld_weak + params.tlt_weak
    if abs(weak_sum + params.shy_weak - 1.0) > 0.001:
        params.shy_weak = max(0.0, 1.0 - weak_sum)
...
def main() -> None:
    """Run full robustness analysis for F49."""
    import yaml
    ...
    # ── Final gate summary ───────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("  F49 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
