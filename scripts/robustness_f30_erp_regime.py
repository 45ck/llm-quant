#!/usr/bin/env python3
"""Robustness analysis for F30: Equity Risk Premium Valuation Regime.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f30_erp_regime.py
"""
...
from backtest_f30_erp_regime import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def run_shuffled_signal_test(
    ...
    """Test whether the ERP regime signal is genuine by shuffling regime labels.

    At each rebalance point, instead of using the real z-score regime, we
    randomly assign one of {EQUITY_CHEAP, NEUTRAL, EQUITY_EXPENSIVE}. If the
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    regimes = ["EQUITY_CHEAP", "NEUTRAL", "EQUITY_EXPENSIVE"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    ("spy_lookback=189 (-25%)", {"spy_lookback": 189}),
    ("spy_lookback=315 (+25%)", {"spy_lookback": 315}),
    ("zscore_lookback=189 (-25%)", {"zscore_lookback": 189}),
    ("zscore_lookback=315 (+25%)", {"zscore_lookback": 315}),
    ("cheap_threshold=0.75", {"cheap_threshold": 0.75}),
    ("cheap_threshold=1.25", {"cheap_threshold": 1.25}),
    ("expensive_threshold=-0.75", {"expensive_threshold": -0.75}),
    ("expensive_threshold=-1.25", {"expensive_threshold": -1.25}),
    ("spy_cheap=0.60", {"spy_cheap": 0.60}),
    ("spy_cheap=0.80", {"spy_cheap": 0.80}),
    ("spy_neutral=0.40", {"spy_neutral": 0.40}),
    ("spy_neutral=0.60", {"spy_neutral": 0.60}),
    ("rebalance_frequency_days=3", {"rebalance_frequency_days": 3}),
...
def _adjust_weights_for_perturbation(params: BacktestParams) -> BacktestParams:
    ...
    When we perturb spy_cheap or spy_neutral, the other weights might not sum
    to 1.0. We adjust SHY to absorb the difference.
    """
    # Check and fix cheap regime weights
    cheap_sum = params.spy_cheap + params.tlt_cheap + params.gld_cheap
    if abs(cheap_sum + params.shy_cheap - 1.0) > 0.001:
        params.shy_cheap = max(0.0, 1.0 - cheap_sum)

    # Check and fix neutral regime weights
    neutral_sum = params.spy_neutral + params.tlt_neutral + params.gld_neutral
    if abs(neutral_sum + params.shy_neutral - 1.0) > 0.001:
        params.shy_neutral = max(0.0, 1.0 - neutral_sum)

    # Check and fix expensive regime weights
    exp_sum = params.spy_expensive + params.tlt_expensive + params.gld_expensive
    if abs(exp_sum + params.shy_expensive - 1.0) > 0.001:
        params.shy_expensive = max(0.0, 1.0 - exp_sum)
...
def main() -> None:
    """Run full robustness analysis for F30."""
    import yaml
    ...
    # ── Final gate summary ───────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("  F30 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
    ...
    robustness_yaml: dict = {
        ...
        "family": "F30_erp_valuation_regime",
        "base_metrics": {
