#!/usr/bin/env python3
"""Robustness analysis for consumer-ratio-momentum-v2 (F19 Consumer Sentiment).

Hypothesis: XLY/XLP (Consumer Discretionary / Consumer Staples) ratio 30-day
momentum captures consumer risk appetite. When XLY outperforms XLP (consumers
spending on discretionary items), economy is strong — hold equity. When XLP
outperforms XLY (consumers retreating to staples), economy weakening — go
defensive.

v2 change: gld_weight reduced from 0.50 to 0.40 (pre-specified from v1
perturbation analysis showing MaxDD=14.76% with Sharpe=0.868 at gld_weight=0.40).
v1 failed only MaxDD gate (16.4% vs 15% threshold).

Signal:
- Compute XLY/XLP price ratio at close t-1
- Compute 30-day ratio momentum (ratio_now / ratio_30d_ago - 1)
- ratio_mom > 0 (risk-on consumer): 80% SPY
- ratio_mom < 0 (defensive consumer): 40% GLD + 20% SHY + 10% SPY

Base parameters: lookback=30, spy_risk_on_weight=0.80, gld_weight=0.40,
  shy_weight=0.20, spy_defensive_weight=0.10.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_consumer_ratio_v2_robustness.py
"""
...
SLUG = "consumer-ratio-momentum-v2"
SYMBOLS = ["XLY", "XLP", "SPY", "GLD", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
xly_df = prices.filter(pl.col("symbol") == "XLY").sort("date")
xly_data = dict(zip(xly_df["date"].to_list(), xly_df["close"].to_list(), strict=False))

xlp_df = prices.filter(pl.col("symbol") == "XLP").sort("date")
xlp_data = dict(zip(xlp_df["date"].to_list(), xlp_df["close"].to_list(), strict=False))
...
def run_single(params):
    ...
    spy_risk_on_weight = float(params.get("spy_risk_on_weight", 0.80))
    gld_weight = float(params.get("gld_weight", 0.40))
    shy_weight = float(params.get("shy_weight", 0.20))
    spy_defensive_weight = float(params.get("spy_defensive_weight", 0.10))

    # Build XLY/XLP ratio series
    ratio_series = []
    for i in range(n):
        d = dates[i]
        xly = xly_data.get(d, 0.0)
        xlp = xlp_data.get(d, 0.0)
        ratio_series.append(xly / xlp if xly > 0 and xlp > 0 else 0.0)
    ...
    for i in range(WARMUP, n):
        ...
        if ratio_mom > 0:
            regime = "risk_on"
            day_ret = spy_rets[i] * spy_risk_on_weight
        else:
            regime = "defensive"
            gld_r = _asset_ret(gld_data, i) * gld_weight
            shy_r = _asset_ret(shy_data, i) * shy_weight
            spy_r = spy_rets[i] * spy_defensive_weight
            day_ret = gld_r + shy_r + spy_r
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "consumer_ratio_momentum",
