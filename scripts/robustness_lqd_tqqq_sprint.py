#!/usr/bin/env python3
"""Robustness analysis for lqd-tqqq-sprint (Track D -- Sprint Alpha).

Mechanism: LQD (investment-grade corporate bonds) 10-day return as lead signal
-> TQQQ (3x leveraged QQQ) follower. LQD captures credit-market risk appetite
before it propagates to leveraged equities.

Signal logic:
  - LQD 10-day return >= entry_threshold (1%) -> entry (buy TQQQ at target_weight)
...
Run: cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_lqd_tqqq_sprint.py
"""
...
SYMBOLS = ["LQD", "TQQQ", "SHY", "VIX"]
DD_THRESHOLD = 0.40  # Track D: 40% max drawdown
LOOKBACK_DAYS = 5 * 365  # NOT 5*365+30
WARMUP = 60

# Parameters from experiment-registry.jsonl (frozen research-spec)
BASE_PARAMS = {
    "entry_threshold": 0.01,  # LQD 10-day return >= 1% -> buy TQQQ
    "exit_threshold": -0.005,  # LQD 10-day return <= -0.5% -> exit
    "lag_days": 3,  # 3-day propagation from credit to leveraged equities
...
def run_single(params: dict) -> dict:
    ...
    - VIX crash filter: if VIX > vix_crash_threshold, force 100% SHY regardless of signal
    - Rebalance daily (or at rebalance_frequency interval)
    - If LQD return >= entry_threshold -> hold TQQQ at target_weight, rest in SHY
    ...
    for i in range(WARMUP, n):
        ...
        # Rebalance check
        is_rebal_day = (day_counter % rebal_freq) == 0 or rebal_freq == 1

        if is_rebal_day:
            # VIX crash filter -- use previous day's VIX (causal)
            vix_level = sym_data["VIX"].get(dates[i - 1], 0.0)
            if vix_level > vix_crash:
                # Force exit to SHY during VIX crash
                in_position = False
            else:
...
perturbations = [
    ...
    # signal_window: [5, 7, 10, 15, 20] -- 10 is base, test others
    ...
    # rebalance_frequency: [1, 3, 5, 7] -- 1 is base, test others
    ...
    # vix_crash_threshold: [25, 35, 40] -- 30 is base, test others
...
output = {
    ...
    "track": "D",
    "mechanism": "LQD (IG credit) 10-day return -> TQQQ (3x leveraged QQQ) with VIX crash filter",
    "key_differentiator": "Credit risk appetite signal via LQD; VIX > 30 forces 100% SHY",
