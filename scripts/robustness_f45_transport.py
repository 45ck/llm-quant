#!/usr/bin/env python3
"""Robustness analysis for F45: Transport Confirmation (Dow Theory).
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f45_transport.py
"""
...
from backtest_f45_transport import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def run_shuffled_signal_test(
    ...
    """Test whether the transport confirmation signal is genuine by shuffling.

    At each rebalance point, instead of using the real regime, we
    randomly assign one of {EXPANSION_CONFIRMED, NEUTRAL, CONTRACTION}.
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    all_needed = [*TRADEABLE_SYMBOLS, "IYT"]
    ...
    regimes = ["EXPANSION_CONFIRMED", "NEUTRAL", "CONTRACTION"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    ...
    # Expansion regime IWM weight
    ("iwm_expansion=0.10", {"iwm_expansion": 0.10, "shy_expansion": 0.20}),
    ("iwm_expansion=0.25", {"iwm_expansion": 0.25, "shy_expansion": 0.05}),
    # Contraction regime GLD weight
    ("gld_contraction=0.25", {"gld_contraction": 0.25, "shy_contraction": 0.25}),
    ("gld_contraction=0.40", {"gld_contraction": 0.40, "shy_contraction": 0.10}),
    # Contraction regime TLT weight
    ("tlt_contraction=0.25", {"tlt_contraction": 0.25, "shy_contraction": 0.25}),
    ("tlt_contraction=0.40", {"tlt_contraction": 0.40, "shy_contraction": 0.10}),
    # Rebalance frequency
...
def _adjust_weights_for_perturbation(params: BacktestParams) -> BacktestParams:
    ...
    exp_sum = (
        ...
        + params.iwm_expansion
        + params.gld_expansion
        + params.tlt_expansion
    )
    if abs(exp_sum + params.shy_expansion - 1.0) > 0.001:
        params.shy_expansion = max(0.0, 1.0 - exp_sum)

    # Check and fix contraction regime weights
    con_sum = (
        params.spy_contraction
        + params.iwm_contraction
        + params.gld_contraction
        + params.tlt_contraction
    )
    if abs(con_sum + params.shy_contraction - 1.0) > 0.001:
        params.shy_contraction = max(0.0, 1.0 - con_sum)
...
def main() -> None:
    """Run full robustness analysis for F45."""
    import yaml
    ...
    # Check if Sharpe meets minimum threshold for robustness
    if base_sharpe < 0.70:
        print(f"\n  Sharpe {base_sharpe:.4f} < 0.70 -- skipping full robustness.")
        print("  Strategy REJECTED at backtest gate.")

        robustness_yaml: dict = {
            "strategy_slug": SLUG,
            "strategy_type": "transport_confirmation",
            "mechanism_family": "F45",
            ...
            "dsr": base_dsr,
            "gates": {
                "sharpe_gt_0.80": False,
                "maxdd_lt_15pct": base_max_dd < 0.15,
                "dsr_gte_0.95": base_dsr >= 0.95,
                "cpcv_oos_positive": "NOT_TESTED",
                "perturbation_gte_60pct": "NOT_TESTED",
                "shuffled_signal_passed": "NOT_TESTED",
            },
            "verdict": "FAIL",
            "failure_reason": (
                f"Sharpe = {base_sharpe:.4f}, below 0.70 threshold "
                "for full robustness testing."
            ),
        }
    ...
    # ── Final gate summary ───────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("  F45 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
