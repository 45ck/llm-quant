#!/usr/bin/env python3
"""Robustness analysis for em-relative-momentum-v1 (F19 EM Risk Appetite).

Mechanism: EEM/SPY price ratio 30-day momentum captures global risk appetite.
  - ratio_mom > 0 (EM outperforming, global risk-on): 50% SPY + 20% EEM + 10% GLD
  - ratio_mom < 0 (US outperforming, flight to US): 80% SPY
Base parameters: lookback=30, spy_us_weight=0.80,
                 spy_global_weight=0.50, eem_weight=0.20, gld_weight=0.10.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_em_relative_momentum_robustness.py
"""
...
def run_single(params):
    ...
    spy_global_w = float(params.get("spy_global_weight", 0.50))
    eem_w = float(params.get("eem_weight", 0.20))
    ...
    for i in range(WARMUP, n):
        ...
        # EEM/SPY ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        eem_now = sym_data["EEM"].get(d_now, 0.0)
        spy_now = sym_data["SPY"].get(d_now, 0.0)
        eem_lb = sym_data["EEM"].get(d_lb, 0.0)
        spy_lb = sym_data["SPY"].get(d_lb, 0.0)

        if eem_now <= 0 or spy_now <= 0 or eem_lb <= 0 or spy_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = eem_now / spy_now
        ratio_lb = eem_lb / spy_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on EEM/SPY ratio momentum
        if ratio_mom > 0:
            # EM outperforming: global risk-on, diversify internationally
            regime = "global_riskon"
            day_ret = (
                spy_rets[i] * spy_global_w
                + asset_ret("EEM", i) * eem_w
                + asset_ret("GLD", i) * gld_w
            )
        else:
            # US outperforming: flight to US, concentrate in SPY
            regime = "us_exceptionalism"
...
perturbations = [
    ...
    ("eem_weight=0.15", {**BASE_PARAMS, "eem_weight": 0.15}),
    ("eem_weight=0.30", {**BASE_PARAMS, "eem_weight": 0.30}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "em_relative_momentum",
