#!/usr/bin/env python3
"""Backtest F40: Metals-Miners Operating Leverage Regime.

Self-contained backtest script. Does NOT use the strategy registry.

Signal: GDX/GLD ratio captures operating leverage of gold miners.
  - 20-day momentum of the ratio + ratio vs 40-day SMA
  - miners_strong (mom > 0 AND ratio > SMA): 60% SPY + 20% GDX + 10% GLD + 10% SHY
  - miners_weak (mom < 0 AND ratio < SMA): 40% GLD + 30% SHY + 20% SPY + 10% TLT
  - neutral (signals disagree): 50% SPY + 30% GLD + 10% SHY + 10% TLT
Rebalance every 5 trading days. 5 bps round-trip cost.

Key distinction from GDX-GLD mean-reversion (Track B): that strategy trades the
pair directly. THIS strategy uses the GDX/GLD divergence as a REGIME SIGNAL to
time equity/commodity/bond allocation. Different mechanism, different risk profile.

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_f40_metals_miners.py
"""
...
SLUG = "metals-miners-leverage-v1"
SIGNAL_SYMBOLS: list[str] = ["GDX", "GLD"]
TRADEABLE_SYMBOLS: list[str] = ["SPY", "GDX", "GLD", "SHY", "TLT"]
...
class BacktestParams:
    """Strategy parameters for the metals-miners operating leverage backtest."""

    # Signal parameters
    mom_lookback: int = 20  # 20-day momentum of GDX/GLD ratio
    sma_period: int = 40  # 40-day SMA of GDX/GLD ratio

    # Miners-strong regime allocation (mom > 0 AND ratio > SMA)
    spy_miners_strong: float = 0.60
    gdx_miners_strong: float = 0.20
    gld_miners_strong: float = 0.10
    shy_miners_strong: float = 0.10  # remainder

    # Miners-weak regime allocation (mom < 0 AND ratio < SMA)
    gld_miners_weak: float = 0.40
    shy_miners_weak: float = 0.30
    spy_miners_weak: float = 0.20
    tlt_miners_weak: float = 0.10

    # Neutral regime allocation (signals disagree)
    spy_neutral: float = 0.50
    gld_neutral: float = 0.30
    shy_neutral: float = 0.10
    ...
    def get_regime_weights(self, regime: str) -> dict[str, float]:
        ...
        """
        if regime == "miners_strong":
            allocated = (
                self.spy_miners_strong
                + self.gdx_miners_strong
                + self.gld_miners_strong
                + self.shy_miners_strong
            )
            shy_remainder = round(
                max(0.0, self.shy_miners_strong + (1.0 - allocated)), 4
            )
            return {
                "SPY": self.spy_miners_strong,
                "GDX": self.gdx_miners_strong,
                "GLD": self.gld_miners_strong,
                "SHY": shy_remainder,
                "TLT": 0.0,
            }
        if regime == "miners_weak":
            allocated = (
                self.gld_miners_weak
                + self.shy_miners_weak
                + self.spy_miners_weak
                + self.tlt_miners_weak
            )
            shy_remainder = round(max(0.0, self.shy_miners_weak + (1.0 - allocated)), 4)
            return {
                "SPY": self.spy_miners_weak,
                "GDX": 0.0,
                "GLD": self.gld_miners_weak,
                "SHY": shy_remainder,
                "TLT": self.tlt_miners_weak,
            }
        # neutral
        allocated = (
            self.spy_neutral + self.gld_neutral + self.shy_neutral + self.tlt_neutral
        )
        shy_remainder = round(max(0.0, self.shy_neutral + (1.0 - allocated)), 4)
        return {
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "spy_miners_strong": self.spy_miners_strong,
            "gdx_miners_strong": self.gdx_miners_strong,
            "gld_miners_strong": self.gld_miners_strong,
            "shy_miners_strong": self.shy_miners_strong,
            "gld_miners_weak": self.gld_miners_weak,
            "shy_miners_weak": self.shy_miners_weak,
            "spy_miners_weak": self.spy_miners_weak,
            "tlt_miners_weak": self.tlt_miners_weak,
...
def run_backtest(
    ...
    **kwargs: object,
    ...
    """Run the metals-miners operating leverage regime backtest.
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    # Warmup: sma_period + mom_lookback + buffer
    warmup = params.sma_period + params.mom_lookback + 5
    ...
    # -- Precompute GDX/GLD ratio --
    gdx_closes = [close_by_symbol["GDX"][d] for d in common_dates]
    gld_closes = [close_by_symbol["GLD"][d] for d in common_dates]

    ratio: list[float] = []
    for i in range(len(common_dates)):
        if gld_closes[i] > 0:
            ratio.append(gdx_closes[i] / gld_closes[i])
        else:
            ratio.append(0.0)

    # -- Compute 20-day momentum of the ratio --
    ratio_momentum: list[float | None] = [None] * len(common_dates)
    for i in range(params.mom_lookback, len(common_dates)):
        if ratio[i] > 0 and ratio[i - params.mom_lookback] > 0:
            ratio_momentum[i] = (ratio[i] - ratio[i - params.mom_lookback]) / ratio[
                i - params.mom_lookback
            ]

    # -- Compute 40-day SMA of the ratio --
...
def main() -> None:
    """Run the F40 backtest and save results."""
    result = run_backtest()
    ...
    entry = {
        ...
        "slug": SLUG,
        "strategy_type": "metals_miners_leverage",
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  F40 Metals-Miners Operating Leverage: {SLUG}")
    print("=" * 60)
