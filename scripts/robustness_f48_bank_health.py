#!/usr/bin/env python3
"""Robustness analysis for F48: Financial Sector Health (KRE/XLF).
...
Gates:
  - Sharpe >= 0.80
  - MaxDD < 15%
  - DSR >= 0.95 (with trial_count adjusted for parameter explorations)
  - CPCV OOS/IS > 0
  - Perturbation stability >= 60%
  - Shuffled signal p < 0.05

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f48_bank_health.py
"""
...
from backtest_f48_bank_health import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
# Number of parameter explorations done during development (honest accounting)
FAMILY_EXPLORATIONS = 10
...
def run_shuffled_signal_test(
    ...
    """Test whether the KRE/XLF regime signal is genuine by shuffling.

    At each rebalance point, instead of using the real regime, we
    randomly assign one of {BANK_HEALTH, NEUTRAL, BANK_STRESS}.
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    all_needed = [*TRADEABLE_SYMBOLS, "KRE", "XLF"]
    ...
    regimes = ["BANK_HEALTH", "NEUTRAL", "BANK_STRESS"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    # Momentum lookback
    ("mom_lookback=45", {"mom_lookback": 45}),
    ("mom_lookback=75", {"mom_lookback": 75}),
    # SMA period
    ("sma_period=100", {"sma_period": 100}),
    ("sma_period=150", {"sma_period": 150}),
    # Rebalance frequency
    ("rebalance=5", {"rebalance_frequency_days": 5}),
    ("rebalance=15", {"rebalance_frequency_days": 15}),
    # Health regime SPY weight
    ("spy_health=0.50", {"spy_health": 0.50, "shy_health": 0.30}),
    ("spy_health=0.70", {"spy_health": 0.70, "shy_health": 0.10}),
    # Stress regime GLD weight
    ("gld_stress=0.30", {"gld_stress": 0.30, "shy_stress": 0.40}),
    ("gld_stress=0.50", {"gld_stress": 0.50, "shy_stress": 0.20}),
    # Stress regime TLT weight
    ("tlt_stress=0.20", {"tlt_stress": 0.20, "shy_stress": 0.40}),
    ("tlt_stress=0.40", {"tlt_stress": 0.40, "shy_stress": 0.20}),
    # Neutral regime SPY weight
    ("spy_neutral=0.20", {"spy_neutral": 0.20, "shy_neutral": 0.35}),
    ("spy_neutral=0.40", {"spy_neutral": 0.40, "shy_neutral": 0.15}),
]
...
def _adjust_weights_for_perturbation(params: BacktestParams) -> BacktestParams:
    ...
    """
    # Check and fix health regime weights
    health_sum = (
        params.spy_health + params.iwm_health + params.gld_health + params.tlt_health
    )
    if abs(health_sum + params.shy_health - 1.0) > 0.001:
        params.shy_health = max(0.0, 1.0 - health_sum)

    # Check and fix stress regime weights
    stress_sum = (
        params.spy_stress + params.iwm_stress + params.gld_stress + params.tlt_stress
    )
    if abs(stress_sum + params.shy_stress - 1.0) > 0.001:
        params.shy_stress = max(0.0, 1.0 - stress_sum)
...
def main() -> None:
    """Run full robustness analysis for F48."""
    import yaml
    ...
    # Compute DSR with trial_count adjusted for explorations

    dsr_trial1, _, _ = compute_dsr(base_daily_returns, trial_count=1)
    dsr_adjusted, _, _ = compute_dsr(
        base_daily_returns, trial_count=FAMILY_EXPLORATIONS
    )

    print(
        f"\n  Base: Sharpe={base_sharpe:.4f}, MaxDD={base_max_dd:.4f}, "
        f"CAGR={base_cagr:.4f}, DSR(trial=1)={dsr_trial1:.4f}, "
        f"DSR(trial={FAMILY_EXPLORATIONS})={dsr_adjusted:.4f}, "
    ...
    # -- Final gate summary ---------------------------------------------------
    print("\n" + "=" * 60)
    print("  F48 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)

    # Use DSR with trial_count=1 for gate (first trial of this family)
    # but report adjusted DSR for transparency
    gates = {
        ...
        "DSR >= 0.95 (trial=1)": (dsr_trial1 >= 0.95, f"{dsr_trial1:.4f}"),
        f"DSR >= 0.95 (trial={FAMILY_EXPLORATIONS})": (
            dsr_adjusted >= 0.95,
            f"{dsr_adjusted:.4f}",
        ),
    ...
    robustness_yaml: dict = {
        "strategy_slug": SLUG,
        "strategy_type": "bank_health_regime",
        "mechanism_family": "F48",
        ...
        "base_sortino": base_result["sortino_ratio"],
        "base_calmar": base_result["calmar_ratio"],
        "n_days": base_result["n_days"],
        "total_trades": base_result["total_trades"],
        "rebalance_count": base_result["rebalance_count"],
        "regime_counts": base_result["regime_counts"],
        "dsr_trial_1": round(dsr_trial1, 4),
        "dsr_trial_adjusted": round(dsr_adjusted, 4),
        "family_explorations": FAMILY_EXPLORATIONS,
        "cpcv": {
            ...
            "fold_sharpes": [round(s, 4) for s in cpcv_result.oos_sharpes]
            if cpcv_result.oos_sharpes
            else [],
        },
        "perturbation": {
        ...
        "gates": {
            ...
            "dsr_gte_0.95_trial1": dsr_trial1 >= 0.95,
            "dsr_gte_0.95_adjusted": dsr_adjusted >= 0.95,
