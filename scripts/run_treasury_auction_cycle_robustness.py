#!/usr/bin/env python3
"""Robustness analysis for treasury-auction-cycle-v1 (F24 candidate).

Mechanism: Predictable TLT supply pressure around treasury auctions.
Pre-auction: dealers hedge by shorting -> TLT dips.
Post-auction: hedges unwind -> TLT recovers.
Calendar-based signal: buy TLT on days 5-8 of month, hold SHY otherwise.

This is DIFFERENT from F6 rate momentum: F6 uses multi-day TLT return magnitude,
this uses calendar predictability (day-of-month patterns).

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_treasury_auction_cycle_robustness.py
"""
...
SLUG = "treasury-auction-cycle-v1"
SYMBOLS = ["TLT", "SHY", "SPY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
BASE_PARAMS = {
    "buy_start": 5,  # Day of month to start buying TLT
    "buy_end": 8,  # Day of month to stop buying TLT
    "tlt_weight": 0.80,  # TLT weight during buy window
    "shy_weight": 0.90,  # SHY weight outside buy window
}
...
def run_single(params):
    ...
    buy_start = int(params.get("buy_start", 5))
    buy_end = int(params.get("buy_end", 8))
    tlt_weight = float(params.get("tlt_weight", 0.80))
    shy_weight = float(params.get("shy_weight", 0.90))
    ...
    for i in range(WARMUP, n):
        # Calendar signal: use day-of-month of previous trading day (lag-1)
        prev_date = dates[i - 1]
        day_of_month = prev_date.day

        tlt_r = asset_ret("TLT", i)
        shy_r = asset_ret("SHY", i)

        if buy_start <= day_of_month <= buy_end:
            # Auction window: buy TLT
            regime = "auction_buy"
            day_ret = tlt_r * tlt_weight
        else:
            # Outside window: hold SHY
            regime = "defensive"
            day_ret = shy_r * shy_weight
...
# ==============================================================================
perturbations = [
    ("buy_start=4", {**BASE_PARAMS, "buy_start": 4}),
    ("buy_start=6", {**BASE_PARAMS, "buy_start": 6}),
    ("buy_end=7", {**BASE_PARAMS, "buy_end": 7}),
    ("buy_end=9", {**BASE_PARAMS, "buy_end": 9}),
    ("tlt_weight=0.60", {**BASE_PARAMS, "tlt_weight": 0.60}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "treasury_auction_cycle",
    "mechanism_family": "F24",
