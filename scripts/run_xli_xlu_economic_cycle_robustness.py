#!/usr/bin/env python3
"""Robustness analysis for xli-xlu-economic-cycle-v1 (F20 Economic Cycle Signal).

Hypothesis: XLI/XLU (Industrials / Utilities) price ratio 30-day momentum
captures the economic cycle. When industrials outperform utilities (XLI/XLU
ratio momentum > 0), economy is expanding — hold equities. When utilities
outperform industrials (ratio momentum <= 0), economy is contracting or
defensive — rotate into gold and short-term treasuries.

Signal:
- Compute XLI/XLU price ratio at close t-1
- Compute lookback-day ratio momentum (ratio_now / ratio_lookback_ago - 1)
- ratio_mom > 0 (expansion): 80% SPY
- ratio_mom <= 0 (contraction): 50% GLD + 20% SHY

Base parameters: lookback=30, spy_expansion_weight=0.80,
  gld_contraction_weight=0.50, shy_contraction_weight=0.20.

Perturbations:
  1. lookback=20 (faster signal)
  2. lookback=45 (slower signal)
  3. spy_expansion_weight=0.70 (less aggressive expansion)
  4. spy_expansion_weight=0.90 (more aggressive expansion)
  5. gld_contraction_weight=0.40 (less gold defensive)
  6. gld_contraction_weight=0.60 (more gold defensive)

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_xli_xlu_economic_cycle_robustness.py
"""
...
SLUG = "xli-xlu-economic-cycle-v1"
SYMBOLS = ["XLI", "XLU", "SPY", "GLD", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
BASE_PARAMS = {
    "lookback": 30,
    "spy_expansion_weight": 0.80,
    "gld_contraction_weight": 0.50,
    "shy_contraction_weight": 0.20,
}
...
def run_single(params):
    ...
    spy_expansion_w = float(params.get("spy_expansion_weight", 0.80))
    gld_contraction_w = float(params.get("gld_contraction_weight", 0.50))
    shy_contraction_w = float(params.get("shy_contraction_weight", 0.20))
    ...
    for i in range(WARMUP, n):
        ...
        xli_now = sym_data["XLI"].get(d_now, 0.0)
        xlu_now = sym_data["XLU"].get(d_now, 0.0)
        xli_lb = sym_data["XLI"].get(d_lb, 0.0)
        xlu_lb = sym_data["XLU"].get(d_lb, 0.0)

        if xli_now <= 0 or xlu_now <= 0 or xli_lb <= 0 or xlu_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = xli_now / xlu_now
        ratio_lb = xli_lb / xlu_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime and allocate
        if ratio_mom > 0:
            # Industrials outperforming utilities → expansion → equities
            regime = "expansion"
            day_ret = spy_rets[i] * spy_expansion_w
        else:
            # Utilities outperforming industrials → contraction → defensive
            regime = "contraction"
            day_ret = (
                asset_ret("GLD", i) * gld_contraction_w
                + asset_ret("SHY", i) * shy_contraction_w
            )
...
perturbations = [
    ...
    ("spy_expansion_weight=0.70", {**BASE_PARAMS, "spy_expansion_weight": 0.70}),
    ("spy_expansion_weight=0.90", {**BASE_PARAMS, "spy_expansion_weight": 0.90}),
    ("gld_contraction_weight=0.40", {**BASE_PARAMS, "gld_contraction_weight": 0.40}),
    ("gld_contraction_weight=0.60", {**BASE_PARAMS, "gld_contraction_weight": 0.60}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "xli_xlu_economic_cycle",
