#!/usr/bin/env python3
"""Robustness analysis for F40: Metals-Miners Operating Leverage Regime.
...
4. Perturbation tests across all key parameters (>= 12 variants)
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f40_metals_miners.py
"""
...
from backtest_f40_metals_miners import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def run_shuffled_signal_test(
    ...
    we randomly assign one of {miners_strong, miners_weak, neutral}. If
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    regimes = ["miners_strong", "miners_weak", "neutral"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    ...
    # spy_miners_strong: 0.50, 0.70 (vs base 0.60)
    ("spy_miners_strong=0.50", {"spy_miners_strong": 0.50}),
    ("spy_miners_strong=0.70", {"spy_miners_strong": 0.70}),
    # gdx_miners_strong: 0.15, 0.30 (vs base 0.20)
    ("gdx_miners_strong=0.15", {"gdx_miners_strong": 0.15}),
    ("gdx_miners_strong=0.30", {"gdx_miners_strong": 0.30}),
    # gld_miners_weak: 0.35, 0.50 (vs base 0.40)
    ("gld_miners_weak=0.35", {"gld_miners_weak": 0.35}),
    ("gld_miners_weak=0.50", {"gld_miners_weak": 0.50}),
    # rebalance_frequency_days: 3, 10 (vs base 5)
...
def main() -> None:
    """Run full robustness analysis for F40."""
    import yaml
    ...
    # -- Final gate summary --
    print("\n" + "=" * 60)
    print("  F40 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
