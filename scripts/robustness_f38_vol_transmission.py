#!/usr/bin/env python3
"""Robustness analysis for F38: Cross-Asset Volatility Transmission.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f38_vol_transmission.py
"""
...
from backtest_f38_vol_transmission import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def run_shuffled_signal_test(
    ...
    we randomly assign one of {bond_stress, equity_stress, neutral}. If the
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    days_since_rebalance = params.rebalance_freq
    cost_per_trade = params.round_trip_cost_bps / 10_000.0

    regimes = ["bond_stress", "equity_stress", "neutral"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    # vol_window: 15, 30 (vs base 20)
    ("vol_window=15", {"vol_window": 15}),
    ("vol_window=30", {"vol_window": 30}),
    ...
    # spy_bond_stress: 0.10, 0.30 (vs base 0.20)
    ("spy_bond_stress=0.10", {"spy_bond_stress": 0.10}),
    ("spy_bond_stress=0.30", {"spy_bond_stress": 0.30}),
    # gld_bond_stress: 0.30, 0.50 (vs base 0.40)
    ("gld_bond_stress=0.30", {"gld_bond_stress": 0.30}),
    ("gld_bond_stress=0.50", {"gld_bond_stress": 0.50}),
    # spy_equity_stress: 0.60, 0.80 (vs base 0.70)
    ("spy_equity_stress=0.60", {"spy_equity_stress": 0.60}),
    ("spy_equity_stress=0.80", {"spy_equity_stress": 0.80}),
]
...
def main() -> None:
    """Run full robustness analysis for F38."""
    import yaml
    ...
    # Step 4: CPCV (15 groups, 3 test, purge=5)
    logger.info("=" * 60)
    ...
    # -- Final gate summary --
    print("\n" + "=" * 60)
    print("  F38 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
