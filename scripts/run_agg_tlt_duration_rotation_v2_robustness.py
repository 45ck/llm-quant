#!/usr/bin/env python3
"""Robustness analysis for agg-tlt-duration-rotation-v2 (F14 Duration Rotation).

Hypothesis: AGG/TLT Duration Rotation v2 (lookback=20)
Economic rationale: AGG has ~6yr duration vs TLT ~17yr. Shorter 20-day lookback
captures institutional portfolio rebalancing cycles at AGG's faster duration response.
V1 used lookback=30 and showed Sharpe=0.792 MaxDD=15.4% (narrow misses on both gates).
V1 perturbation at lookback=20 showed Sharpe=0.915 MaxDD=13.4%.

Pre-specified change from v1: lookback 30 -> 20. All other parameters identical.

Signal: AGG/TLT price ratio 20-day momentum.
...
Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_agg_tlt_duration_rotation_v2_robustness.py
"""
...
BASE_PARAMS = {
    "lookback": 20,  # CHANGED from v1's 30
...
def run_single(params):
    ...
    spy_steepening_w = float(params.get("spy_steepening_weight", 0.80))
    spy_flattening_w = float(params.get("spy_flattening_weight", 0.50))
    gld_flattening_w = float(params.get("gld_flattening_weight", 0.20))
    ...
    for i in range(WARMUP, n):
        ...
        if ratio_mom > 0:
            # AGG outperforming TLT = curve flattening: balanced risk-on
            regime = "flattening"
            day_ret = (
                spy_rets[i] * spy_flattening_w + asset_ret("GLD", i) * gld_flattening_w
            )
        else:
            # TLT outperforming AGG = steepening/rate cuts: concentrated equity
            regime = "steepening"
            day_ret = spy_rets[i] * spy_steepening_w
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "agg_tlt_duration_rotation",
    "version": "v2",
    "change_from_v1": "lookback 30 -> 20 (pre-specified from v1 perturbation analysis)",
