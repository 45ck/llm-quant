#!/usr/bin/env python3
"""Robustness analysis for sector-dispersion-mr-v1 (F25 candidate).

Mechanism: When XLK-XLU return spread widens beyond 2-sigma of its rolling
window, sector rotation is overextended -> mean-revert to equal-weight SPY.
When spread is normal, ride the momentum direction.

This is DIFFERENT from F12 XLK-XLE sector rotation: F12 trades XLK/XLE ratio
direction. This trades the DISPERSION (spread width) as a mean-reversion signal.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_sector_dispersion_mr_robustness.py
"""
...
SLUG = "sector-dispersion-mr-v1"
SYMBOLS = ["SPY", "QQQ", "XLU", "XLK", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
BASE_PARAMS = {
    "z_threshold": 2.0,
    "lookback": 60,
    "momentum_weight": 0.90,  # Weight for momentum regime (QQQ or XLU)
    "mr_weight": 0.45,  # SPY weight during mean-reversion regime
}
...
def run_single(params):
    ...
    momentum_weight = float(params.get("momentum_weight", 0.90))
    mr_weight = float(params.get("mr_weight", 0.45))

    daily_returns = []
    cost_per_switch = 0.0003  # 3 bps

    # Precompute daily return spreads (XLK - XLU)
    spread_rets = []
    for i in range(n):
        if i == 0:
            spread_rets.append(0.0)
        else:
            xlk_r = asset_ret("XLK", i)
            xlu_r = asset_ret("XLU", i)
            spread_rets.append(xlk_r - xlu_r)
    ...
    for i in range(WARMUP, n):
        ...
        # Compute rolling z-score of cumulative spread at close i-1 (lag-1)
        # Use rolling sum of daily return spreads as cumulative spread
        window = spread_rets[i - lookback : i]
        if len(window) < lookback:
            daily_returns.append(0.0)
            continue

        cum_spread = sum(window)
        w_mean = (
            sum(sum(spread_rets[j - lookback : j]) for j in range(i - lookback, i))
            / lookback
        )
        w_std = (
            sum(
                (sum(spread_rets[j - lookback : j]) - w_mean) ** 2
        ...
        z_score = (cum_spread - w_mean) / w_std

        # Determine regime
        qqq_r = asset_ret("QQQ", i)
        xlu_r = asset_ret("XLU", i)

        if abs(z_score) > z_threshold:
            # Overextended -> mean-revert to SPY
            regime = "mean_revert"
            day_ret = spy_rets[i] * mr_weight
        elif z_score > 0:
            # Normal tech momentum -> ride QQQ
            regime = "tech_momentum"
            day_ret = qqq_r * momentum_weight
        else:
            # Utilities leading -> ride XLU
            regime = "util_momentum"
            day_ret = xlu_r * momentum_weight
...
# ==============================================================================
perturbations = [
    ("z_threshold=1.5", {**BASE_PARAMS, "z_threshold": 1.5}),
    ("z_threshold=2.5", {**BASE_PARAMS, "z_threshold": 2.5}),
    ("lookback=40", {**BASE_PARAMS, "lookback": 40}),
    ("lookback=90", {**BASE_PARAMS, "lookback": 90}),
    ("momentum_weight=0.70", {**BASE_PARAMS, "momentum_weight": 0.70}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "sector_dispersion_mr",
    "mechanism_family": "F25",
