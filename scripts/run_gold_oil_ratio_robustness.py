#!/usr/bin/env python3
"""Robustness analysis for gold-oil-ratio-v1 (F19, Gold-Oil Fear Gauge).

Mechanism: GLD/USO price ratio 30-day momentum captures the fear-vs-growth
macro regime. When gold outperforms oil (fear premium expanding, flight to
safety): go defensive. When oil outperforms gold (growth expectations rising):
hold equity.

This is DIFFERENT from F18 (commodity carry, which uses USO/DBC ratio).
This uses GLD/USO -- gold vs oil as a macro fear gauge.

Signal:
  - Compute GLD/USO price ratio at close t-1
  - Compute 30-day ratio momentum (ratio_now / ratio_30d_ago - 1)
  - ratio_mom > 0 (gold outperforming oil, fear rising): 50% GLD + 20% SHY + 10% SPY
  - ratio_mom < 0 (oil outperforming gold, growth): 80% SPY

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_gold_oil_ratio_robustness.py
"""
...
SLUG = "gold-oil-ratio-v1"
SYMBOLS = ["GLD", "USO", "SPY", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
BASE_PARAMS = {
    "lookback": 30,
    "spy_growth_weight": 0.80,
    "gld_fear_weight": 0.50,
    "shy_weight": 0.20,
    "spy_fear_weight": 0.10,
}
...
def run_single(params):
    ...
    spy_growth_w = float(params.get("spy_growth_weight", 0.80))
    gld_fear_w = float(params.get("gld_fear_weight", 0.50))
    shy_w = float(params.get("shy_weight", 0.20))
    spy_fear_w = float(params.get("spy_fear_weight", 0.10))
    ...
    for i in range(WARMUP, n):
        ...
        # GLD/USO ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        ...
        if gld_now <= 0 or uso_now <= 0 or gld_lb <= 0 or uso_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = gld_now / uso_now
        ratio_lb = gld_lb / uso_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on GLD/USO ratio momentum
        if ratio_mom > 0:
            # Fear regime: gold outperforming oil, flight to safety
            # 50% GLD + 20% SHY + 10% SPY (defensive allocation)
            regime = "fear"
            day_ret = (
                asset_ret("GLD", i) * gld_fear_w
                + asset_ret("SHY", i) * shy_w
                + spy_rets[i] * spy_fear_w
            )
        else:
            # Growth regime: oil outperforming gold, growth expectations rising
            # 80% SPY
            regime = "growth"
            day_ret = spy_rets[i] * spy_growth_w
...
perturbations = [
    ...
    ("spy_growth_weight=0.70", {**BASE_PARAMS, "spy_growth_weight": 0.70}),
    ("spy_growth_weight=0.90", {**BASE_PARAMS, "spy_growth_weight": 0.90}),
    ("gld_fear_weight=0.40", {**BASE_PARAMS, "gld_fear_weight": 0.40}),
    ("gld_fear_weight=0.60", {**BASE_PARAMS, "gld_fear_weight": 0.60}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "gold_oil_fear_gauge",
