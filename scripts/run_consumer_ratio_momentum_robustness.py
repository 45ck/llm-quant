#!/usr/bin/env python3
"""Robustness analysis for consumer-ratio-momentum-v1 (F19 Consumer Sentiment).
...
- ratio_mom < 0 (defensive consumer): 50% GLD + 20% SHY + 10% SPY

Base parameters: lookback=30, spy_risk_on_weight=0.80, gld_weight=0.50,
  shy_weight=0.20, spy_defensive_weight=0.10.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_consumer_ratio_momentum_robustness.py
"""
...
SLUG = "consumer-ratio-momentum-v1"
...
def run_single(params):
    ...
    gld_weight = float(params.get("gld_weight", 0.50))
...
perturbations = [
    ...
    ("gld_weight=0.60", {**BASE_PARAMS, "gld_weight": 0.60}),
]
