#!/usr/bin/env python3
"""Robustness analysis for F47: Homebuilder Lead Indicator.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f47_homebuilder.py
"""
...
from backtest_f47_homebuilder import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def run_shuffled_signal_test(
    ...
    """Test whether the homebuilder lead signal is genuine by shuffling.

    At each rebalance point, instead of using the real regime, we
    randomly assign one of {HOUSING_EXPANSION, NEUTRAL, HOUSING_CONTRACTION}.
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    all_needed = [*TRADEABLE_SYMBOLS, "XHB"]
    ...
    regimes = ["HOUSING_EXPANSION", "NEUTRAL", "HOUSING_CONTRACTION"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    ...
    # Expansion regime SPY weight
    ("spy_expansion=0.60", {"spy_expansion": 0.60, "shy_expansion": 0.25}),
    ("spy_expansion=0.80", {"spy_expansion": 0.80, "shy_expansion": 0.05}),
    # Expansion regime QQQ weight
    ("qqq_expansion=0.10", {"qqq_expansion": 0.10, "shy_expansion": 0.20}),
    ("qqq_expansion=0.25", {"qqq_expansion": 0.25, "shy_expansion": 0.05}),
    # Contraction regime GLD weight
    ("gld_contraction=0.25", {"gld_contraction": 0.25, "shy_contraction": 0.35}),
    ("gld_contraction=0.40", {"gld_contraction": 0.40, "shy_contraction": 0.20}),
    # Contraction regime TLT weight
    ("tlt_contraction=0.15", {"tlt_contraction": 0.15, "shy_contraction": 0.35}),
    ("tlt_contraction=0.30", {"tlt_contraction": 0.30, "shy_contraction": 0.20}),
    # Rebalance frequency
...
def _adjust_weights_for_perturbation(params: BacktestParams) -> BacktestParams:
    ...
    """
    # Check and fix expansion regime weights
    expansion_sum = (
        params.spy_expansion
        + params.qqq_expansion
        + params.gld_expansion
        + params.tlt_expansion
    )
    if abs(expansion_sum + params.shy_expansion - 1.0) > 0.001:
        params.shy_expansion = max(0.0, 1.0 - expansion_sum)

    # Check and fix contraction regime weights
    contraction_sum = (
        params.spy_contraction
        + params.qqq_contraction
        + params.gld_contraction
        + params.tlt_contraction
    )
    if abs(contraction_sum + params.shy_contraction - 1.0) > 0.001:
        params.shy_contraction = max(0.0, 1.0 - contraction_sum)
...
def main() -> None:
    """Run full robustness analysis for F47."""
    import yaml
    ...
    if base_sharpe < 0.70:
        ...
        robustness_yaml: dict = {
            "strategy_slug": SLUG,
            "strategy_type": "homebuilder_lead_indicator",
            "mechanism_family": "F47",
    ...
    # ── Final gate summary ───────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("  F47 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
