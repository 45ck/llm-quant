#!/usr/bin/env python3
"""Robustness analysis for xle-xop-mean-reversion-v1 (Energy Sector Pairs, Track B).

Hypothesis: XLE/XOP ratio (broad energy vs pure E&P) exhibits mean-reversion.
XLE includes integrated majors (Exxon, Chevron) with diversified revenue streams,
while XOP is pure oil & gas exploration/production, tracking crude oil more directly.
The ratio should mean-revert as relative valuations oscillate around fair value.

Signal: XLE/XOP price ratio z-score (60-day BB window, 2.0 std).
  - z < -1.0 (XLE cheap vs XOP): 50% XLE + 20% XOP
  - z > +1.0 (XLE expensive vs XOP): 50% XOP + 20% XLE
  - neutral: 35% XLE + 35% XOP

ZERO broad equity exposure -- both are energy sector ETFs. Should yield
near-zero correlation with equity-timing or cross-asset strategies.
...
Perturbations:
  1. bb_window=40 (faster mean-reversion lookback)
  2. bb_window=90 (slower mean-reversion lookback)
  3. bb_std=1.5 (tighter bands, more frequent signals)
  4. bb_std=2.5 (wider bands, fewer signals)
  5. overweight=0.40 (less aggressive tilt)
  6. overweight=0.60 (more aggressive tilt)

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_xle_xop_mean_reversion_robustness.py
"""
...
SLUG = "xle-xop-mean-reversion-v1"
SYMBOLS = ["XLE", "XOP"]
DD_THRESHOLD = 0.30  # Track B gate
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
# Build price series -- use XLE dates as the reference timeline
xle_df = prices.filter(pl.col("symbol") == "XLE").sort("date")
dates = xle_df["date"].to_list()
n = len(dates)
...
def run_single(params: dict) -> dict:
    ...
    Signal logic (all using close t-1 to avoid look-ahead):
      - Compute XLE/XOP price ratio
      - Compute bb_window-day rolling mean and std of ratio
      - Z-score = (ratio - mean) / std
      - Z < -1.0: regime='long_xle' (XLE cheap vs XOP) -> overweight XLE + underweight XOP
      - Z > +1.0: regime='long_xop' (XLE expensive vs XOP) -> overweight XOP + underweight XLE
      - else:     regime='neutral'                          -> neutral_weight in each
    """
    bb_window = int(params.get("bb_window", 60))
    bb_std = float(params.get("bb_std", 2.0))  # used for z-score band width
    overweight = float(params.get("overweight", 0.50))
    ...
    # z-score thresholds derived from bb_std
    z_low = -bb_std / 2.0  # -1.0 at bb_std=2.0
    z_high = bb_std / 2.0  # +1.0 at bb_std=2.0

    # Precompute full ratio history for mean/std computation
    ratio_by_idx: dict[int, float] = {}
    for i in range(n):
        d = dates[i]
        xle_p = sym_data["XLE"].get(d, 0.0)
        xop_p = sym_data["XOP"].get(d, 0.0)
        if xle_p > 0 and xop_p > 0:
            ratio_by_idx[i] = xle_p / xop_p
    ...
    for i in range(WARMUP, n):
        ...
        if z < z_low:
            # XLE cheap relative to XOP -> overweight XLE, underweight XOP
            regime = "long_xle"
            xle_w = overweight
            xop_w = underweight
        elif z > z_high:
            # XLE expensive relative to XOP -> overweight XOP, underweight XLE
            regime = "long_xop"
            xle_w = underweight
            xop_w = overweight
        else:
            # Neutral: equal baseline
            regime = "neutral"
            xle_w = neutral_w
            xop_w = neutral_w

        # Compute weighted daily return from day i's actual returns
        day_ret = asset_ret("XLE", i) * xle_w + asset_ret("XOP", i) * xop_w
...
# ==============================================================================
perturbations = [
    ("bb_window=40", {**BASE_PARAMS, "bb_window": 40}),
    ("bb_window=90", {**BASE_PARAMS, "bb_window": 90}),
    ("bb_std=1.5", {**BASE_PARAMS, "bb_std": 1.5}),
    ("bb_std=2.5", {**BASE_PARAMS, "bb_std": 2.5}),
    ("overweight=0.40", {**BASE_PARAMS, "overweight": 0.40}),
    ("overweight=0.60", {**BASE_PARAMS, "overweight": 0.60}),
]
...
# ==============================================================================
# Benchmark: equal-weight 50/50 XLE/XOP portfolio (the naive no-signal alternative)
# The shuffled test checks whether our z-score timing beats random regime assignment
print("\n--- SHUFFLED SIGNAL TEST ---")
eq_weight_rets: list[float] = []
for i in range(WARMUP, n):
    eq_weight_rets.append(0.5 * asset_ret("XLE", i) + 0.5 * asset_ret("XOP", i))
...
print(f"  Benchmark (50/50 XLE/XOP) returns: {len(aligned_bench)} days")
...
gate1 = base["sharpe"] > 1.0  # Track B: Sharpe > 1.0
gate2 = base["max_dd"] < DD_THRESHOLD  # Track B: MaxDD < 30%
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "xle_xop_mean_reversion",
    "mechanism_family": "F2",
    "track": "B",
    "hypothesis": (
        "XLE/XOP ratio (broad energy vs pure E&P) exhibits mean-reversion. "
        "XLE includes integrated majors (Exxon, Chevron) while XOP is pure "
        "oil & gas exploration/production. When XLE is cheap relative to XOP "
        "(z < -1.0), overweight XLE (50%). When XLE is expensive (z > +1.0), "
        "overweight XOP (50%). Always holds both assets -- intra-sector pairs "
        "trade with zero broad equity exposure."
    ),
