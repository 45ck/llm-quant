#!/usr/bin/env python3
"""Robustness analysis for xlf-xlu-rate-sensitivity-v1 (F20 Rate Sensitivity).

Mechanism: XLF/XLU price ratio 30-day momentum captures rate sensitivity regime.
  - ratio_mom > 0 (financials > utilities, risk-on): 80% SPY
  - ratio_mom < 0 (utilities > financials, defensive): 50% GLD + 20% SHY

When financials outperform utilities, rates are rising and risk appetite is strong
-- overweight equities. When utilities outperform financials -- defensive positioning.
This is a NEW mechanism family (F20) distinct from existing sector rotation strategies.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_xlf_xlu_rate_sensitivity_robustness.py
"""
...
SLUG = "xlf-xlu-rate-sensitivity-v1"
SYMBOLS = ["XLF", "XLU", "SPY", "GLD", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
def run_single(params):
    ...
    for i in range(WARMUP, n):
        ...
        # XLF/XLU ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        xlf_now = sym_data["XLF"].get(d_now, 0.0)
        xlu_now = sym_data["XLU"].get(d_now, 0.0)
        xlf_lb = sym_data["XLF"].get(d_lb, 0.0)
        xlu_lb = sym_data["XLU"].get(d_lb, 0.0)

        if xlf_now <= 0 or xlu_now <= 0 or xlf_lb <= 0 or xlu_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = xlf_now / xlu_now
        ratio_lb = xlf_lb / xlu_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on XLF/XLU ratio momentum
        if ratio_mom > 0:
            # Risk-on: financials outperforming utilities, rates rising
            # 80% SPY
            regime = "risk_on"
            day_ret = spy_rets[i] * spy_risk_on_w
        else:
            # Defensive: utilities outperforming financials, rate sensitivity
            # 50% GLD + 20% SHY
            regime = "defensive"
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "rate_sensitivity_barometer",
