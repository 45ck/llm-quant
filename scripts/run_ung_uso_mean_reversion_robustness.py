#!/usr/bin/env python3
"""Robustness analysis for ung-uso-mean-reversion-v1 (Track B Energy Mean Reversion).

Hypothesis: UNG/USO natural gas vs crude oil ratio exhibits mean-reversion.
Natural gas and crude oil are energy commodities that share macro drivers
(geopolitics, demand cycles, dollar strength) but diverge on weather,
storage dynamics, and substitution effects. The ratio should mean-revert
when one commodity becomes cheap relative to the other.

Signal:
  - Compute UNG/USO price ratio at close t-1
  - Compute bb_window-day rolling mean and rolling std of ratio
  - Z-score = (ratio - mean) / std
  - Z < -1.0 (UNG cheap vs USO): 50% UNG + 20% USO
  - Z > +1.0 (UNG expensive vs USO): 50% USO + 20% UNG
  - Neutral: 35% UNG + 35% USO

ZERO equity exposure -- pure commodity pairs trade.

Track B gates: Sharpe > 1.0, MaxDD < 30%, DSR >= 0.95, CPCV OOS > 0,
               Perturbation >= 60%, Shuffled p < 0.05

Perturbations:
  1. bb_window=40 (faster mean-reversion lookback)
  2. bb_window=90 (slower mean-reversion lookback)
  3. bb_std=1.5 (tighter bands)
  4. bb_std=2.5 (wider bands)
  5. overweight=0.40 (less aggressive tilt)
  6. overweight=0.60 (more aggressive tilt)

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_ung_uso_mean_reversion_robustness.py
"""
...
SLUG = "ung-uso-mean-reversion-v1"
SYMBOLS = ["UNG", "USO"]
DD_THRESHOLD = 0.30  # Track B
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
# Build price series -- use UNG dates as the reference timeline
ung_df = prices.filter(pl.col("symbol") == "UNG").sort("date")
dates = ung_df["date"].to_list()
n = len(dates)
...
def run_single(params: dict) -> dict:
    ...
    Signal logic (all using close t-1 to avoid look-ahead):
      - Compute UNG/USO price ratio
      - Compute bb_window-day rolling mean and rolling std of ratio
      - Z-score = (ratio - mean) / std
      - Z < -1.0: regime='long_ung' (UNG cheap) -> overweight UNG + underweight USO
      - Z > +1.0: regime='long_uso' (UNG expensive) -> overweight USO + underweight UNG
      - else:     regime='neutral'                   -> neutral_weight in each
    """
    bb_window = int(params.get("bb_window", 60))
    _bb_std = float(params.get("bb_std", 2.0))  # reserved for z-score threshold
    ...
    # UNG/USO ratio at each date where both have data
    ratio_by_idx: dict[int, float] = {}
    for i in range(n):
        d = dates[i]
        ung_p = sym_data["UNG"].get(d, 0.0)
        uso_p = sym_data["USO"].get(d, 0.0)
        if ung_p > 0 and uso_p > 0:
            ratio_by_idx[i] = ung_p / uso_p
    ...
    for i in range(WARMUP, n):
        ...
        # Collect ratio values for indices [i-bb_window, ..., i-1]
        ...
        # Determine regime and weights using fixed z-score thresholds
        if z < -1.0:
            # UNG cheap relative to USO -> overweight UNG, underweight USO
            regime = "long_ung"
            ung_w = overweight
            uso_w = underweight
        elif z > 1.0:
            # UNG expensive relative to USO -> overweight USO, underweight UNG
            regime = "long_uso"
            ung_w = underweight
            uso_w = overweight
        else:
            # Neutral: balanced weight in each
            regime = "neutral"
            ung_w = neutral_w
            uso_w = neutral_w

        # Compute weighted daily return from day i's actual returns
        day_ret = asset_ret("UNG", i) * ung_w + asset_ret("USO", i) * uso_w
...
# ==============================================================================
# Benchmark: equal-weight 50/50 UNG/USO portfolio (the naive no-signal alternative)
# The shuffled test checks whether our z-score timing beats random regime assignment
print("\n--- SHUFFLED SIGNAL TEST ---")
# Compute equal-weight UNG+USO returns as the benchmark asset
eq_weight_rets: list[float] = []
for i in range(WARMUP, n):
    eq_weight_rets.append(0.5 * asset_ret("UNG", i) + 0.5 * asset_ret("USO", i))
...
print(f"  Benchmark (50/50 UNG/USO) returns: {len(aligned_bench)} days")
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "ung_uso_mean_reversion",
    "mechanism_family": "F2",
    "track": "B",
    "hypothesis": (
        "UNG/USO ratio (natural gas vs crude oil) exhibits mean-reversion. "
        "When UNG is cheap relative to USO (z < -1.0), overweight UNG. "
        "When UNG is expensive relative to USO (z > +1.0), overweight USO. "
        "Always holds both assets -- commodity pairs trade, zero equity exposure."
    ),
