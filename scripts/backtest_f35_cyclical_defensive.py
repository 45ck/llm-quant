#!/usr/bin/env python3
"""Backtest F35: Cyclical-Defensive Sector Breadth.

Self-contained backtest script. Does NOT use the strategy registry.

Signal: XLY/XLP ratio as real-economy expansion/contraction proxy,
confirmed by XLI industrial momentum. Three regimes:
  - expansion: ratio_mom > 0, ratio > ratio_sma, xli_mom > 0 -> 80% SPY
  - contraction: ratio_mom < 0, ratio < ratio_sma, xli_mom < 0 -> 40% GLD + 40% TLT + 10% SHY
  - transition: mixed signals -> 50% SPY + 30% GLD + 10% SHY
Rebalance every 5 trading days. 5 bps round-trip cost.

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_f35_cyclical_defensive.py
"""
...
SLUG = "cyclical-defensive-breadth-v1"
SIGNAL_SYMBOLS: list[str] = ["XLY", "XLP", "XLI"]
TRADEABLE_SYMBOLS: list[str] = ["SPY", "GLD", "TLT", "SHY"]
...
class BacktestParams:
    """Strategy parameters for the cyclical-defensive breadth backtest."""

    ratio_lookback: int = 20
    sma_period: int = 40
    xli_lookback: int = 20
    spy_expansion: float = 0.80
    gld_contraction: float = 0.40
    tlt_contraction: float = 0.40
    shy_contraction: float = 0.10
    spy_transition: float = 0.50
    gld_transition: float = 0.30
    shy_transition: float = 0.10
    ...
    def get_regime_weights(self, regime: str) -> dict[str, float]:
        ...
        if regime == "expansion":
            return {
                "SPY": self.spy_expansion,
                "GLD": 0.0,
                "TLT": 0.0,
                "SHY": round(1.0 - self.spy_expansion, 4),
            }
        if regime == "contraction":
            shy_w = max(0.0, 1.0 - self.gld_contraction - self.tlt_contraction)
            return {
                "SPY": 0.0,
                "GLD": self.gld_contraction,
                "TLT": self.tlt_contraction,
                "SHY": round(shy_w, 4),
            }
        # transition
        shy_w = max(0.0, 1.0 - self.spy_transition - self.gld_transition)
        return {
            "SPY": self.spy_transition,
            "GLD": self.gld_transition,
            "TLT": 0.0,
            "SHY": round(shy_w, 4),
        }
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "xli_lookback": self.xli_lookback,
            "spy_expansion": self.spy_expansion,
            "gld_contraction": self.gld_contraction,
            "tlt_contraction": self.tlt_contraction,
            "shy_contraction": self.shy_contraction,
            "spy_transition": self.spy_transition,
            "gld_transition": self.gld_transition,
            "shy_transition": self.shy_transition,
...
def run_cyclical_defensive_backtest(
    params: BacktestParams | None = None,
    return_daily_returns: bool = True,
    prices_df: object | None = None,
) -> dict:
    """Run the cyclical-defensive breadth backtest.
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    # Warmup needs max(sma_period, ratio_lookback, xli_lookback) to have full signal
    ...
    xli_closes = [close_by_symbol["XLI"][d] for d in common_dates]
    ...
    # Ratio momentum (20-day)
    ratio_mom: list[float | None] = [None] * len(common_dates)
    for i in range(params.ratio_lookback, len(common_dates)):
        past = ratio[i - params.ratio_lookback]
        if past > 0:
    ...
    # XLI momentum (20-day)
    xli_mom: list[float | None] = [None] * len(common_dates)
    for i in range(params.xli_lookback, len(common_dates)):
        past = xli_closes[i - params.xli_lookback]
        if past > 0:
            xli_mom[i] = (xli_closes[i] / past) - 1.0
    ...
    def classify_regime(i: int) -> str | None:
        ...
        rs = ratio_sma[i]
        xm = xli_mom[i]
        if rm is None or rs is None or xm is None:
            return None

        r = ratio[i]
        if rm > 0 and r > rs and xm > 0:
            return "expansion"
        if rm < 0 and r < rs and xm < 0:
            return "contraction"
        return "transition"
...
def main() -> None:
    """Run the F35 backtest and save results."""
    result = run_cyclical_defensive_backtest()
    ...
    entry = {
        ...
        "slug": SLUG,
        "strategy_type": "cyclical_defensive_breadth",
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  F35 Cyclical-Defensive Sector Breadth: {SLUG}")
    print("=" * 60)
