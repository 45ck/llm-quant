...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_f46_biotech_risk_v2.py
"""
...
TRIAL_NUMBER = 2  # Second trial of biotech_risk_appetite family


@dataclass
class BacktestParams:
    """Strategy parameters for the biotech risk appetite v2 backtest."""

    mom_lookback: int = 20
    sma_period: int = 40
    rebalance_frequency_days: int = 10  # v2: was 5 in v1
...
def run_backtest(
    ...
    """Run the biotech risk appetite v2 backtest.
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    # DSR (trial_count=2 for second trial of F46 family)
    dsr, psr, sr0 = compute_dsr(strategy_returns, trial_count=TRIAL_NUMBER)
    ...
    logger.info(
        "Backtest complete: Sharpe=%.4f, MaxDD=%.4f, CAGR=%.4f, Trades=%d, DSR=%.4f (trial=%d)",
        sharpe,
        abs(max_dd),
        cagr,
        total_trades,
...
def main() -> None:
    """Run the F46 v2 backtest and save results."""
    result = run_backtest()
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  F46 v2 Biotech Risk Appetite Backtest: {SLUG}")
    print("=" * 60)
    ...
    print(f"  DSR:              {result['dsr']:.4f} (trial_number={TRIAL_NUMBER})")
    print(
    ...
    gates = {
        ...
        "DSR >= 0.95 (trial=2)": result["dsr"] >= 0.95,
    }
    print("\n  Gate Check:")
    ...
    if all_basic:
        print("  --> Sharpe >= 0.70, proceeding to robustness analysis.")
    else:
        print("  --> Check if Sharpe >= 0.70 for robustness eligibility.")
    print()
