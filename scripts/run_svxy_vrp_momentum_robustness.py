#!/usr/bin/env python3
"""Robustness analysis for svxy-vrp-momentum-v1 (F4-VRP-v2).

Mechanism: SVXY/SPY price ratio 20-day momentum as volatility risk premium proxy.
  - Ratio momentum > 0 (VRP positive, vol selling profitable): 80% SPY
  - Ratio momentum < 0 (VRP negative, vol sellers getting hurt): 40% GLD + 30% SHY + 10% SPY

Hypothesis: SVXY outperforming SPY signals that volatility selling is being compensated
(positive VRP) — the market is pricing risk correctly. When SPY outperforms SVXY, vol
sellers are losing money, risk appetite is waning, and defensive posture is warranted.

Base parameters: lookback=20, spy_risk_on_weight=0.80, gld_weight=0.40,
                 shy_weight=0.30, spy_defensive_weight=0.10.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_svxy_vrp_momentum_robustness.py
"""
