#!/usr/bin/env python3
"""Backtest D11: SOXX -> SOXL Semiconductor Lead-Lag (Track D).

Self-contained backtest script. Does NOT use the strategy registry.

Signal: SOXX 5-day rolling return (semiconductor momentum).
  - Entry: SOXX 5d return >= entry_threshold (strong semi momentum)
           -> 40% SOXL + 10% SHY
  - Exit:  SOXX 5d return <= exit_threshold (semi weakness)
           -> 100% SHY
  - VIX > 30 override: 100% SHY (crash filter)
Rebalance every 5 trading days. 10 bps round-trip cost.

Track D gates: Sharpe >= 0.80, MaxDD < 40%, DSR >= 0.90, CPCV OOS > 0

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_d11_soxx_soxl.py
"""
...
SLUG = "soxx-soxl-lead-lag-v1"
SIGNAL_SYMBOLS: list[str] = ["SOXX", "VIX"]
TRADEABLE_SYMBOLS: list[str] = ["SOXL", "SHY"]
BENCHMARK_SYMBOLS: dict[str, float] = {"SOXL": 1.00}  # TQQQ unavailable; use SOXL B&H
...
class BacktestParams:
    """Strategy parameters for the SOXX->SOXL lead-lag backtest."""

    signal_window: int = 7
    entry_threshold: float = 0.03
    exit_threshold: float = -0.01
    soxl_weight: float = 0.50
    shy_weight_risk_on: float = 0.50
    shy_weight_risk_off: float = 1.00
    vix_crash_threshold: float = 35.0
    rebalance_frequency_days: int = 7
    round_trip_cost_bps: float = ROUND_TRIP_COST_BPS

    def get_risk_on_weights(self) -> dict[str, float]:
        """Return target weights for risk-on (entry signal).

        SHY absorbs whatever is not allocated to SOXL so weights sum to 1.0.
        """
        shy_residual = max(0.0, 1.0 - self.soxl_weight)
        return {
            "SOXL": self.soxl_weight,
            "SHY": shy_residual,
        }

    def get_risk_off_weights(self) -> dict[str, float]:
        """Return target weights for risk-off (exit signal or VIX crash)."""
        return {
            "SHY": self.shy_weight_risk_off,
        }
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "soxl_weight": self.soxl_weight,
            "shy_weight_risk_on": self.shy_weight_risk_on,
            "shy_weight_risk_off": self.shy_weight_risk_off,
            "vix_crash_threshold": self.vix_crash_threshold,
...
def run_backtest(
    ...
    """Run the SOXX->SOXL lead-lag backtest.
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    Core engine, strictly causal: at each rebalance point t,
    ...
    all_needed = [*TRADEABLE_SYMBOLS, "SOXX", "VIX"]
    ...
    # Warmup needs signal_window days to have full signal history
    warmup = params.signal_window + 10
    ...
    # ── Precompute SOXX rolling return for all dates ──────────────────────
    soxx_closes = [close_by_symbol["SOXX"][d] for d in common_dates]
    vix_closes = [close_by_symbol["VIX"][d] for d in common_dates]

    # SOXX signal_window-day return: (price[t] / price[t - signal_window]) - 1
    soxx_return: list[float | None] = [None] * len(common_dates)
    for i in range(params.signal_window, len(common_dates)):
        if soxx_closes[i - params.signal_window] > 0:
            soxx_return[i] = (
                soxx_closes[i] / soxx_closes[i - params.signal_window] - 1.0
            )
    ...
    position_state = "RISK_OFF"  # start in cash
    ...
    for i, date_str in enumerate(common_dates):
        ...
        # ── Determine regime from SOXX return + VIX ───────────────────────
        soxx_ret = soxx_return[i]
        vix_level = vix_closes[i]

        if soxx_ret is None:
            continue  # Skip if signal not available

        # VIX crash filter takes priority
        if vix_level > params.vix_crash_threshold:
            regime = "VIX_CRASH"
            target_weights = params.get_risk_off_weights()
            position_state = "RISK_OFF"
        elif soxx_ret >= params.entry_threshold:
            regime = "RISK_ON"
            target_weights = params.get_risk_on_weights()
            position_state = "RISK_ON"
        elif soxx_ret <= params.exit_threshold:
            regime = "RISK_OFF"
            target_weights = params.get_risk_off_weights()
            position_state = "RISK_OFF"
        else:
            # Neutral zone: maintain current position state
            regime = f"HOLD_{position_state}"
            if position_state == "RISK_ON":
                target_weights = params.get_risk_on_weights()
            else:
                target_weights = params.get_risk_off_weights()
...
def main() -> None:
    """Run the D11 backtest and save results."""
    result = run_backtest()
    ...
    entry = {
        ...
        "slug": SLUG,
        "strategy_type": "soxx_soxl_lead_lag",
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  D11 SOXX->SOXL Lead-Lag Backtest: {SLUG}")
    print("=" * 60)
    ...
    if not all_basic:
        print("  --> Strategy does not pass basic gates. Review parameters.")
    print()
