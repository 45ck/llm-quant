#!/usr/bin/env python3
"""Robustness analysis for correlation-mean-reversion-v1 (M6, Correlation Mean-Reversion).

Mechanism: SPY-TLT rolling correlation z-score mean-reversion.
  - Corr z-score > 1.5 (extremely high corr, will revert = decorrelate):
    40% SPY + 40% TLT (both benefit as correlation drops)
  - Corr z-score < -1.5 (extremely negative corr, will revert = correlate up):
    60% GLD + 20% SHY (avoid both SPY and TLT)
  - Neutral zone: 50% SPY + 20% TLT + 10% GLD
Base parameters: corr_window=60, zscore_lookback=252, zscore_thresh=1.5.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_correlation_mean_reversion_robustness.py
"""
...
SLUG = "correlation-mean-reversion-v1"
...
def run_single(params):
    ...
    zscore_lookback = int(params.get("zscore_lookback", 252))
    zscore_thresh = float(params.get("zscore_thresh", 1.5))

    daily_returns = []
    cost_per_switch = 0.0003  # 3 bps

    # Pre-compute SPY and TLT daily returns for correlation calculation
    spy_daily = all_rets["SPY"]
    tlt_daily = all_rets["TLT"]

    prev_regime = None
    for i in range(WARMUP, n):
        # Need enough history for corr_window + zscore_lookback
        min_history = corr_window + zscore_lookback
        if i < min_history + 1:
            daily_returns.append(0.0)
            continue

        # Compute rolling correlations over the zscore_lookback period
        # We need zscore_lookback rolling correlations, each computed over corr_window
        # So we need corr_window + zscore_lookback - 1 days of return data
        # All computed using data up to day i-1 (lag-1, no look-ahead)

        # Compute current rolling correlation (up to day i-1)
        spy_window = spy_daily[i - corr_window : i]
        tlt_window = tlt_daily[i - corr_window : i]
        ...
        mean_s = sum(spy_window) / n_w
        mean_t = sum(tlt_window) / n_w
        cov = (
            sum((spy_window[j] - mean_s) * (tlt_window[j] - mean_t) for j in range(n_w))
            / n_w
        )
        std_s = (sum((spy_window[j] - mean_s) ** 2 for j in range(n_w)) / n_w) ** 0.5
        std_t = (sum((tlt_window[j] - mean_t) ** 2 for j in range(n_w)) / n_w) ** 0.5

        if std_s <= 0 or std_t <= 0:
            daily_returns.append(0.0)
            continue

        current_corr = cov / (std_s * std_t)

        # Compute historical rolling correlations for z-score
        # We need zscore_lookback past correlation values
        hist_corrs = []
        for k in range(zscore_lookback):
            offset = i - 1 - k  # going backwards from i-1
            if offset < corr_window:
                continue
            sw = spy_daily[offset - corr_window + 1 : offset + 1]
            tw = tlt_daily[offset - corr_window + 1 : offset + 1]
            nk = len(sw)
            if nk < corr_window:
                continue
            ms = sum(sw) / nk
            mt = sum(tw) / nk
            cv = sum((sw[j] - ms) * (tw[j] - mt) for j in range(nk)) / nk
            ss = (sum((sw[j] - ms) ** 2 for j in range(nk)) / nk) ** 0.5
            st = (sum((tw[j] - mt) ** 2 for j in range(nk)) / nk) ** 0.5
            if ss > 0 and st > 0:
                hist_corrs.append(cv / (ss * st))

        if len(hist_corrs) < 60:
            daily_returns.append(0.0)
            continue

        # Z-score of current correlation vs historical distribution
        hist_mean = sum(hist_corrs) / len(hist_corrs)
        hist_std = (
            sum((c - hist_mean) ** 2 for c in hist_corrs) / len(hist_corrs)
        ) ** 0.5
        ...
        z = (current_corr - hist_mean) / hist_std

        # Determine regime based on z-score
        if z > zscore_thresh:
            # Correlation extremely high, will revert = decorrelate
            # Both SPY and TLT benefit as correlation drops
            regime = "high_corr"
            day_ret = spy_rets[i] * 0.40 + asset_ret("TLT", i) * 0.40
        elif z < -zscore_thresh:
            # Correlation extremely negative, will revert = correlate up
            # Avoid both SPY and TLT, go defensive
            regime = "low_corr"
            day_ret = asset_ret("GLD", i) * 0.60 + asset_ret("SHY", i) * 0.20
        else:
            # Neutral zone
            regime = "neutral"
            day_ret = (
...
# ==============================================================================
perturbations = [
    ("corr_window=40", {**BASE_PARAMS, "corr_window": 40}),
    ("corr_window=90", {**BASE_PARAMS, "corr_window": 90}),
    ("zscore_thresh=1.0", {**BASE_PARAMS, "zscore_thresh": 1.0}),
    ("zscore_thresh=2.0", {**BASE_PARAMS, "zscore_thresh": 2.0}),
    ("zscore_lookback=200", {**BASE_PARAMS, "zscore_lookback": 200}),
    ("zscore_lookback=300", {**BASE_PARAMS, "zscore_lookback": 300}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "correlation_mean_reversion",
