#!/usr/bin/env python3
"""Robustness analysis for size-ratio-momentum-v1 (F19 Size Premium Timing).

Mechanism: IWM/SPY price ratio 30-day momentum captures the small-cap premium cycle.
When ratio_mom > 0 (small caps leading, risk appetite broadening): 80% SPY.
When ratio_mom <= 0 (large cap flight to quality): 40% SPY + 30% GLD + 10% SHY.
...
Perturbations:
  ...
  5. gld_weight=0.20 (less gold defensive)
  6. gld_weight=0.40 (more gold defensive)

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_size_ratio_momentum_robustness.py
"""
...
SLUG = "size-ratio-momentum-v1"
SYMBOLS = ["IWM", "SPY", "GLD", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
def run_single(params):
    ...
    spy_def_w = float(params.get("spy_defensive_weight", 0.40))
    ...
    for i in range(WARMUP, n):
        ...
        # IWM/SPY ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        ...
        if iwm_now <= 0 or spy_now <= 0 or iwm_lb <= 0 or spy_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = iwm_now / spy_now
        ratio_lb = iwm_lb / spy_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on IWM/SPY ratio momentum
        if ratio_mom > 0:
            # Small caps leading, risk appetite broadening → risk-on
            regime = "risk_on"
            day_ret = spy_rets[i] * spy_risk_on_w
        else:
            # Large cap flight to quality → defensive
            regime = "defensive"
            day_ret = (
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "size_ratio_momentum",
