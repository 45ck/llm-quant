#!/usr/bin/env python3
"""Robustness analysis for slv-gdx-mean-reversion-v1 (F2 Precious Metals Mean Reversion, Track B).

Hypothesis: SLV/GDX (silver vs gold miners) ratio exhibits mean-reversion.
Silver and gold miners share precious metals exposure but diverge on
industrial demand (SLV) vs mining leverage/operational costs (GDX). The
SLV/GDX ratio should mean-revert around its rolling average.

Signal:
  - Compute SLV/GDX price ratio at close t-1
  - Compute bb_window-day SMA and rolling std of ratio
  - Z-score = (ratio - SMA) / std
  - Z < -1.0 (SLV cheap vs GDX): 50% SLV + 20% GDX
  - Z > +1.0 (SLV expensive vs GDX): 50% GDX + 20% SLV
  - Neutral: 35% SLV + 35% GDX

This is a ZERO equity exposure strategy -- near-zero correlation with
equity-timing strategies. Track B gates apply (Sharpe > 1.0, MaxDD < 30%).

Perturbations:
  1. bb_window=40 (faster mean-reversion lookback)
  2. bb_window=90 (slower mean-reversion lookback)
  3. bb_std=1.5 (wider entry bands, fewer signals)
  4. bb_std=2.5 (very wide bands, only extreme signals)
  5. overweight=0.40 (less aggressive tilt)
  6. overweight=0.60 (more aggressive tilt)

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_slv_gdx_mean_reversion_robustness.py
"""
...
SLUG = "slv-gdx-mean-reversion-v1"
SYMBOLS = ["SLV", "GDX"]
...
# Build price series -- use SLV dates as the reference timeline
slv_df = prices.filter(pl.col("symbol") == "SLV").sort("date")
dates = slv_df["date"].to_list()
n = len(dates)
...
def run_single(params: dict) -> dict:
    ...
    Signal logic (all using close t-1 to avoid look-ahead):
      - Compute SLV/GDX price ratio
      - Compute bb_window-day SMA and rolling std of ratio
      - Z-score = (ratio - SMA) / std
      - Z < -1.0: regime='long_slv' (SLV cheap) -> overweight SLV + underweight GDX
      - Z > +1.0: regime='long_gdx' (SLV expensive) -> overweight GDX + underweight SLV
    ...
    # SLV/GDX ratio -- when SLV rises relative to GDX, ratio increases
    ratio_by_idx: dict[int, float] = {}
    for i in range(n):
        d = dates[i]
        slv_p = sym_data["SLV"].get(d, 0.0)
        gdx_p = sym_data["GDX"].get(d, 0.0)
        if slv_p > 0 and gdx_p > 0:
            ratio_by_idx[i] = slv_p / gdx_p
    ...
    for i in range(WARMUP, n):
        ...
        # z < -1.0: SLV/GDX ratio is below mean => SLV cheap vs GDX
        if z < -bb_std:
            regime = "long_slv"
            slv_w = overweight
            gdx_w = underweight
        # z > +1.0: SLV/GDX ratio is above mean => SLV expensive vs GDX
        elif z > bb_std:
            regime = "long_gdx"
            slv_w = underweight
            gdx_w = overweight
        ...
        day_ret = asset_ret("SLV", i) * slv_w + asset_ret("GDX", i) * gdx_w
...
# ==============================================================================
# Benchmark: equal-weight 50/50 SLV/GDX portfolio (the naive no-signal alternative)
# The shuffled test checks whether our z-score timing beats random regime assignment
print("\n--- SHUFFLED SIGNAL TEST ---")
eq_weight_rets: list[float] = []
for i in range(WARMUP, n):
    eq_weight_rets.append(0.5 * asset_ret("SLV", i) + 0.5 * asset_ret("GDX", i))
...
print(f"  Benchmark (50/50 SLV/GDX) returns: {len(aligned_bench)} days")
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "slv_gdx_mean_reversion",
    "mechanism_family": "F2",
    "track": "B",
    "hypothesis": (
        "SLV/GDX ratio exhibits mean-reversion. Silver and gold miners share "
        "precious metals exposure but diverge on industrial demand (SLV) vs "
        "mining leverage/operational costs (GDX). When SLV is cheap relative "
        "to GDX (z < -1), overweight SLV. When SLV is expensive relative to "
        "GDX (z > +1), overweight GDX. Always holds both assets -- pairs "
        "trade with zero equity exposure."
    ),
