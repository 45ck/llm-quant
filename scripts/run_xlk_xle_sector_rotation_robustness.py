#!/usr/bin/env python3
"""Robustness analysis for xlk-xle-sector-rotation-v1 (H15.2).

Mechanism: XLK/XLE ratio momentum + SMA regime -> QQQ/GLD+DBA allocation.
- Growth regime (momentum > 0 AND ratio > SMA): 90% QQQ
- Value/inflation regime (momentum < 0 AND ratio < SMA): 50% GLD + 30% DBA
- Neutral: 45% SPY

Base parameters: lookback=40, sma_period=20, growth_weight=0.90,
  inflation_gold_weight=0.50, inflation_commodity_weight=0.30, neutral_weight=0.45.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_xlk_xle_sector_rotation_robustness.py
"""
...
BASE_PARAMS = {
    ...
    "sma_period": 20,
    "growth_weight": 0.90,
    "inflation_gold_weight": 0.50,
    "inflation_commodity_weight": 0.30,
    "neutral_weight": 0.45,
}
...
qqq_df = prices.filter(pl.col("symbol") == "QQQ").sort("date")
qqq_data = dict(zip(qqq_df["date"].to_list(), qqq_df["close"].to_list(), strict=False))
...
dba_data = dict(zip(dba_df["date"].to_list(), dba_df["close"].to_list(), strict=False))

xlk_df = prices.filter(pl.col("symbol") == "XLK").sort("date")
xlk_data = dict(zip(xlk_df["date"].to_list(), xlk_df["close"].to_list(), strict=False))

xle_df = prices.filter(pl.col("symbol") == "XLE").sort("date")
xle_data = dict(zip(xle_df["date"].to_list(), xle_df["close"].to_list(), strict=False))
...
def run_single(params):
    ...
    growth_weight = float(params.get("growth_weight", 0.90))
    inflation_gold_weight = float(params.get("inflation_gold_weight", 0.50))
    inflation_commodity_weight = float(params.get("inflation_commodity_weight", 0.30))
    ...
    for i in range(WARMUP, n):
        ...
        if len(sma_window) < sma_period:
            daily_returns.append(0.0)
            continue
        ratio_sma = sum(sma_window) / sma_period
        if ratio_sma <= 0:
            daily_returns.append(0.0)
            continue

        # Determine regime and allocate
        regime = _classify_regime(ratio_mom, ratio_now, ratio_sma)
        day_ret = _compute_day_return(
            regime,
            i,
            growth_weight,
            inflation_gold_weight,
            inflation_commodity_weight,
            neutral_weight,
        )
...
def _classify_regime(ratio_mom, ratio_now, ratio_sma):
...
def _compute_day_return(regime, i, gw, igw, icw, nw):
    """Compute portfolio return for one day given the regime."""
    if regime == "growth":
        return _asset_ret(qqq_data, i) * gw
    if regime == "inflation":
        gld_r = _asset_ret(gld_data, i) * igw
        dba_r = _asset_ret(dba_data, i) * icw
        return gld_r + dba_r
    return spy_rets[i] * nw
...
perturbations = [
    ...
    ("growth_weight=0.70", {**BASE_PARAMS, "growth_weight": 0.70}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "xlk_xle_sector_rotation",
