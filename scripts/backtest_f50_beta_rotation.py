#!/usr/bin/env python3
"""Backtest F50: High-Beta / Low-Beta Rotation.

Self-contained backtest script. Does NOT use the strategy registry.

Hypothesis: The relative performance of high-beta stocks (SPHB) vs
low-volatility stocks (SPLV) captures risk appetite at a fundamental level.
When SPHB outperforms SPLV, the market rewards risk-taking -- stay in equities.
When SPLV outperforms SPHB, the market rotates to safety -- go defensive.

Signal: SPHB/SPLV ratio, 20-day momentum, 40-day SMA.
Regime classification:
  - RISK_SEEKING (mom > 0 AND ratio > SMA): 70% SPY + 15% QQQ + 15% SHY
  - RISK_AVERSE (mom < 0 AND ratio < SMA): 30% GLD + 30% SHY + 20% TLT + 20% SPY
  - NEUTRAL (one condition): 50% SPY + 30% SHY + 10% GLD + 10% TLT
Rebalance every 5 trading days. Cost: 5 bps round-trip.

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_f50_beta_rotation.py
"""
...
SLUG = "high-low-beta-v1"
SIGNAL_SYMBOLS: list[str] = ["SPHB", "SPLV"]
...
class BacktestParams:
    """Strategy parameters for the high-beta/low-beta rotation backtest."""

    mom_lookback: int = 20
    sma_period: int = 40
    rebalance_frequency_days: int = 5
    # Risk-seeking regime (SPHB outperforming = risk appetite strong)
    spy_risk_seeking: float = 0.70
    qqq_risk_seeking: float = 0.15
    gld_risk_seeking: float = 0.00
    shy_risk_seeking: float = 0.15
    tlt_risk_seeking: float = 0.00
    # Risk-averse regime (SPLV outperforming = flight to safety)
    spy_risk_averse: float = 0.20
    qqq_risk_averse: float = 0.00
    gld_risk_averse: float = 0.30
    shy_risk_averse: float = 0.30
    tlt_risk_averse: float = 0.20
    ...
    def get_regime_weights(self, regime: str) -> dict[str, float]:
        ...
        if regime == "RISK_SEEKING":
            return {
                "SPY": self.spy_risk_seeking,
                "QQQ": self.qqq_risk_seeking,
                "GLD": self.gld_risk_seeking,
                "SHY": self.shy_risk_seeking,
                "TLT": self.tlt_risk_seeking,
            }
        if regime == "RISK_AVERSE":
            return {
                "SPY": self.spy_risk_averse,
                "QQQ": self.qqq_risk_averse,
                "GLD": self.gld_risk_averse,
                "SHY": self.shy_risk_averse,
                "TLT": self.tlt_risk_averse,
            }
        # NEUTRAL
        return {
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "spy_risk_seeking": self.spy_risk_seeking,
            "qqq_risk_seeking": self.qqq_risk_seeking,
            "shy_risk_seeking": self.shy_risk_seeking,
            "spy_risk_averse": self.spy_risk_averse,
            "gld_risk_averse": self.gld_risk_averse,
            "shy_risk_averse": self.shy_risk_averse,
            "tlt_risk_averse": self.tlt_risk_averse,
...
def run_backtest(
    ...
    """Run the high-beta/low-beta rotation backtest.
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    all_needed = [*TRADEABLE_SYMBOLS, "SPHB", "SPLV"]
    ...
    # ── Precompute SPHB/SPLV ratio, momentum, and SMA for all dates ────
    sphb_closes = [close_by_symbol["SPHB"][d] for d in common_dates]
    splv_closes = [close_by_symbol["SPLV"][d] for d in common_dates]

    # Compute ratio
    ratio: list[float] = []
    for i in range(len(common_dates)):
        if splv_closes[i] > 0:
            ratio.append(sphb_closes[i] / splv_closes[i])
        else:
            ratio.append(0.0)
    ...
    for i, date_str in enumerate(common_dates):
        ...
        # Use i-1 for signal to ensure strict causality
        signal_idx = i - 1
        if signal_idx < 0:
            continue

        mom = momentum[signal_idx]
        sma = ratio_sma[signal_idx]
        if mom is None or sma is None:
            continue  # Skip if signals not available

        current_ratio = ratio[signal_idx]
...
def main() -> None:
    """Run the F50 backtest and save results."""
    result = run_backtest()
    ...
    entry = {
        ...
        "slug": SLUG,
        "strategy_type": "high_low_beta_rotation",
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  F50 High-Beta/Low-Beta Rotation Backtest: {SLUG}")
    print("=" * 60)
