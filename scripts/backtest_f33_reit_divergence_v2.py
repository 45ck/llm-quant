#!/usr/bin/env python3
"""Backtest F33: REIT Divergence Financial Conditions v2.

Self-contained backtest that:
- Fetches OHLCV for XLRE, SPY, QQQ, GLD, SHY, TLT
- Computes XLRE/SPY ratio, 60-day z-score, 15-day momentum
- Classifies regime: easing / tightening / neutral
- Allocates across assets based on regime (v2 allocations)
...
- Saves results to data/strategies/reit-divergence-v2/

Changes from v1:
- mom_window=15 (pre-specified from v1 perturbation analysis)
- Easing: 80% SPY + 10% QQQ + 10% GLD (was 80% SPY)
- Tightening: 40% GLD + 30% SHY + 30% SPY (was 60% GLD + 30% SHY)
- Neutral: 50% SPY + 40% SHY + 10% GLD (was 50% SPY + 40% SHY)
- family_trial_number=2 for DSR computation
"""
...
# Constants
# ---------------------------------------------------------------------------
SLUG = "reit-divergence-v2"
SIGNAL_SYMBOLS = ["XLRE", "SPY"]
TRADE_SYMBOLS = ["SPY", "QQQ", "GLD", "SHY"]
BENCHMARK_SYMBOLS = ["SPY", "TLT"]
ALL_SYMBOLS = list(set(SIGNAL_SYMBOLS + TRADE_SYMBOLS + BENCHMARK_SYMBOLS))

# Parameters (frozen from research-spec v2)
ZSCORE_WINDOW = 60  # 60-day z-score of XLRE/SPY ratio
MOM_WINDOW = 15  # 15-day momentum -- PRE-SPECIFIED from v1 perturbation
ZSCORE_THRESH = 0.5  # z-score threshold for regime classification

# Easing regime: growth-tilted equity + gold inflation hedge
SPY_EASING = 0.80
QQQ_EASING = 0.10
GLD_EASING = 0.10

# Tightening regime: defensive with reduced equity
GLD_TIGHTENING = 0.40
SHY_TIGHTENING = 0.30
SPY_TIGHTENING = 0.30

# Neutral regime: balanced with gold hedge
SPY_NEUTRAL = 0.50
SHY_NEUTRAL = 0.40
GLD_NEUTRAL = 0.10
...
FAMILY_TRIAL_NUMBER = 2  # second trial in F33 family
...
def _get_target_weights(
    ...
    if regime == "easing":
        return {"SPY": spy_easing, "QQQ": qqq_easing, "GLD": gld_easing}
    ...
    return {"SPY": spy_neutral, "SHY": shy_neutral, "GLD": gld_neutral}
...
def run_backtest(
    ...
    qqq_easing: float = QQQ_EASING,
    gld_easing: float = GLD_EASING,
    gld_tightening: float = GLD_TIGHTENING,
    shy_tightening: float = SHY_TIGHTENING,
    spy_tightening: float = SPY_TIGHTENING,
    spy_neutral: float = SPY_NEUTRAL,
    shy_neutral: float = SHY_NEUTRAL,
    gld_neutral: float = GLD_NEUTRAL,
    rebalance_freq: int = REBALANCE_FREQ,
    cost_bps: float = COST_BPS,
) -> dict:
    """Run the REIT divergence v2 backtest.
    ...
    logger.info("=== F33 REIT Divergence v2 Backtest Results ===")
    logger.info(
...
def main() -> None:
    ...
    print("\n" + "=" * 60)
    print("F33 REIT DIVERGENCE v2 -- GATE SUMMARY")
    print("=" * 60)
    gates = {
    ...
    print(f"Trial count: {FAMILY_TRIAL_NUMBER} (DSR penalized)")
    print("=" * 60)
