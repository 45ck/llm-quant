#!/usr/bin/env python3
"""Robustness analysis for emb-agg-em-credit-spread-v1 (F21 EM Credit).

Mechanism: EMB/AGG price ratio 30-day momentum captures EM credit spread direction.
  - ratio_mom > 0 (EMB outperforming AGG, EM credit tightening): 80% SPY
  - ratio_mom <= 0 (AGG outperforming EMB, EM credit widening): 50% GLD + 20% SHY

This is DIFFERENT from EMB->SPY credit lead-lag:
  - EMB-SPY uses EMB absolute returns as signal -> SPY follower
  - This uses EMB/AGG RELATIVE performance as signal (credit spread proxy)

Base parameters: lookback=30, spy_risk_on_weight=0.80,
                 gld_defensive_weight=0.50, shy_defensive_weight=0.20.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_emb_agg_em_credit_spread_robustness.py
"""
...
SLUG = "emb-agg-em-credit-spread-v1"
SYMBOLS = ["EMB", "AGG", "SPY", "GLD", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
def run_single(params):
    ...
    gld_def_w = float(params.get("gld_defensive_weight", 0.50))
    shy_def_w = float(params.get("shy_defensive_weight", 0.20))
    ...
    for i in range(WARMUP, n):
        ...
        # EMB/AGG ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        emb_now = sym_data["EMB"].get(d_now, 0.0)
        agg_now = sym_data["AGG"].get(d_now, 0.0)
        emb_lb = sym_data["EMB"].get(d_lb, 0.0)
        agg_lb = sym_data["AGG"].get(d_lb, 0.0)

        if emb_now <= 0 or agg_now <= 0 or emb_lb <= 0 or agg_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = emb_now / agg_now
        ratio_lb = emb_lb / agg_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on EMB/AGG ratio momentum
        if ratio_mom > 0:
            # EMB outperforming AGG: EM credit tightening, global risk-on
            regime = "risk_on"
            day_ret = spy_rets[i] * spy_risk_on_w
        else:
            # AGG outperforming EMB: EM credit widening, defensive
            regime = "defensive"
            day_ret = asset_ret("GLD", i) * gld_def_w + asset_ret("SHY", i) * shy_def_w
...
perturbations = [
    ...
    ("gld_defensive_weight=0.40", {**BASE_PARAMS, "gld_defensive_weight": 0.40}),
    ("gld_defensive_weight=0.60", {**BASE_PARAMS, "gld_defensive_weight": 0.60}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "emb_agg_em_credit_spread",
