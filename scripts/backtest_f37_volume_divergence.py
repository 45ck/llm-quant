#!/usr/bin/env python3
"""Backtest F37: Volume-Price Divergence Regime.

Self-contained backtest script. Does NOT use the strategy registry.

Signal: Compute price momentum (10-day) and volume momentum (10/40 day ratio)
for SPY. The divergence = price_momentum - volume_momentum. Z-score the
divergence over a 60-day window. Three regimes:
  - hollow_rally: zscore > 1.5 (price up, volume weak) -> 30% SPY + 30% GLD + 30% SHY
  - exhausted_selling: zscore < -1.5 (price down, volume weak) -> 80% SPY + 10% GLD
  - neutral: everything else -> 55% SPY + 20% GLD + 15% SHY
Rebalance every 5 trading days. 5 bps round-trip cost.

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_f37_volume_divergence.py
"""
...
SLUG = "volume-price-divergence-v1"
SIGNAL_SYMBOL = "SPY"  # SPY provides both price and volume for the signal
TRADEABLE_SYMBOLS: list[str] = ["SPY", "GLD", "SHY"]
BENCHMARK_SYMBOLS: dict[str, float] = {"SPY": 0.60, "TLT": 0.40}
ALL_SYMBOLS: list[str] = [*TRADEABLE_SYMBOLS, "TLT"]  # TLT for benchmark only
...
class BacktestParams:
    """Strategy parameters for the volume-price divergence backtest."""
    ...
    spy_hollow: float = 0.30
    gld_hollow: float = 0.30
    shy_hollow: float = 0.30
    spy_exhausted: float = 0.80
    gld_exhausted: float = 0.10
    spy_neutral: float = 0.55
    gld_neutral: float = 0.20
    ...
    def get_regime_weights(self, regime: str) -> dict[str, float]:
        ...
        if regime == "hollow_rally":
            return {
                "SPY": self.spy_hollow,
                "GLD": self.gld_hollow,
                "SHY": self.shy_hollow,
            }
        if regime == "exhausted_selling":
            # Remaining goes to SHY (cash equivalent)
            shy_remainder = round(1.0 - self.spy_exhausted - self.gld_exhausted, 4)
            return {
                "SPY": self.spy_exhausted,
                "GLD": self.gld_exhausted,
                "SHY": max(0.0, shy_remainder),
            }
        # neutral
        shy_remainder = round(
            1.0 - self.spy_neutral - self.gld_neutral - self.shy_neutral, 4
        )
        return {
            "SPY": self.spy_neutral,
            "GLD": self.gld_neutral,
            "SHY": self.shy_neutral + max(0.0, shy_remainder),
        }
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "spy_hollow": self.spy_hollow,
            "gld_hollow": self.gld_hollow,
            "shy_hollow": self.shy_hollow,
            "spy_exhausted": self.spy_exhausted,
            "gld_exhausted": self.gld_exhausted,
...
def run_backtest(
    ...
    **kwargs: object,
    ...
    """Run the volume-price divergence regime backtest.
    ...
    **kwargs : object
        Parameter overrides applied to BacktestParams.

    Returns
    -------
    dict
        Backtest results including sharpe_ratio, max_drawdown, dsr, cagr, etc.
    """
    if params is None:
        params = BacktestParams(**{k: v for k, v in kwargs.items() if v is not None})
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    # Build per-symbol close price series AND volume series, aligned by date
    close_by_symbol: dict[str, dict[str, float]] = {}
    volume_by_symbol: dict[str, dict[str, float]] = {}
    ...
    for sym in ALL_SYMBOLS:
        ...
        # Volume data (needed for SPY signal)
        if sym == SIGNAL_SYMBOL and "volume" in sym_data.columns:
            volumes = sym_data["volume"].to_list()
            volume_by_symbol[sym] = {
                str(d): float(v) for d, v in zip(dates, volumes, strict=True)
            }
    ...
    if SIGNAL_SYMBOL not in volume_by_symbol:
        logger.error("No volume data for signal symbol %s", SIGNAL_SYMBOL)
        return {"error": f"No volume data for {SIGNAL_SYMBOL}"}
    ...
    # Also filter by volume availability
    common_dates = [d for d in common_dates if d in volume_by_symbol[SIGNAL_SYMBOL]]
    logger.info(
    ...
    # Warmup needs zscore_window + vol_long + price_lookback + buffer
    warmup = params.zscore_window + params.vol_long + params.price_lookback + 10
    ...
    spy_closes = [close_by_symbol[SIGNAL_SYMBOL][d] for d in common_dates]
    spy_volumes = [volume_by_symbol[SIGNAL_SYMBOL][d] for d in common_dates]

    # Price momentum: (close / close.shift(price_lookback)) - 1
    price_mom: list[float | None] = [None] * len(common_dates)
    for i in range(params.price_lookback, len(common_dates)):
        past_price = spy_closes[i - params.price_lookback]
        if past_price > 0:
            price_mom[i] = (spy_closes[i] - past_price) / past_price

    # Volume momentum: (volume.rolling(vol_short).mean() / volume.rolling(vol_long).mean()) - 1
    vol_mom: list[float | None] = [None] * len(common_dates)
    for i in range(params.vol_long - 1, len(common_dates)):
        # Short-term volume average
        if i >= params.vol_short - 1:
            vol_short_window = spy_volumes[i - params.vol_short + 1 : i + 1]
            vol_short_avg = sum(vol_short_window) / len(vol_short_window)
        else:
            continue

        # Long-term volume average
        vol_long_window = spy_volumes[i - params.vol_long + 1 : i + 1]
        vol_long_avg = sum(vol_long_window) / len(vol_long_window)

        if vol_long_avg > 0:
            vol_mom[i] = (vol_short_avg / vol_long_avg) - 1.0

    # Divergence: price_momentum - volume_momentum
    divergence: list[float | None] = [None] * len(common_dates)
    for i in range(len(common_dates)):
        if price_mom[i] is not None and vol_mom[i] is not None:
            divergence[i] = price_mom[i] - vol_mom[i]

    # Divergence z-score over zscore_window
    div_zscore: list[float | None] = [None] * len(common_dates)
    for i in range(len(common_dates)):
        # Collect the last zscore_window divergence values ending at i
        window_vals: list[float] = []
        for j in range(max(0, i - params.zscore_window + 1), i + 1):
            if divergence[j] is not None:
                window_vals.append(divergence[j])
        if len(window_vals) >= params.zscore_window:
            mean_val = sum(window_vals) / len(window_vals)
            std_val = (
                sum((x - mean_val) ** 2 for x in window_vals) / len(window_vals)
            ) ** 0.5
            if std_val > 1e-8 and divergence[i] is not None:
                div_zscore[i] = (divergence[i] - mean_val) / std_val
    ...
    final_nav = daily_navs[-1] if daily_navs else 0.0
...
def main() -> None:
    """Run the F37 backtest and save results."""
    result = run_backtest()
    ...
    entry = {
        ...
        "slug": SLUG,
        "strategy_type": "volume_price_divergence",
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  F37 Volume-Price Divergence: {SLUG}")
    print("=" * 60)
