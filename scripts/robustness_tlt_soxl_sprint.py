...
Run: cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_tlt_soxl_sprint.py
"""
...
# Fetch VIX separately (uses ^VIX ticker)
vix_prices = fetch_ohlcv(["^VIX"], lookback_days=LOOKBACK_DAYS)
...
# Build VIX lookup (^VIX stored as "VIX" by the fetcher)
vix_sym_name = "VIX"
...
if len(vix_sdf) == 0:
    ...
    vix_sdf = vix_prices.filter(pl.col("symbol") == vix_sym_name).sort("date")
vix_data: dict = dict(
    zip(vix_sdf["date"].to_list(), vix_sdf["close"].to_list(), strict=False)
)
