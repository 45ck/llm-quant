#!/usr/bin/env python3
"""Robustness analysis for crypto-weekend-equity-v1 (BTC weekend -> QQQ Monday).

Trial 1 defaults: weekend_threshold=-0.02, target_weight=0.95, hold_days=2,
signal_symbol=BTC-USD, trade_symbol=QQQ.

Perturbations (5 variants, each must be within +/-25% of base Sharpe):
  1. weekend_threshold=0.0   (no filter — enter every Monday)
  2. weekend_threshold=-0.04 (stricter — skip more Mondays)
  3. hold_days=3             (hold Mon-Wed instead of Mon-Tue)
  4. target_weight=0.70      (reduced position weight)
  5. weekend_threshold=-0.01 (slight relaxation from default)

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_crypto_weekend_robustness.py
"""
...
SLUG = "crypto-weekend-equity-v1"
STRATEGY = "crypto_weekend_equity"
SYMBOLS = ["BTC-USD", "QQQ"]
DD_THRESHOLD = 0.15
...
def run_single(params, warmup=300):
    ...
    config = StrategyConfig(
        name=STRATEGY,
        rebalance_frequency_days=1,  # daily check (entries only on Mondays internally)
        max_positions=1,
...
# ==============================================================================
perturbations = [
    ("weekend_threshold=0.0", {**BASE_PARAMS, "weekend_threshold": 0.0}),
    ("weekend_threshold=-0.04", {**BASE_PARAMS, "weekend_threshold": -0.04}),
    ("hold_days=3", {**BASE_PARAMS, "hold_days": 3}),
    ("target_weight=0.70", {**BASE_PARAMS, "target_weight": 0.70}),
    ("weekend_threshold=-0.01", {**BASE_PARAMS, "weekend_threshold": -0.01}),
]
