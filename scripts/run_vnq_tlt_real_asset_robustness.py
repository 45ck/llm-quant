#!/usr/bin/env python3
"""Robustness analysis for vnq-tlt-real-asset-v1.

Mechanism: VNQ/TLT price ratio 30-day momentum captures real asset vs nominal bond preference.
  - ratio_mom > 0 (VNQ outperforming TLT, real asset demand): 80% SPY
  - ratio_mom < 0 (TLT outperforming VNQ, flight to safety): 50% GLD + 20% SHY

Different from all existing bond-based signals:
  - F6 uses TLT/IEF absolute returns -> QQQ (rate channel)
  - F14 uses TLT/SHY ratio momentum (curve shape)
  - This uses VNQ/TLT (real estate vs long bonds - asset class rotation)

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_vnq_tlt_real_asset_robustness.py
"""
...
SLUG = "vnq-tlt-real-asset-v1"
SYMBOLS = ["VNQ", "TLT", "SPY", "GLD", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
def run_single(params):
    ...
    for i in range(WARMUP, n):
        ...
        # VNQ/TLT ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        vnq_now = sym_data["VNQ"].get(d_now, 0.0)
        tlt_now = sym_data["TLT"].get(d_now, 0.0)
        vnq_lb = sym_data["VNQ"].get(d_lb, 0.0)
        tlt_lb = sym_data["TLT"].get(d_lb, 0.0)

        if vnq_now <= 0 or tlt_now <= 0 or vnq_lb <= 0 or tlt_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = vnq_now / tlt_now
        ratio_lb = vnq_lb / tlt_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on VNQ/TLT ratio momentum
        if ratio_mom > 0:
            # VNQ outperforming TLT: real asset demand, inflationary/risk-on
            regime = "risk_on"
            day_ret = spy_rets[i] * spy_risk_on_w
        else:
            # TLT outperforming VNQ: flight to safety, deflationary
            regime = "defensive"
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "vnq_tlt_real_asset",
