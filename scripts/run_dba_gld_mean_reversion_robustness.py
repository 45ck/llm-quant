#!/usr/bin/env python3
"""Robustness analysis for dba-gld-mean-reversion-v1 (F2-Agriculture Mean Reversion).

Hypothesis: DBA/GLD ratio (agriculture vs gold) exhibits mean-reversion driven by
inflation expectations. Both are inflation hedges but driven by different supply/demand
dynamics. When DBA outperforms GLD (food inflation hot), mean-revert to GLD. When GLD
outperforms DBA (monetary fear premium), mean-revert to DBA. Pairs trade with zero
equity exposure.

Signal:
  - Compute DBA/GLD price ratio at close t-1
  - Compute bb_window-day SMA and rolling std of ratio
  - Z-score = (ratio - SMA) / std
  - Z < -bb_std (DBA cheap): 50% DBA + 20% GLD
  - Z > +bb_std (DBA expensive): 50% GLD + 20% DBA
  - Neutral: 35% DBA + 35% GLD
...
Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_dba_gld_mean_reversion_robustness.py
"""
...
SLUG = "dba-gld-mean-reversion-v1"
SYMBOLS = ["DBA", "GLD"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 90
...
# Build price series -- use DBA dates as the reference timeline
dba_df = prices.filter(pl.col("symbol") == "DBA").sort("date")
dates = dba_df["date"].to_list()
n = len(dates)
...
def run_single(params: dict) -> dict:
    ...
    Signal logic (all using close t-1 to avoid look-ahead):
      - Compute DBA/GLD price ratio
      - Compute bb_window-day SMA and rolling std of ratio
      - Z-score = (ratio - SMA) / std
      - Z < -bb_std: regime='long_dba' (DBA cheap) -> overweight DBA + underweight GLD
      - Z > +bb_std: regime='long_gld' (GLD cheap) -> overweight GLD + underweight DBA
    ...
    for i in range(n):
        d = dates[i]
        dba_p = sym_data["DBA"].get(d, 0.0)
        gld_p = sym_data["GLD"].get(d, 0.0)
        if dba_p > 0 and gld_p > 0:
            ratio_by_idx[i] = dba_p / gld_p
    ...
    for i in range(WARMUP, n):
        ...
        if z < -bb_std:
            # DBA cheap relative to GLD -> overweight DBA, underweight GLD
            regime = "long_dba"
            dba_w = overweight
            gld_w = underweight
        elif z > bb_std:
            # GLD cheap relative to DBA -> overweight GLD, underweight DBA
            regime = "long_gld"
            dba_w = underweight
            gld_w = overweight
        else:
        ...
        day_ret = asset_ret("DBA", i) * dba_w + asset_ret("GLD", i) * gld_w
...
# ==============================================================================
# Benchmark: equal-weight 50/50 DBA/GLD portfolio (the naive no-signal alternative)
# The shuffled test checks whether our z-score timing beats random regime assignment
print("\n--- SHUFFLED SIGNAL TEST ---")
eq_weight_rets: list[float] = []
for i in range(WARMUP, n):
    eq_weight_rets.append(0.5 * asset_ret("DBA", i) + 0.5 * asset_ret("GLD", i))
...
print(f"  Benchmark (50/50 DBA/GLD) returns: {len(aligned_bench)} days")
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "dba_gld_mean_reversion",
    "mechanism_family": "F2",
    "hypothesis": (
        "DBA/GLD ratio exhibits mean-reversion driven by inflation expectations. "
        "When DBA is cheap relative to GLD (z < -1), overweight DBA (food inflation "
        "underpriced). When GLD is cheap (z > +1), overweight GLD (monetary fear "
        "premium underpriced). Always holds both assets -- pairs trade, no equity exposure."
    ),
