#!/usr/bin/env python3
"""Robustness analysis for dxy-commodity-v2 (A3, DXY-Commodity Rotation).
...
Mechanism: UUP (dollar bull ETF) 30-day momentum.
  ...
  - UUP momentum < 0 (dollar weakening): 40% GLD + 20% DBA + 30% SPY
Base parameters: lookback=30, spy_weight_strong=0.80,
                 gld_weight_weak=0.40, dba_weight_weak=0.20, spy_weight_weak=0.30.

Changes from v1:
  - lookback: 20 -> 30 (perturbation test showed lookback=30 gave Sharpe=0.981)
  - dba_weight_weak: 0.30 -> 0.20 (reduce pure commodity exposure for MaxDD control)
  - spy_weight_weak: 0.20 -> 0.30 (increase defensive allocation in weak-dollar regime)

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_dxy_commodity_v2_robustness.py
"""
...
BASE_PARAMS = {
    ...
    "gld_weight_weak": 0.40,
    "dba_weight_weak": 0.20,
    "spy_weight_weak": 0.30,
}
...
def run_single(params):
    ...
    gld_w_weak = float(params.get("gld_weight_weak", 0.40))
    dba_w_weak = float(params.get("dba_weight_weak", 0.20))
    spy_w_weak = float(params.get("spy_weight_weak", 0.30))
...
perturbations = [
    ...
    ("gld_weight_weak=0.30", {**BASE_PARAMS, "gld_weight_weak": 0.30}),
    ("gld_weight_weak=0.50", {**BASE_PARAMS, "gld_weight_weak": 0.50}),
]
