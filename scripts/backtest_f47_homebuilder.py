#!/usr/bin/env python3
"""Backtest F47: Homebuilder Lead Indicator.

Self-contained backtest script. Does NOT use the strategy registry.

Signal: Compute XHB/SPY ratio, 20-day momentum, and 40-day SMA.
Three regimes based on momentum direction + ratio vs SMA:
  - HOUSING_EXPANSION (mom > 0 AND ratio > SMA): 70% SPY + 15% QQQ + 15% SHY
  - HOUSING_CONTRACTION (mom < 0 AND ratio < SMA): 20% SPY + 30% GLD + 30% SHY + 20% TLT
  - NEUTRAL: 50% SPY + 30% SHY + 10% GLD + 10% TLT
Rebalance every 5 trading days.

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_f47_homebuilder.py
"""
...
SLUG = "homebuilder-lead-v1"
SIGNAL_SYMBOLS: list[str] = ["XHB"]
...
def run_backtest(
    ...
    """Run the homebuilder lead indicator backtest.
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    # ── Precompute XHB/SPY ratio, momentum, and SMA for all dates ────────
    xhb_closes = [close_by_symbol["XHB"][d] for d in common_dates]
    ...
    for i in range(len(common_dates)):
        if spy_closes[i] > 0:
            ratio.append(xhb_closes[i] / spy_closes[i])
        else:
            ratio.append(0.0)
    ...
    for i, date_str in enumerate(common_dates):
        ...
        if mom > 0 and current_ratio > sma:
            regime = "HOUSING_EXPANSION"
        elif mom < 0 and current_ratio < sma:
            regime = "HOUSING_CONTRACTION"
        else:
            regime = "NEUTRAL"
...
def main() -> None:
    """Run the F47 backtest and save results."""
    result = run_backtest()
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  F47 Homebuilder Lead Indicator Backtest: {SLUG}")
    print("=" * 60)
