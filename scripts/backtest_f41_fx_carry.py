#!/usr/bin/env python3
"""Backtest F41: FX Carry Risk Appetite Signal.

Self-contained backtest script. Does NOT use the strategy registry.

Signal: FXA/FXY price ratio 10-day momentum captures carry trade
positioning velocity. AUD is high-yield commodity currency; JPY is
safe-haven funding currency. Ratio momentum captures global risk appetite.

Three regimes:
  - risk_on: ratio_mom > 0.02 (AUD strengthening) -> 80% SPY + 10% EFA
  - risk_off: ratio_mom < -0.02 (JPY strengthening) -> 50% GLD + 30% SHY
  - neutral: everything else -> 50% SPY + 20% GLD + 20% SHY

Rebalance every 5 trading days. 5 bps round-trip cost.
Benchmark: 60/40 SPY/TLT.

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_f41_fx_carry.py
"""
...
SLUG = "fx-carry-risk-appetite-v1"
SIGNAL_SYMBOLS: list[str] = ["FXA", "FXY"]
TRADEABLE_SYMBOLS: list[str] = ["SPY", "EFA", "GLD", "SHY"]
...
class BacktestParams:
    """Strategy parameters for the FX carry risk appetite backtest."""

    lookback: int = 10
    entry_threshold: float = 0.02
    spy_risk_on: float = 0.80
    efa_risk_on: float = 0.10
    gld_risk_off: float = 0.50
    shy_risk_off: float = 0.30
    spy_neutral: float = 0.50
    gld_neutral: float = 0.20
    shy_neutral: float = 0.20
    rebalance_freq: int = 5
    cost_bps: float = ROUND_TRIP_COST_BPS

    def get_regime_weights(self, regime: str) -> dict[str, float]:
        """Return target weights for a given regime. Always sums to 1.0.

        SHY absorbs any remainder to ensure full capital deployment.
        """
        if regime == "risk_on":
            spy_w = self.spy_risk_on
            efa_w = self.efa_risk_on
            shy_w = round(1.0 - spy_w - efa_w, 4)
            return {"SPY": spy_w, "EFA": efa_w, "GLD": 0.0, "SHY": max(shy_w, 0.0)}
        if regime == "risk_off":
            gld_w = self.gld_risk_off
            shy_w = round(1.0 - gld_w, 4)
            return {"SPY": 0.0, "EFA": 0.0, "GLD": gld_w, "SHY": max(shy_w, 0.0)}
        # neutral
        spy_w = self.spy_neutral
        gld_w = self.gld_neutral
        shy_w = round(1.0 - spy_w - gld_w, 4)
        return {"SPY": spy_w, "EFA": 0.0, "GLD": gld_w, "SHY": max(shy_w, 0.0)}
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "spy_risk_on": self.spy_risk_on,
            "efa_risk_on": self.efa_risk_on,
            "gld_risk_off": self.gld_risk_off,
            "shy_risk_off": self.shy_risk_off,
...
def run_backtest(
    ...
    **kwargs: object,
    ...
    """Run the FX carry risk appetite backtest.
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    # Verify we have all needed symbols (except TLT which is benchmark only)
    needed = [*SIGNAL_SYMBOLS, *TRADEABLE_SYMBOLS]
    ...
    # Build aligned date index (intersection of signal + tradeable symbols)
    ...
    # Warmup needs lookback + buffer to have full signal
    warmup = params.lookback + 5
    ...
    fxa_closes = [close_by_symbol["FXA"][d] for d in common_dates]
    fxy_closes = [close_by_symbol["FXY"][d] for d in common_dates]

    # Ratio series: FXA / FXY
    ratio = [
        fxa / fxy if fxy > 0 else 0.0
        for fxa, fxy in zip(fxa_closes, fxy_closes, strict=True)
    ]

    # Ratio momentum (lookback-day percentage change)
    ratio_mom: list[float | None] = [None] * len(common_dates)
    for i in range(params.lookback, len(common_dates)):
        past = ratio[i - params.lookback]
        if past > 0:
...
def main() -> None:
    """Run the F41 backtest and save results."""
    result = run_backtest()
    ...
    # Save backtest results (without daily_returns/benchmark_returns for YAML)
    ...
    entry = {
        ...
        "slug": SLUG,
        "stage": "backtest",
        "strategy_type": "fx_carry_risk_appetite",
        "family": "fx_carry_risk_appetite",
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  F41 FX Carry Risk Appetite: {SLUG}")
    print("=" * 60)
    ...
    print(f"\n  Backtest gates: {'ALL PASS' if all_passed else 'FAILED'}")
    if result["sharpe_ratio"] >= 0.70:
        print("  Sharpe >= 0.70: eligible for robustness testing")
    else:
        print("  Sharpe < 0.70: NOT eligible for robustness testing")
    print()
