#!/usr/bin/env python3
"""Robustness analysis for A7 cross-asset-rotation-sharpe."""
...
SLUG = "cross-asset-rotation-sharpe"
STRATEGY = "asset_rotation"
SYMBOLS = ["TLT", "GLD", "SPY", "BTC-USD"]
BASE_PARAMS = {
    "symbols_list": "TLT,GLD,SPY,BTC-USD",
    "lookback_days": 60,
    "top_k": 1,
    "rank_by": "sharpe",
    "target_weight": 0.90,
...
def run_single(params):
    config = StrategyConfig(
        name=STRATEGY,
        rebalance_frequency_days=params.get("rebalance_frequency_days", 20),
...
perturbations = [
    ("lookback_days=30", {**BASE_PARAMS, "lookback_days": 30}),
    ("lookback_days=90", {**BASE_PARAMS, "lookback_days": 90}),
    ("top_k=2", {**BASE_PARAMS, "top_k": 2}),
    ("rank_by=return", {**BASE_PARAMS, "rank_by": "return"}),
