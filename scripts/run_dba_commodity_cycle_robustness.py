#!/usr/bin/env python3
"""Robustness analysis for dba-commodity-cycle-v1 (H14.5, Family F11).

Mechanism: DBA absolute momentum → SPY/GLD inflation-tilt allocation.
Base parameters: lookback=60, inflation: 50% GLD + 30% SPY, disinflation: 90% SPY.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_dba_commodity_cycle_robustness.py
"""
...
BASE_PARAMS = {
    "lookback": 60,
    "inflation_equity_weight": 0.30,
    "inflation_gold_weight": 0.50,
    "disinflation_equity_weight": 0.90,
}
...
def run_single(params):
    ...
    inf_eq_w = float(params.get("inflation_equity_weight", 0.30))
    inf_gld_w = float(params.get("inflation_gold_weight", 0.50))
    disinf_eq_w = float(params.get("disinflation_equity_weight", 0.90))
    ...
    for i in range(WARMUP, n):
        ...
        # DBA momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        ...
        # Determine regime based on DBA momentum
        if dba_mom > 0:
            # Inflation regime: commodity prices rising
            regime = "inflation"
            day_ret = spy_rets[i] * inf_eq_w + asset_ret("GLD", i) * inf_gld_w
        else:
            # Disinflation regime: commodity prices falling
            regime = "disinflation"
            day_ret = spy_rets[i] * disinf_eq_w
...
perturbations = [
    ...
    ("inflation_gold_weight=0.40", {**BASE_PARAMS, "inflation_gold_weight": 0.40}),
    ("inflation_gold_weight=0.60", {**BASE_PARAMS, "inflation_gold_weight": 0.60}),
    (
        "disinflation_equity_weight=0.70",
        {**BASE_PARAMS, "disinflation_equity_weight": 0.70},
    ),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "dba_commodity_cycle",
