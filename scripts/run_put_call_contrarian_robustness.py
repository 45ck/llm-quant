#!/usr/bin/env python3
"""Robustness analysis for vix-fear-gauge-contrarian-v1 (F23 candidate).

Mechanism: VIX level as fear gauge. High VIX (>25) = extreme fear -> buy SPY.
Low VIX (<15) = complacency -> defensive SHY. Moderate = half-weight SPY.

NOTE: ^PCALL data unavailable via yfinance. Using VIX LEVEL thresholds instead.
This is DIFFERENT from existing VIX strategies (F9 uses HYG/SHY ratio, F13 uses
VIX/VIX3M term structure ratio) -- this uses raw VIX price levels as a contrarian
fear gauge.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_put_call_contrarian_robustness.py
"""
...
SLUG = "vix-fear-gauge-contrarian-v1"
SYMBOLS = ["SPY", "SHY", "VIX"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
BASE_PARAMS = {
    "sma_window": 5,
    "threshold_high": 25.0,  # VIX above this = fear -> buy SPY
    "threshold_low": 15.0,  # VIX below this = complacency -> defensive
    "fear_weight": 0.90,  # SPY weight during fear
    "complacency_weight": 0.90,  # SHY weight during complacency
    "neutral_weight": 0.45,  # SPY weight during normal
}
...
def run_single(params):
    ...
    sma_window = int(params.get("sma_window", 5))
    threshold_high = float(params.get("threshold_high", 25.0))
    threshold_low = float(params.get("threshold_low", 15.0))
    fear_weight = float(params.get("fear_weight", 0.90))
    complacency_weight = float(params.get("complacency_weight", 0.90))
    neutral_weight = float(params.get("neutral_weight", 0.45))

    daily_returns = []
    cost_per_switch = 0.0003  # 3 bps

    # Build VIX close series aligned to dates
    vix_closes = []
    for i in range(n):
        d = dates[i]
        vix_closes.append(sym_data["VIX"].get(d, 0.0))
    ...
    for i in range(WARMUP, n):
        ...
        # VIX SMA at close of day i-1 (lag-1, no look-ahead)
        vix_window = vix_closes[i - sma_window : i]
        valid_vix = [v for v in vix_window if v > 0]
        if len(valid_vix) < sma_window:
            daily_returns.append(0.0)
            continue

        vix_sma = sum(valid_vix) / len(valid_vix)

        # Determine regime based on VIX level
        shy_r = asset_ret("SHY", i)

        if vix_sma > threshold_high:
            # High fear -> contrarian buy SPY
            regime = "fear"
            day_ret = spy_rets[i] * fear_weight
        elif vix_sma < threshold_low:
            # Complacency -> defensive SHY
            regime = "complacency"
            day_ret = shy_r * complacency_weight
        else:
            # Moderate -> half SPY
            regime = "neutral"
            day_ret = spy_rets[i] * neutral_weight
...
# ==============================================================================
perturbations = [
    ("threshold_high=22", {**BASE_PARAMS, "threshold_high": 22.0}),
    ("threshold_high=28", {**BASE_PARAMS, "threshold_high": 28.0}),
    ("threshold_low=12", {**BASE_PARAMS, "threshold_low": 12.0}),
    ("threshold_low=18", {**BASE_PARAMS, "threshold_low": 18.0}),
    ("sma_window=3", {**BASE_PARAMS, "sma_window": 3}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "vix_fear_gauge_contrarian",
    "mechanism_family": "F23",
