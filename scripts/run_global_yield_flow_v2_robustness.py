#!/usr/bin/env python3
"""Robustness analysis for global-yield-flow-v2 (N8, Global Bond Yield Convergence).

Mechanism: TLT/EFA price ratio 30-day momentum.
  - Ratio momentum > 0 (TLT outperforming EFA = US bonds attractive,
    capital flowing to US): 80% SPY
  - Ratio momentum < 0 (EFA outperforming TLT = international preferred):
    40% EFA + 20% GLD  [v2: efa_weight reduced from 0.50 to 0.40]
Base parameters: lookback=30, spy_weight=0.80,
                 efa_weight=0.40, gld_weight=0.20.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_global_yield_flow_v2_robustness.py
"""
...
def run_single(params):
    ...
    spy_w = float(params.get("spy_weight", 0.80))
    efa_w = float(params.get("efa_weight", 0.40))
    ...
    for i in range(WARMUP, n):
        ...
        # TLT/EFA ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        ...
        if tlt_now <= 0 or efa_now <= 0 or tlt_lb <= 0 or efa_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = tlt_now / efa_now
        ratio_lb = tlt_lb / efa_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on TLT/EFA ratio momentum
        if ratio_mom > 0:
            # TLT outperforming EFA = US bonds attractive, capital to US equities
            regime = "us_inflow"
            day_ret = spy_rets[i] * spy_w
        else:
            # EFA outperforming TLT = international preferred
            regime = "intl_preferred"
            day_ret = asset_ret("EFA", i) * efa_w + asset_ret("GLD", i) * gld_w
...
perturbations = [
    ...
    ("spy_weight=0.70", {**BASE_PARAMS, "spy_weight": 0.70}),
    ("spy_weight=0.90", {**BASE_PARAMS, "spy_weight": 0.90}),
    ("efa_weight=0.30", {**BASE_PARAMS, "efa_weight": 0.30}),
    ("efa_weight=0.50", {**BASE_PARAMS, "efa_weight": 0.50}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "global_yield_flow",
