#!/usr/bin/env python3
"""Robustness analysis for efa-eem-dm-em-flow-v1 (F20 DM-vs-EM Capital Flow).

Mechanism: EFA/EEM price ratio 30-day momentum captures DM-vs-EM capital flow.
  - ratio_mom > 0 (EFA outperforming EEM, DM preferred): 80% SPY
  - ratio_mom <= 0 (EEM outperforming EFA, EM strong): 50% EEM + 20% GLD
Base parameters: lookback=30, spy_dm_weight=0.80,
                 eem_em_weight=0.50, gld_em_weight=0.20.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_efa_eem_dm_em_flow_robustness.py
"""
...
SLUG = "efa-eem-dm-em-flow-v1"
SYMBOLS = ["EFA", "EEM", "SPY", "GLD"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
def run_single(params):
    ...
    spy_dm_w = float(params.get("spy_dm_weight", 0.80))
    eem_em_w = float(params.get("eem_em_weight", 0.50))
    gld_em_w = float(params.get("gld_em_weight", 0.20))
    ...
    for i in range(WARMUP, n):
        ...
        # EFA/EEM ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        efa_now = sym_data["EFA"].get(d_now, 0.0)
        eem_now = sym_data["EEM"].get(d_now, 0.0)
        efa_lb = sym_data["EFA"].get(d_lb, 0.0)
        eem_lb = sym_data["EEM"].get(d_lb, 0.0)

        if efa_now <= 0 or eem_now <= 0 or efa_lb <= 0 or eem_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = efa_now / eem_now
        ratio_lb = efa_lb / eem_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on EFA/EEM ratio momentum
        if ratio_mom > 0:
            # EFA outperforming EEM: DM preferred, risk-off/US flight
            regime = "dm_preferred"
            day_ret = spy_rets[i] * spy_dm_w
        else:
            # EEM outperforming EFA: EM strong, global risk-on
            regime = "em_strong"
            day_ret = asset_ret("EEM", i) * eem_em_w + asset_ret("GLD", i) * gld_em_w
...
perturbations = [
    ...
    ("spy_dm_weight=0.70", {**BASE_PARAMS, "spy_dm_weight": 0.70}),
    ("spy_dm_weight=0.90", {**BASE_PARAMS, "spy_dm_weight": 0.90}),
    ("eem_em_weight=0.40", {**BASE_PARAMS, "eem_em_weight": 0.40}),
    ("eem_em_weight=0.60", {**BASE_PARAMS, "eem_em_weight": 0.60}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "efa_eem_dm_em_flow",
