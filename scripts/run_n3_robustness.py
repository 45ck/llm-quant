#!/usr/bin/env python3
"""Robustness analysis for N3 tlt-yield-momentum."""
...
SLUG = "tlt-yield-momentum"
STRATEGY = "yield_curve_regime"
SYMBOLS = ["TLT", "^TNX", "SPY"]
DD_THRESHOLD = 0.12
...
print(f"DD Threshold: {DD_THRESHOLD:.2f}")

perturbations = [
    ("momentum_window=42", {**BASE_PARAMS, "momentum_window": 42}),
    ("momentum_window=90", {**BASE_PARAMS, "momentum_window": 90}),
    ("momentum_window=126", {**BASE_PARAMS, "momentum_window": 126}),
...
print(f"\nStable perturbations: {n_stable}/{len(perturbations)}")
...
print(f"DD gate (< {DD_THRESHOLD:.0%}): {dd_result} (max_dd={base['max_dd']:.4f})")
