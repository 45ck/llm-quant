#!/usr/bin/env python3
"""Robustness analysis for F39: Equity-Bond Correlation Regime Change.
...
4. Perturbation tests across all key parameters (>= 10 variants)
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f39_equity_bond_corr.py
"""
...
from backtest_f39_equity_bond_corr import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
FAMILY = "equity_bond_corr_regime"
...
def run_shuffled_signal_test(
    ...
    we randomly assign one of {hedge_breakdown, hedge_restoring, neutral}. If
    the real signal is genuine, the shuffled Sharpe distribution should be lower.
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    regimes = ["hedge_breakdown", "hedge_restoring", "neutral"]

    for i, date_str in enumerate(common_dates):
        if i < warmup:
            if i == 0:
                # Hold neutral allocation during warmup
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    # corr_window: 20, 45 (vs base 30)
    ("corr_window=20", {"corr_window": 20}),
    ("corr_window=45", {"corr_window": 45}),
    # mom_lookback: 5, 15 (vs base 10)
    ("mom_lookback=5", {"mom_lookback": 5}),
    ("mom_lookback=15", {"mom_lookback": 15}),
    # entry_threshold: 1.0, 2.0 (vs base 1.5)
    ("entry_threshold=1.0", {"entry_threshold": 1.0}),
    ("entry_threshold=2.0", {"entry_threshold": 2.0}),
    # spy_hedge_break: 0.10, 0.30 (vs base 0.20)
    ("spy_hedge_break=0.10", {"spy_hedge_break": 0.10}),
    ("spy_hedge_break=0.30", {"spy_hedge_break": 0.30}),
    # gld_hedge_break: 0.30, 0.50 (vs base 0.40)
    ("gld_hedge_break=0.30", {"gld_hedge_break": 0.30}),
    ("gld_hedge_break=0.50", {"gld_hedge_break": 0.50}),
    # spy_hedge_restore: 0.70, 0.90 (vs base 0.80)
    ("spy_hedge_restore=0.70", {"spy_hedge_restore": 0.70}),
    ("spy_hedge_restore=0.90", {"spy_hedge_restore": 0.90}),
]
...
def main() -> None:
    """Run full robustness analysis for F39."""
    import yaml
    ...
    logger.info("=" * 60)
    logger.info("Step 3: CPCV (15 groups, 3 test, purge=5)")
    logger.info("=" * 60)
    ...
    # -- Final gate summary --
    print("\n" + "=" * 60)
    print("  F39 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
    ...
    entry = {
        ...
        "perturbation_stability_pct": perturbation_result["stability_pct"],
