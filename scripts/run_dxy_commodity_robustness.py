#!/usr/bin/env python3
"""Robustness analysis for dxy-commodity-v1 (A3, DXY-Commodity Rotation).

Hypothesis: DXY strength predicts commodity underperformance over 20 days.
Mechanism: UUP (dollar bull ETF) 20-day momentum.
  - UUP momentum > 0 (dollar strengthening): 80% SPY (avoid commodities)
  - UUP momentum < 0 (dollar weakening): 40% GLD + 30% DBA + 20% SPY
Base parameters: lookback=20, spy_weight_strong=0.80,
                 gld_weight_weak=0.40, dba_weight_weak=0.30, spy_weight_weak=0.20.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_dxy_commodity_robustness.py
"""
...
BASE_PARAMS = {
    ...
    "dba_weight_weak": 0.30,
    "spy_weight_weak": 0.20,
}
...
def run_single(params):
    ...
    dba_w_weak = float(params.get("dba_weight_weak", 0.30))
    spy_w_weak = float(params.get("spy_weight_weak", 0.20))
