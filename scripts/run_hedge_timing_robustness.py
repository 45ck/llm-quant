#!/usr/bin/env python3
"""Robustness analysis for hedge-timing-v1 (F20 Hedge Timing).

Mechanism: Use credit spread momentum (HYG/LQD ratio) to time equity hedging
via SH (inverse S&P 500 ETF). When credit quality deteriorates (LQD outperforming
HYG), add SH as portfolio hedge. When credit is healthy (HYG outperforming LQD),
remove hedge and go full SPY.

Key insight: During risk-off periods, this strategy SHORTS equity via SH, which
produces NEGATIVE correlation with existing long-equity strategies -- extremely
valuable for portfolio diversification.

Signal:
  - Compute HYG/LQD price ratio at close t-1
  - Compute 20-day ratio momentum (ratio_now / ratio_20d_ago - 1)
  - ratio_mom > 0 (credit healthy, HY outperforming): 80% SPY
  - ratio_mom < 0 (credit stress, quality flight): 40% SH + 20% GLD + 10% SHY

Base parameters: lookback=20, spy_risk_on_weight=0.80,
                 sh_hedge_weight=0.40, gld_weight=0.20, shy_weight=0.10.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_hedge_timing_robustness.py
"""
...
SLUG = "hedge-timing-v1"
SYMBOLS = ["HYG", "LQD", "SPY", "SH", "GLD", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
def run_single(params):
    ...
    sh_hedge_w = float(params.get("sh_hedge_weight", 0.40))
    ...
    for i in range(WARMUP, n):
        ...
        # HYG/LQD ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        hyg_now = sym_data["HYG"].get(d_now, 0.0)
        lqd_now = sym_data["LQD"].get(d_now, 0.0)
        hyg_lb = sym_data["HYG"].get(d_lb, 0.0)
        lqd_lb = sym_data["LQD"].get(d_lb, 0.0)

        if hyg_now <= 0 or lqd_now <= 0 or hyg_lb <= 0 or lqd_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = hyg_now / lqd_now
        ratio_lb = hyg_lb / lqd_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on HYG/LQD ratio momentum
        if ratio_mom > 0:
            # Credit healthy: HY outperforming IG -> hold equity
            regime = "risk_on"
            day_ret = spy_rets[i] * spy_risk_on_w
        else:
            # Credit stress: quality flight -> HEDGE with SH (inverse SPY)
            # Net SHORT equity via SH -- key diversification value
            regime = "hedged"
            day_ret = (
                asset_ret("SH", i) * sh_hedge_w
...
perturbations = [
    ...
    ("sh_hedge_weight=0.30", {**BASE_PARAMS, "sh_hedge_weight": 0.30}),
    ("sh_hedge_weight=0.50", {**BASE_PARAMS, "sh_hedge_weight": 0.50}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "hedge_timing",
    "mechanism_family": "F20_hedge_timing",
