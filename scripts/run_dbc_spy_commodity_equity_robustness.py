#!/usr/bin/env python3
"""Robustness analysis for dbc-spy-commodity-equity-v1 (F21 Commodity Cycle).

Mechanism: DBC/SPY price ratio 30-day momentum captures the commodity-equity
rotation cycle. When commodities (DBC) outperform equities (SPY), the commodity
cycle is dominant -- overweight commodities. When SPY outperforms DBC, equity
cycle is dominant -- overweight equities.

This is DIFFERENT from existing commodity strategies:
  - F11 uses DBA absolute momentum (single commodity signal)
  - F18 uses USO/DBC ratio (within-commodity rotation)
  - F16 uses TIP/IEF ratio (inflation proxy)
  - This uses DBC/SPY (cross-asset class rotation)

Signal:
  - Compute DBC/SPY price ratio at close t-1
  - Compute lookback-day ratio momentum (ratio_now / ratio_lb - 1)
  - ratio_mom > 0 (DBC outperforming SPY, commodity cycle): 40% GLD + 30% DBA + 10% SPY
  - ratio_mom <= 0 (SPY outperforming DBC, equity cycle): 80% SPY

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_dbc_spy_commodity_equity_robustness.py
"""
...
BASE_PARAMS = {
    "lookback": 30,
    "spy_equity_weight": 0.80,
    "gld_commodity_weight": 0.40,
    "dba_commodity_weight": 0.30,
    "spy_commodity_weight": 0.10,
}
...
def run_single(params):
    ...
    spy_equity_w = float(params.get("spy_equity_weight", 0.80))
    gld_commodity_w = float(params.get("gld_commodity_weight", 0.40))
    dba_commodity_w = float(params.get("dba_commodity_weight", 0.30))
    spy_commodity_w = float(params.get("spy_commodity_weight", 0.10))
    ...
    for i in range(WARMUP, n):
        ...
        # DBC/SPY ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        dbc_now = sym_data["DBC"].get(d_now, 0.0)
        spy_now = sym_data["SPY"].get(d_now, 0.0)
        dbc_lb = sym_data["DBC"].get(d_lb, 0.0)
        spy_lb = sym_data["SPY"].get(d_lb, 0.0)

        if dbc_now <= 0 or spy_now <= 0 or dbc_lb <= 0 or spy_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = dbc_now / spy_now
        ratio_lb = dbc_lb / spy_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on DBC/SPY ratio momentum
        if ratio_mom > 0:
            # Commodity cycle: DBC outperforming SPY
            # 40% GLD + 30% DBA + 10% SPY (commodity-tilted allocation)
            regime = "commodity"
            day_ret = (
                asset_ret("GLD", i) * gld_commodity_w
                + asset_ret("DBA", i) * dba_commodity_w
                + spy_rets[i] * spy_commodity_w
            )
        else:
            # Equity cycle: SPY outperforming DBC
            # 80% SPY
            regime = "equity"
            day_ret = spy_rets[i] * spy_equity_w
...
perturbations = [
    ...
    ("spy_equity_weight=0.70", {**BASE_PARAMS, "spy_equity_weight": 0.70}),
    ("spy_equity_weight=0.90", {**BASE_PARAMS, "spy_equity_weight": 0.90}),
    ("gld_commodity_weight=0.30", {**BASE_PARAMS, "gld_commodity_weight": 0.30}),
    ("gld_commodity_weight=0.50", {**BASE_PARAMS, "gld_commodity_weight": 0.50}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "commodity_equity_rotation",
