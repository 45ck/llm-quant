#!/usr/bin/env python3
"""Robustness analysis for F46 v2: Biotech Risk Appetite (improved).

v2 change from v1: rebalance_frequency_days = 10 (was 5).
DSR uses trial_number=2 (second trial of biotech_risk_appetite family).
...
Gates (all 6):
  - Sharpe >= 0.80
  - MaxDD < 15%
  - DSR >= 0.95 (with trial_number=2)
  - CPCV OOS > 0
  - Perturbation stability >= 60%
  - Shuffled signal p < 0.05

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f46_biotech_risk_v2.py
"""
...
from backtest_f46_biotech_risk_v2 import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    Uses rebalance_frequency_days=10 (v2 default).
    """
    params = BacktestParams()  # v2 default: rebalance_frequency_days=10
...
# v2 base rebalance is 10, so perturbation range is +/-25% around 10 => [7, 13]
# Also perturb all other params same as v1
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    ...
    # Rebalance frequency (+/-25% around 10 => 7 and 13)
    ("rebalance=7", {"rebalance_frequency_days": 7}),
    ("rebalance=13", {"rebalance_frequency_days": 13}),
]
...
def run_perturbation_tests(
    ...
    for name, overrides in PERTURBATION_CONFIGS:
        # Start from v2 defaults (rebalance=10)
...
def main() -> None:
    """Run full robustness analysis for F46 v2."""
    import yaml
    ...
    logger.info("=" * 60)
    logger.info("Step 1: Base backtest (rebalance=10, trial_number=2)")
    logger.info("=" * 60)
    ...
    print(
        ...
        f"CAGR={base_cagr:.4f}, DSR={base_dsr:.4f} (trial={TRIAL_NUMBER}), "
    ...
    # ── Final gate summary ───────────────────────────────────────────────
    print("\n" + "=" * 60)
    print(f"  F46 v2 ROBUSTNESS GATE SUMMARY (trial_number={TRIAL_NUMBER})")
    print("=" * 60)
    ...
    gates = {
        ...
        f"DSR >= 0.95 (trial={TRIAL_NUMBER})": (base_dsr >= 0.95, f"{base_dsr:.4f}"),
    ...
    robustness_yaml: dict = {
        ...
        "v1_comparison": {
            ...
            "v1_max_dd": 0.1997,
            "v1_dsr": 0.9634,
            "v2_sharpe": base_sharpe,
            "v2_max_dd": base_max_dd,
            "v2_dsr": base_dsr,
            "sharpe_improvement": round(base_sharpe - 0.8254, 4),
            "maxdd_improvement": round(0.1997 - base_max_dd, 4),
            "rebalance_change": "5 -> 10 days",
        },
    }
