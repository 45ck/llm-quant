#!/usr/bin/env python3
"""Robustness analysis for gdx-gld-mean-reversion-v1 (F2-Miners Mean Reversion).

Hypothesis: GDX/GLD ratio (gold miners vs gold) exhibits mean-reversion.
Gold miners are levered plays on gold. When GDX outperforms GLD (miners
expensive relative to gold), mean-revert by buying GLD. When GLD outperforms
GDX (miners cheap relative to gold), mean-revert by buying GDX. This is a
pairs trade similar to GLD/SLV (F2) and should have near-zero equity correlation.

Signal:
  - Compute GDX/GLD price ratio at close t-1
  - Compute bb_window-day SMA and rolling std of ratio
  - Z-score = (ratio - SMA) / std
  - Z < -bb_std (miners cheap): overweight GDX + underweight GLD
  - Z > +bb_std (miners expensive): overweight GLD + underweight GDX
...
Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_gdx_gld_mean_reversion_robustness.py
"""
...
# Build price series -- use GDX dates as the reference timeline
gdx_df = prices.filter(pl.col("symbol") == "GDX").sort("date")
dates = gdx_df["date"].to_list()
n = len(dates)
...
def run_single(params: dict) -> dict:
    ...
    Signal logic (all using close t-1 to avoid look-ahead):
      - Compute GDX/GLD price ratio
      - Compute bb_window-day SMA and rolling std of ratio
      - Z-score = (ratio - SMA) / std
      - Z < -bb_std: regime='long_gdx' (miners cheap) -> overweight GDX + underweight GLD
      - Z > +bb_std: regime='long_gld' (miners expensive) -> overweight GLD + underweight GDX
    ...
    # We need ratio at each date where both GDX and GLD have data
    ratio_by_idx: dict[int, float] = {}
    for i in range(n):
        d = dates[i]
        gdx_p = sym_data["GDX"].get(d, 0.0)
        gld_p = sym_data["GLD"].get(d, 0.0)
        if gdx_p > 0 and gld_p > 0:
            ratio_by_idx[i] = gdx_p / gld_p
    ...
    for i in range(WARMUP, n):
        ...
        if z < -bb_std:
            # GDX cheap relative to GLD -> overweight GDX, underweight GLD
            regime = "long_gdx"
            gdx_w = overweight
            gld_w = underweight
        elif z > bb_std:
            # GLD cheap relative to GDX (miners expensive) -> overweight GLD, underweight GDX
            regime = "long_gld"
            gdx_w = underweight
            gld_w = overweight
        else:
        ...
        day_ret = asset_ret("GDX", i) * gdx_w + asset_ret("GLD", i) * gld_w
...
# ==============================================================================
# Benchmark: equal-weight 50/50 GDX/GLD portfolio (the naive no-signal alternative)
# The shuffled test checks whether our z-score timing beats random regime assignment
print("\n--- SHUFFLED SIGNAL TEST ---")
# Compute equal-weight GDX+GLD returns as the benchmark asset
eq_weight_rets: list[float] = []
for i in range(WARMUP, n):
    eq_weight_rets.append(0.5 * asset_ret("GDX", i) + 0.5 * asset_ret("GLD", i))
...
print(f"  Benchmark (50/50 GDX/GLD) returns: {len(aligned_bench)} days")
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "gdx_gld_mean_reversion",
    "mechanism_family": "F2",
    "hypothesis": (
        "GDX/GLD ratio exhibits mean-reversion. When GDX is cheap relative "
        "to GLD (z < -1), overweight GDX (miners cheap). When GLD is cheap "
        "relative to GDX (z > +1), overweight GLD (miners expensive). "
        "Always holds both assets -- pairs trade, no SPY."
    ),
