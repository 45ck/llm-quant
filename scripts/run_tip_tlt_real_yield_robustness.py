#!/usr/bin/env python3
"""Robustness analysis for tip-tlt-real-yield-v1 (H16.5, Real Yield Proxy).

Mechanism: TIP/TLT price ratio 20-day momentum.
  - Ratio momentum < 0 (real yields falling = loosening): 75% SPY
  - Ratio momentum > 0 (real yields rising = tightening): 50% GLD + 20% SHY
Base parameters: lookback=20, loosening_spy_weight=0.75,
                 tightening_gld_weight=0.50, tightening_shy_weight=0.20.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_tip_tlt_real_yield_robustness.py
"""
...
BASE_PARAMS = {
    "lookback": 20,
    "loosening_spy_weight": 0.75,
    "tightening_gld_weight": 0.50,
    "tightening_shy_weight": 0.20,
}
...
def run_single(params):
    ...
    loosening_spy_w = float(params.get("loosening_spy_weight", 0.75))
    tightening_gld_w = float(params.get("tightening_gld_weight", 0.50))
    tightening_shy_w = float(params.get("tightening_shy_weight", 0.20))
    ...
    for i in range(WARMUP, n):
        ...
        # TIP/TLT ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        ...
        # Determine regime based on TIP/TLT ratio momentum
        if ratio_mom < 0:
            # Real yields falling = loosening: risk-on
            regime = "loosening"
            day_ret = spy_rets[i] * loosening_spy_w
        else:
            # Real yields rising = tightening: defensive
            regime = "tightening"
            day_ret = (
                asset_ret("GLD", i) * tightening_gld_w
                + asset_ret("SHY", i) * tightening_shy_w
            )
...
perturbations = [
    ...
    ("loosening_spy_weight=0.65", {**BASE_PARAMS, "loosening_spy_weight": 0.65}),
    ("tightening_gld_weight=0.40", {**BASE_PARAMS, "tightening_gld_weight": 0.40}),
    ("tightening_shy_weight=0.30", {**BASE_PARAMS, "tightening_shy_weight": 0.30}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "tip_tlt_real_yield",
