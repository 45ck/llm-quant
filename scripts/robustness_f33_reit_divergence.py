#!/usr/bin/env python3
"""Robustness analysis for F33: REIT Divergence Financial Conditions v1.

Runs:
1. Base backtest
2. Shuffled signal test (1000 permutations)
3. CPCV (5 folds, OOS/IS ratio)
...
from backtest_f33_reit_divergence import SLUG, run_backtest
...
def run_perturbation_suite(base_sharpe: float) -> list[dict]:
    ...
    perturbations: list[tuple[str, dict]] = [
        ...
        ("gld_tightening=0.70", {"gld_tightening": 0.70}),
    ]
...
def _run_cpcv_analysis(
    ...
    """Run CPCV analysis with 5 folds. Returns (mean, std, oos_is, passed, sharpes)."""
    cpcv = run_cpcv(
...
def main() -> None:
    ...
    print("=" * 70)
    print("F33 REIT DIVERGENCE -- ROBUSTNESS ANALYSIS")
    print("=" * 70)
