#!/usr/bin/env python3
"""Backtest F45: Transport Confirmation (Dow Theory).

Self-contained backtest script. Does NOT use the strategy registry.

Signal: Compute IYT/SPY ratio, 20-day momentum, and 40-day SMA.
Three regimes based on momentum direction + ratio vs SMA:
  - EXPANSION_CONFIRMED (mom > 0 AND ratio > SMA): 70% SPY + 15% IWM + 15% SHY
  - CONTRACTION (mom < 0 AND ratio < SMA): 30% TLT + 30% GLD + 20% SHY + 20% SPY
  - NEUTRAL: 50% SPY + 30% SHY + 10% GLD + 10% TLT
Rebalance every 5 trading days.

Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_f45_transport.py
"""
...
SLUG = "transport-confirmation-v1"
SIGNAL_SYMBOLS: list[str] = ["IYT"]
TRADEABLE_SYMBOLS: list[str] = ["SPY", "IWM", "GLD", "SHY", "TLT"]
...
class BacktestParams:
    """Strategy parameters for the transport confirmation backtest."""

    mom_lookback: int = 20
    sma_period: int = 40
    rebalance_frequency_days: int = 5
    # Expansion confirmed regime: transports leading (risk-on)
    spy_expansion: float = 0.70
    iwm_expansion: float = 0.15
    shy_expansion: float = 0.15
    gld_expansion: float = 0.00
    tlt_expansion: float = 0.00
    # Contraction regime: transports lagging (defensive)
    spy_contraction: float = 0.20
    iwm_contraction: float = 0.00
    shy_contraction: float = 0.20
    gld_contraction: float = 0.30
    tlt_contraction: float = 0.30
    # Neutral regime: mixed signals
    spy_neutral: float = 0.50
    iwm_neutral: float = 0.00
    ...
    def get_regime_weights(self, regime: str) -> dict[str, float]:
        ...
        if regime == "EXPANSION_CONFIRMED":
            return {
                "SPY": self.spy_expansion,
                "IWM": self.iwm_expansion,
                "SHY": self.shy_expansion,
                "GLD": self.gld_expansion,
                "TLT": self.tlt_expansion,
            }
        if regime == "CONTRACTION":
            return {
                "SPY": self.spy_contraction,
                "IWM": self.iwm_contraction,
                "SHY": self.shy_contraction,
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "iwm_expansion": self.iwm_expansion,
            "shy_expansion": self.shy_expansion,
            "gld_expansion": self.gld_expansion,
            "tlt_expansion": self.tlt_expansion,
            "spy_contraction": self.spy_contraction,
            "iwm_contraction": self.iwm_contraction,
...
def run_backtest(
    ...
    """Run the transport confirmation backtest.
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    # ── Precompute IYT/SPY ratio, momentum, and SMA for all dates ────────
    iyt_closes = [close_by_symbol["IYT"][d] for d in common_dates]
    ...
    for i in range(len(common_dates)):
        if spy_closes[i] > 0:
            ratio.append(iyt_closes[i] / spy_closes[i])
        else:
            ratio.append(0.0)
...
def main() -> None:
    """Run the F45 backtest and save results."""
    result = run_backtest()
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  F45 Transport Confirmation Backtest: {SLUG}")
    print("=" * 60)
