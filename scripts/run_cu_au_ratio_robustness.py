#!/usr/bin/env python3
"""Robustness analysis for cu-au-ratio-v1 (Hypothesis A1: Cu/Au leads equity drawdowns).

Mechanism: COPX/GLD price ratio 20-day momentum.
  - Ratio momentum < 0 (copper weakening vs gold = risk-off): 50% SHY + 20% GLD
  - Ratio momentum > 0 (copper strengthening vs gold = risk-on): 80% SPY
Base parameters: lookback=20, spy_weight=0.80,
                 defensive_shy_weight=0.50, defensive_gld_weight=0.20.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_cu_au_ratio_robustness.py
"""
...
BASE_PARAMS = {
    "lookback": 20,
    "spy_weight": 0.80,
    "defensive_shy_weight": 0.50,
    "defensive_gld_weight": 0.20,
}
...
def run_single(params):
    ...
    def_shy_w = float(params.get("defensive_shy_weight", 0.50))
    def_gld_w = float(params.get("defensive_gld_weight", 0.20))
    ...
    for i in range(WARMUP, n):
        ...
        # COPX/GLD ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        copx_now = sym_data["COPX"].get(d_now, 0.0)
        gld_now = sym_data["GLD"].get(d_now, 0.0)
        copx_lb = sym_data["COPX"].get(d_lb, 0.0)
        gld_lb = sym_data["GLD"].get(d_lb, 0.0)

        if copx_now <= 0 or gld_now <= 0 or copx_lb <= 0 or gld_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = copx_now / gld_now
        ratio_lb = copx_lb / gld_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on COPX/GLD ratio momentum
        if ratio_mom < 0:
            # Copper weakening vs gold = risk-off: defensive
            regime = "risk_off"
            day_ret = asset_ret("SHY", i) * def_shy_w + asset_ret("GLD", i) * def_gld_w
        else:
            # Copper strengthening vs gold = risk-on
            regime = "risk_on"
...
perturbations = [
    ...
    ("defensive_gld_weight=0.30", {**BASE_PARAMS, "defensive_gld_weight": 0.30}),
]
