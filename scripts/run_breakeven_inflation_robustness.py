#!/usr/bin/env python3
"""Robustness analysis for breakeven-inflation-v1 (N5, Breakeven Inflation Proxy).

Mechanism: TIP/IEF price ratio 30-day momentum as breakeven inflation proxy.
  - Ratio momentum > 0 (inflation expectations rising): 40% DBA + 30% GLD + 10% SPY
  - Ratio momentum < 0 (inflation expectations falling): 70% SPY + 10% SHY
Base parameters: lookback=30, dba_weight=0.40, gld_weight=0.30, spy_inflation_weight=0.10,
                 spy_deflation_weight=0.70, shy_weight=0.10.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_breakeven_inflation_robustness.py
"""
...
SYMBOLS = ["TIP", "IEF", "DBA", "GLD", "SPY", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
def run_single(params):
    ...
    dba_w = float(params.get("dba_weight", 0.40))
    gld_w = float(params.get("gld_weight", 0.30))
    spy_inf_w = float(params.get("spy_inflation_weight", 0.10))
    spy_def_w = float(params.get("spy_deflation_weight", 0.70))
    ...
    for i in range(WARMUP, n):
        ...
        # TIP/IEF ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        tip_now = sym_data["TIP"].get(d_now, 0.0)
        ief_now = sym_data["IEF"].get(d_now, 0.0)
        tip_lb = sym_data["TIP"].get(d_lb, 0.0)
        ...
        # Determine regime based on TIP/IEF ratio momentum
        if ratio_mom > 0:
            # Inflation expectations rising: commodities + gold + small equity
            regime = "inflation_rising"
            day_ret = (
                asset_ret("DBA", i) * dba_w
                + asset_ret("GLD", i) * gld_w
                + spy_rets[i] * spy_inf_w
            )
        else:
            # Inflation expectations falling: equities + safety
            regime = "inflation_falling"
            day_ret = spy_rets[i] * spy_def_w + asset_ret("SHY", i) * shy_w
...
perturbations = [
    ...
    ("dba_weight=0.30", {**BASE_PARAMS, "dba_weight": 0.30}),
    ("dba_weight=0.50", {**BASE_PARAMS, "dba_weight": 0.50}),
    ("spy_deflation_weight=0.60", {**BASE_PARAMS, "spy_deflation_weight": 0.60}),
    ("spy_deflation_weight=0.80", {**BASE_PARAMS, "spy_deflation_weight": 0.80}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "breakeven_inflation",
