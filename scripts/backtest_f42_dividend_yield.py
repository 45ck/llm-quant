#!/usr/bin/env python3
"""Backtest F42: Dividend Yield Regime.

Self-contained backtest script. Does NOT use the strategy registry.

Signal: Compute SPYD/SPY ratio, 20-day momentum, and 40-day SMA.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/backtest_f42_dividend_yield.py
"""
...
SLUG = "dividend-yield-regime-v1"
SIGNAL_SYMBOLS: list[str] = ["SPYD"]
...
class BacktestParams:
    """Strategy parameters for the dividend yield regime backtest."""

    mom_lookback: int = 20
    sma_period: int = 40
    rebalance_frequency_days: int = 5
    # Income-preference regime (SPYD outperforming = defensive/late-cycle)
    spy_income: float = 0.20
    qqq_income: float = 0.00
    gld_income: float = 0.40
    shy_income: float = 0.20
    tlt_income: float = 0.20
    # Growth-preference regime (SPY outperforming = risk-on/early-cycle)
    spy_growth: float = 0.70
    qqq_growth: float = 0.15
    gld_growth: float = 0.00
    shy_growth: float = 0.15
    tlt_growth: float = 0.00
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "spy_income": self.spy_income,
            "gld_income": self.gld_income,
            "shy_income": self.shy_income,
            "tlt_income": self.tlt_income,
            "spy_growth": self.spy_growth,
            "qqq_growth": self.qqq_growth,
            "shy_growth": self.shy_growth,
...
def run_backtest(
    ...
    """Run the dividend yield regime backtest.
...
def _run_backtest_on_data(  # noqa: PLR0912
    ...
    # ── Precompute SPYD/SPY ratio, momentum, and SMA for all dates ────────
    spyd_closes = [close_by_symbol["SPYD"][d] for d in common_dates]
    ...
    for i in range(len(common_dates)):
        if spy_closes[i] > 0:
            ratio.append(spyd_closes[i] / spy_closes[i])
        else:
            ratio.append(0.0)
...
def main() -> None:
    """Run the F42 backtest and save results."""
    result = run_backtest()
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print(f"  F42 Dividend Yield Regime Backtest: {SLUG}")
    print("=" * 60)
