#!/usr/bin/env python3
"""Robustness analysis for global-yield-flow-v1 (N8, Global Bond Yield Convergence).
...
Base parameters: lookback=30, spy_weight=0.80,
                 efa_weight=0.50, gld_weight=0.20.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_global_yield_flow_robustness.py
"""
...
def run_single(params):
    ...
    efa_w = float(params.get("efa_weight", 0.50))
...
perturbations = [
    ...
    ("efa_weight=0.40", {**BASE_PARAMS, "efa_weight": 0.40}),
    ("efa_weight=0.60", {**BASE_PARAMS, "efa_weight": 0.60}),
]
