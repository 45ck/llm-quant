#!/usr/bin/env python3
"""Robustness analysis for commodity-carry-v1 (F9-H1, Commodity Carry/Backwardation).

Mechanism: USO/DBC price ratio 30-day momentum as crude oil carry/backwardation proxy.
  - Ratio momentum > 0 (backwardation / carry positive): 40% USO + 30% GLD + 10% SPY
  - Ratio momentum < 0 (contango / carry negative): 70% SPY + 10% SHY

Hypothesis: When crude oil is in backwardation (USO outperforms broad commodity basket
DBC), it signals commodity supply tightness and inflationary pressure. This is a
different signal from DBA momentum (F11) because it captures energy carry rather than
agricultural momentum.

Base parameters: lookback=30, uso_weight=0.40, gld_weight=0.30, spy_carry_weight=0.10,
                 spy_contango_weight=0.70, shy_weight=0.10.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_commodity_carry_robustness.py
"""
...
def run_single(params):
    ...
    uso_w = float(params.get("uso_weight", 0.40))
    gld_w = float(params.get("gld_weight", 0.30))
    spy_carry_w = float(params.get("spy_carry_weight", 0.10))
...
perturbations = [
    ...
    ("uso_weight=0.30", {**BASE_PARAMS, "uso_weight": 0.30}),
    ("uso_weight=0.50", {**BASE_PARAMS, "uso_weight": 0.50}),
    ("gld_weight=0.20", {**BASE_PARAMS, "gld_weight": 0.20}),
    ("gld_weight=0.40", {**BASE_PARAMS, "gld_weight": 0.40}),
]
