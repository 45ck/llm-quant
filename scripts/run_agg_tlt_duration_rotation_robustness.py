#!/usr/bin/env python3
"""Robustness analysis for agg-tlt-duration-rotation-v1 (F21 Duration).

Hypothesis: AGG/TLT Duration Rotation
When short-duration bonds (AGG) outperform long-duration bonds (TLT), the yield
curve is flattening or short rates are falling faster -> intermediate monetary
conditions -> equities moderate. When TLT outperforms AGG -> yield curve bull
steepening -> rate cuts -> equities benefit strongly.

Signal: AGG/TLT price ratio 30-day momentum.
  - ratio_mom > 0 (AGG outperforming TLT, curve flattening): 50% SPY + 20% GLD
  - ratio_mom <= 0 (TLT outperforming AGG, rate cuts/steepening): 80% SPY

DIFFERENT from:
  - F6 (TLT->SPY/QQQ): uses TLT absolute return, not AGG/TLT ratio
  - F14 (TLT/SHY): uses long vs very short duration
  - This uses AGG/TLT: aggregate vs long-only duration spread

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_agg_tlt_duration_rotation_robustness.py
"""
...
SLUG = "agg-tlt-duration-rotation-v1"
...
BASE_PARAMS = {
    "lookback": 30,
    "spy_steepening_weight": 0.80,
    "spy_flattening_weight": 0.50,
    "gld_flattening_weight": 0.20,
}
...
def run_single(params):
    ...
    spy_steep_w = float(params.get("spy_steepening_weight", 0.80))
    spy_flat_w = float(params.get("spy_flattening_weight", 0.50))
    gld_flat_w = float(params.get("gld_flattening_weight", 0.20))
    ...
    for i in range(WARMUP, n):
        ...
        # AGG/TLT ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        agg_now = sym_data["AGG"].get(d_now, 0.0)
        tlt_now = sym_data["TLT"].get(d_now, 0.0)
        agg_lb = sym_data["AGG"].get(d_lb, 0.0)
        tlt_lb = sym_data["TLT"].get(d_lb, 0.0)

        if agg_now <= 0 or tlt_now <= 0 or agg_lb <= 0 or tlt_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = agg_now / tlt_now
        ratio_lb = agg_lb / tlt_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on AGG/TLT ratio momentum
        if ratio_mom > 0:
            # AGG outperforming TLT -> curve flattening -> moderate equities + gold hedge
            regime = "flattening"
            day_ret = spy_rets[i] * spy_flat_w + asset_ret("GLD", i) * gld_flat_w
        else:
            # TLT outperforming AGG -> bull steepening / rate cuts -> strong equities
            regime = "steepening"
            day_ret = spy_rets[i] * spy_steep_w
...
perturbations = [
    ...
    ("spy_steepening_weight=0.70", {**BASE_PARAMS, "spy_steepening_weight": 0.70}),
    ("spy_steepening_weight=0.90", {**BASE_PARAMS, "spy_steepening_weight": 0.90}),
    ("spy_flattening_weight=0.40", {**BASE_PARAMS, "spy_flattening_weight": 0.40}),
    ("spy_flattening_weight=0.60", {**BASE_PARAMS, "spy_flattening_weight": 0.60}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "duration_rotation",
    "mechanism_family": "F21_duration",
    "hypothesis": (
        "AGG/TLT ratio momentum captures yield curve flattening/steepening regime. "
        "AGG outperforming TLT (flattening) -> moderate equity + gold hedge. "
        "TLT outperforming AGG (steepening/rate cuts) -> strong equity exposure."
    ),
