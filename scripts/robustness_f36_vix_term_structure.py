#!/usr/bin/env python3
"""Robustness analysis for F36: VIX Term Structure Momentum.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f36_vix_term_structure.py
"""
...
from backtest_f36_vix_term_structure import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def run_shuffled_signal_test(
    ...
    we randomly assign one of {hedging_rising, hedging_receding, normal}. If the
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    regimes = ["hedging_rising", "hedging_receding", "normal"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    # mom_window
    ("mom_window=7", {"mom_window": 7}),
    ("mom_window=15", {"mom_window": 15}),
    # zscore_window
    ("zscore_window=20", {"zscore_window": 20}),
    ("zscore_window=40", {"zscore_window": 40}),
    # spy_weight_riskon
    ("spy_weight_riskon=0.75", {"spy_weight_riskon": 0.75}),
    ("spy_weight_riskon=0.95", {"spy_weight_riskon": 0.95}),
    # gld_weight_stress
    ("gld_weight_stress=0.30", {"gld_weight_stress": 0.30}),
    ("gld_weight_stress=0.50", {"gld_weight_stress": 0.50}),
    # shy_weight_neutral
    ("shy_weight_neutral=0.25", {"shy_weight_neutral": 0.25}),
    ("shy_weight_neutral=0.45", {"shy_weight_neutral": 0.45}),
]
...
def main() -> None:
    """Run full robustness analysis for F36."""
    import yaml
    ...
    # -- Final gate summary --
    print("\n" + "=" * 60)
    print("  F36 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
    ...
    robustness_yaml: dict = {
        ...
        "family": "F36_vix_term_structure_momentum",
        "base_metrics": {
