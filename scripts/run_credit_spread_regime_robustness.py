#!/usr/bin/env python3
"""Robustness analysis for credit-spread-regime-v1 (H13.1, Family F9).

Mechanism: HYG/SHY ratio momentum + SMA regime → SPY/SHY allocation.
Base parameters: ratio_lookback=10, sma_period=20, target_weight=0.90.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_credit_spread_regime_robustness.py
"""
...
hyg_data = dict(zip(hyg_df["date"].to_list(), hyg_df["close"].to_list(), strict=False))
...
def shy_ret(i):
    ...
    if d in shy_data and dp in shy_data and shy_data[dp] > 0:
        return shy_data[d] / shy_data[dp] - 1
    return 0.0
...
def run_single(params):
    ...
    for i in range(WARMUP, n):
        ...
        if ratio_mom > 0 and ratio_vs_sma > 0:
            regime = "risk_on"
            day_ret = spy_rets[i] * target_weight
        elif ratio_mom < 0 and ratio_vs_sma < 0:
            regime = "risk_off"
            day_ret = shy_ret(i) * defensive_weight
        else:
            regime = "neutral"
...
# ==============================================================================
perturbations = [
    ("ratio_lookback=5", {**BASE_PARAMS, "ratio_lookback": 5}),
    ("ratio_lookback=20", {**BASE_PARAMS, "ratio_lookback": 20}),
...
# ==============================================================================
# DSR (compute inline since the engine backtest failed)
# ==============================================================================
print("\n--- DSR ---")
# For trial 1, DSR with family trial count = 1
sr = base["sharpe"]
T = len(base["daily_returns"])
# Simplified DSR: probability that Sharpe > 0 given T and trial count
# DSR = 1 - Phi(-SR * sqrt(T/252) / sqrt(1 + SR^2/(2*T/252))) approx
# Using the project's standard: from experiment registry
dsr_value = 0.0
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "credit_spread_regime",
