#!/usr/bin/env python3
"""Robustness analysis for commodity-carry-v2 (F9-H1, Commodity Carry/Backwardation).

Mechanism: USO/DBC price ratio 20-day momentum as crude oil carry/backwardation proxy.
  - Ratio momentum > 0 (backwardation / carry positive): 20% USO + 20% GLD + 30% SPY
  - Ratio momentum < 0 (contango / carry negative): 70% SPY + 10% SHY

v2 changes from v1:
  - lookback reduced from 30 to 20 (v1 perturbation showed lookback=20 gives Sharpe=1.18)
  - uso_weight reduced from 0.40 to 0.20 (reduce USO exposure to control MaxDD)
  - gld_weight reduced from 0.30 to 0.20 (reduce total backwardation regime exposure)
  - spy_carry_weight increased from 0.10 to 0.30 (SPY buffer in carry regime)
  - spy_contango_weight unchanged at 0.70
  - shy_weight unchanged at 0.10

Carry regime allocation: 20% USO + 20% GLD + 30% SPY = 70% total (30% cash buffer)
Contango regime allocation: 70% SPY + 10% SHY = 80% total (20% cash buffer)

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_commodity_carry_v2_robustness.py
"""
...
BASE_PARAMS = {
    ...
    "uso_weight": 0.20,
    "gld_weight": 0.20,
    "spy_carry_weight": 0.30,
    "spy_contango_weight": 0.70,
    "shy_weight": 0.10,
}
...
def run_single(params):
    ...
    uso_w = float(params.get("uso_weight", 0.20))
    gld_w = float(params.get("gld_weight", 0.20))
    spy_carry_w = float(params.get("spy_carry_weight", 0.30))
    spy_cont_w = float(params.get("spy_contango_weight", 0.70))
    shy_w = float(params.get("shy_weight", 0.10))
    ...
    for i in range(WARMUP, n):
        ...
        # USO/DBC ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        uso_now = sym_data["USO"].get(d_now, 0.0)
        dbc_now = sym_data["DBC"].get(d_now, 0.0)
        uso_lb = sym_data["USO"].get(d_lb, 0.0)
        dbc_lb = sym_data["DBC"].get(d_lb, 0.0)

        if uso_now <= 0 or dbc_now <= 0 or uso_lb <= 0 or dbc_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = uso_now / dbc_now
        ratio_lb = uso_lb / dbc_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on USO/DBC ratio momentum
        if ratio_mom > 0:
            # Backwardation / carry positive: crude outperforming broad commodities
            # v2: 20% USO + 20% GLD + 30% SPY (reduced commodity exposure, added SPY buffer)
            regime = "backwardation"
            day_ret = (
                asset_ret("USO", i) * uso_w
                + asset_ret("GLD", i) * gld_w
                + spy_rets[i] * spy_carry_w
            )
        else:
            # Contango / carry negative: crude underperforming broad commodities
            regime = "contango"
            day_ret = spy_rets[i] * spy_cont_w + asset_ret("SHY", i) * shy_w
...
perturbations = [
    ...
    ("uso_weight=0.15", {**BASE_PARAMS, "uso_weight": 0.15}),
    ("uso_weight=0.25", {**BASE_PARAMS, "uso_weight": 0.25}),
    ("spy_carry_weight=0.20", {**BASE_PARAMS, "spy_carry_weight": 0.20}),
    ("spy_carry_weight=0.40", {**BASE_PARAMS, "spy_carry_weight": 0.40}),
]
