#!/usr/bin/env python3
"""Robustness analysis for F29: Bond Volatility Regime (TLT Vol Proxy).
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f29_bond_vol_regime.py
"""
...
from backtest_f29_bond_vol_regime import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def run_shuffled_signal_test(
    ...
    """Test whether the bond vol regime signal is genuine by shuffling regimes.

    Instead of using the actual bond_vol_ratio percentile to determine regime,
    we randomly assign one of the three regimes at each rebalance point.
    If the real signal is genuine, the shuffled distribution of Sharpes should
...
def _run_shuffled_backtest(
    prices_df: object,
    rng: np.random.Generator,
) -> float | None:
    """Run a single backtest with randomly assigned regimes.

    At each rebalance point, instead of computing the bond_vol_ratio
    percentile, we randomly choose among bond_stress, equity_overpriced,
    and neutral with equal probability. Everything else is identical.
    """
    import polars as pl
    ...
    regimes = ["bond_stress", "equity_overpriced", "neutral"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    ...
    ("ratio_lookback=60", {"ratio_lookback": 60}),
    ("ratio_lookback=120", {"ratio_lookback": 120}),
    ("bond_stress_threshold=0.70", {"bond_stress_threshold": 0.70}),
    ("bond_stress_threshold=0.90", {"bond_stress_threshold": 0.90}),
    ("equity_cheap_threshold=0.10", {"equity_cheap_threshold": 0.10}),
    ("equity_cheap_threshold=0.30", {"equity_cheap_threshold": 0.30}),
    ("spy_bond_stress=0.10", {"spy_bond_stress": 0.10}),
    ("spy_bond_stress=0.30", {"spy_bond_stress": 0.30}),
...
def main() -> None:
    """Run full robustness analysis for F29."""
    import yaml
    ...
    print(f"  Regime counts: {base_result['regime_counts']}")
    ...
    # ── Final gate summary ───────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("  F29 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
