#!/usr/bin/env python3
"""Robustness analysis for F34: Intraday Range Compression Regime.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f34_range_compression.py
"""
...
from backtest_f34_range_compression import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def run_shuffled_signal_test(
    ...
    we randomly assign one of the five regimes. If the real signal is genuine,
    the shuffled Sharpe distribution should be lower.
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    We override the regime classification by randomly choosing among the five
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    # range_window perturbations (spec: [40, 80])
    ("range_window=40", {"range_window": 40}),
    ("range_window=80", {"range_window": 80}),
    # percentile_thresh perturbations (spec: [15, 35])
    ("percentile_compressed=15", {"percentile_compressed": 15}),
    ("percentile_compressed=35", {"percentile_compressed": 35}),
    # sma_period perturbations (spec: [30, 70])
    ("sma_period=30", {"sma_period": 30}),
    ("sma_period=70", {"sma_period": 70}),
    # spy_compressed weight perturbations (spec: [0.60, 0.80])
    ("spy_compressed_bullish=0.60", {"spy_compressed_bullish": 0.60}),
    ("spy_compressed_bullish=0.80", {"spy_compressed_bullish": 0.80}),
    # gld_bearish weight perturbations (spec: [0.40, 0.60])
    ("gld_expanding_bearish=0.40", {"gld_expanding_bearish": 0.40}),
    ("gld_expanding_bearish=0.60", {"gld_expanding_bearish": 0.60}),
    # Additional perturbations for robustness coverage
    ("spy_expanding_bullish=0.70", {"spy_expanding_bullish": 0.70}),
    ("spy_expanding_bullish=0.90", {"spy_expanding_bullish": 0.90}),
    ("spy_normal=0.40", {"spy_normal": 0.40}),
    ("spy_normal=0.60", {"spy_normal": 0.60}),
]
...
def main() -> None:
    """Run full robustness analysis for F34."""
    import yaml
    ...
    # Step 4: CPCV (5 folds)
    logger.info("=" * 60)
    logger.info("Step 3: CPCV (5 folds, purge=5)")
    logger.info("=" * 60)
    ...
    # -- Final gate summary --
    print("\n" + "=" * 60)
    print("  F34 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
    ...
    entry = {
        ...
        "cagr": base_cagr,
        "dsr": base_dsr,
        "cpcv_oos_sharpe": round(cpcv_mean, 4),
        "cpcv_oos_is_ratio": round(oos_is_ratio, 4),
        "perturbation_stability": perturbation_result["stability_pct"],
        "shuffled_p_value": shuffle_result["p_value"],
        "overall_passed": all_passed,
    }
    with registry_path.open("a") as f:
        f.write(json.dumps(entry) + "\n")
    logger.info("Robustness experiment %s appended to %s", experiment_id, registry_path)
