#!/usr/bin/env python3
"""Robustness analysis for lqd-upro-credit-lead-lag (Track D).

Mechanism: LQD (IG credit) 5-day return as lead signal -> UPRO (3x leveraged SPY) follower.
This is a leveraged re-expression of the proven LQD-SPY credit lead signal (Sharpe=1.250).

Signal logic:
  - LQD 5-day return >= entry_threshold (0.5%) -> entry (buy UPRO at target_weight)
  - LQD 5-day return <= exit_threshold (-0.5%) -> exit to SHY (cash proxy)
...
Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_lqd_upro_sprint_robustness.py
"""
...
BASE_PARAMS = {
    "entry_threshold": 0.005,  # LQD 5-day return >= 0.5% -> buy UPRO
    "exit_threshold": -0.005,  # LQD 5-day return <= -0.5% -> exit
    "lag_days": 5,  # Use signal from 5 days ago
    "signal_window": 5,  # 5-day return lookback for LQD
...
# ==============================================================================
perturbations = [
    ("entry_threshold=0.003 (gentler)", {**BASE_PARAMS, "entry_threshold": 0.003}),
    ("entry_threshold=0.008 (stricter)", {**BASE_PARAMS, "entry_threshold": 0.008}),
...
output = {
    ...
    "track": "D",
    "mechanism": "LQD (IG credit) 5-day return -> UPRO (3x leveraged SPY)",
