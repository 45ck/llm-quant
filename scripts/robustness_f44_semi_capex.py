#!/usr/bin/env python3
"""Robustness analysis for F44: Semiconductor Capex Cycle.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f44_semi_capex.py
"""
...
from backtest_f44_semi_capex import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def run_shuffled_signal_test(
    ...
    """Test whether the semiconductor capex cycle signal is genuine by shuffling.

    At each rebalance point, instead of using the real regime, we
    randomly assign one of {CAPEX_EXPANSION, NEUTRAL, CAPEX_CONTRACTION}.
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    regimes = ["CAPEX_EXPANSION", "NEUTRAL", "CAPEX_CONTRACTION"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    ...
    ("qqq_expansion=0.25", {"qqq_expansion": 0.25, "shy_expansion": 0.00}),
    ...
    ("tlt_contraction=0.15", {"tlt_contraction": 0.15, "shy_contraction": 0.25}),
    ("tlt_contraction=0.30", {"tlt_contraction": 0.30, "shy_contraction": 0.10}),
    # Rebalance frequency
...
def main() -> None:
    """Run full robustness analysis for F44."""
    import yaml
    ...
    # ── Final gate summary ───────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("  F44 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
