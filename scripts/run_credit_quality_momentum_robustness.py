#!/usr/bin/env python3
"""Robustness analysis for credit-quality-momentum-v1 (F19 Credit Quality Spread).

Mechanism: HYG/LQD price ratio 20-day momentum captures credit quality preference
as a risk appetite proxy. When HY outperforms IG (investors reaching for yield,
risk appetite strong): hold equity. When IG outperforms HY (flight to quality):
go defensive with bonds/gold.

Signal:
  - Compute HYG/LQD price ratio at close t-1
  - Compute 20-day ratio momentum (ratio_now / ratio_20d_ago - 1)
  - ratio_mom > 0 (risk appetite): 80% SPY
  - ratio_mom < 0 (quality flight): 50% SHY + 30% GLD + 10% SPY

Base parameters: lookback=20, spy_risk_on_weight=0.80,
                 gld_weight=0.30, shy_weight=0.50, spy_defensive_weight=0.10.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_credit_quality_momentum_robustness.py
"""
...
SLUG = "credit-quality-momentum-v1"
SYMBOLS = ["HYG", "LQD", "SPY", "GLD", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
def run_single(params):
    ...
    shy_w = float(params.get("shy_weight", 0.50))
    spy_defensive_w = float(params.get("spy_defensive_weight", 0.10))
    ...
    for i in range(WARMUP, n):
        ...
        if ratio_mom > 0:
            # Risk appetite: HY outperforming IG → hold equity
            regime = "risk_on"
            day_ret = spy_rets[i] * spy_risk_on_w
        else:
            # Quality flight: IG outperforming HY → defensive
            regime = "quality_flight"
            day_ret = (
                asset_ret("SHY", i) * shy_w
                + asset_ret("GLD", i) * gld_w
                + spy_rets[i] * spy_defensive_w
            )
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "credit_quality_momentum",
