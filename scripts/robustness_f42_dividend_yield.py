#!/usr/bin/env python3
"""Robustness analysis for F42: Dividend Yield Regime.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f42_dividend_yield.py
"""
...
from backtest_f42_dividend_yield import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def run_shuffled_signal_test(
    ...
    """Test whether the dividend yield regime signal is genuine by shuffling.

    At each rebalance point, instead of using the real regime, we
    randomly assign one of {INCOME_PREFERENCE, NEUTRAL, GROWTH_PREFERENCE}.
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    all_needed = [*TRADEABLE_SYMBOLS, "SPYD"]
    ...
    regimes = ["INCOME_PREFERENCE", "NEUTRAL", "GROWTH_PREFERENCE"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    ...
    # Growth regime SPY weight
    ("spy_growth=0.60", {"spy_growth": 0.60, "shy_growth": 0.25}),
    ("spy_growth=0.80", {"spy_growth": 0.80, "shy_growth": 0.05}),
    # Growth regime QQQ weight
    ("qqq_growth=0.10", {"qqq_growth": 0.10, "shy_growth": 0.20}),
    ("qqq_growth=0.25", {"qqq_growth": 0.25, "shy_growth": 0.05}),
    # Income regime GLD weight
    ("gld_income=0.35", {"gld_income": 0.35, "shy_income": 0.25}),
    ("gld_income=0.50", {"gld_income": 0.50, "shy_income": 0.10}),
    # Income regime TLT weight
    ("tlt_income=0.15", {"tlt_income": 0.15, "shy_income": 0.25}),
    ("tlt_income=0.30", {"tlt_income": 0.30, "shy_income": 0.10}),
    # Rebalance frequency
...
def _adjust_weights_for_perturbation(params: BacktestParams) -> BacktestParams:
    ...
    """
    # Check and fix income regime weights
    income_sum = params.spy_income + params.gld_income + params.tlt_income
    if abs(income_sum + params.shy_income - 1.0) > 0.001:
        params.shy_income = max(0.0, 1.0 - income_sum)

    # Check and fix growth regime weights
    growth_sum = (
        params.spy_growth + params.qqq_growth + params.gld_growth + params.tlt_growth
    )
    if abs(growth_sum + params.shy_growth - 1.0) > 0.001:
        params.shy_growth = max(0.0, 1.0 - growth_sum)
...
def main() -> None:
    """Run full robustness analysis for F42."""
    import yaml
    ...
    # ── Final gate summary ───────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("  F42 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
    ...
    robustness_yaml: dict = {
        "strategy_slug": SLUG,
        "strategy_type": "dividend_yield_regime",
        "mechanism_family": "F42",
