#!/usr/bin/env python3
"""Robustness analysis for erp-timing-v2 (K6, Equity Risk Premium Timing).

Mechanism: SPY-IEF 300-day relative return as ERP proxy.
  - Relative return < -15% (equities cheap vs bonds): 90% SPY
  - Relative return > +20% (equities expensive vs bonds): 50% IEF + 20% GLD + 20% SHY
  - Neutral zone: 60% SPY + 20% IEF
Base parameters: lookback=300, cheap_thresh=-0.15, expensive_thresh=0.20,
                 cheap_spy_weight=0.90, expensive_ief_weight=0.50,
                 expensive_gld_weight=0.20, expensive_shy_weight=0.20,
                 neutral_spy_weight=0.60, neutral_ief_weight=0.20.

Change from v1: lookback=300 (was 252) — captures slightly longer business cycle.
PRE-SPECIFIED CHANGE: lookback=300 based on v1 perturbation showing Sharpe=0.898.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_erp_timing_v2_robustness.py
"""
...
BASE_PARAMS = {
    "lookback": 300,
    "cheap_thresh": -0.15,
    "expensive_thresh": 0.20,
    "cheap_spy_weight": 0.90,
    "expensive_ief_weight": 0.50,
    "expensive_gld_weight": 0.20,
    "expensive_shy_weight": 0.20,
    "neutral_spy_weight": 0.60,
    "neutral_ief_weight": 0.20,
}
...
def run_single(params):
    ...
    cheap_thresh = float(params.get("cheap_thresh", -0.15))
    expensive_thresh = float(params.get("expensive_thresh", 0.20))
    cheap_spy_w = float(params.get("cheap_spy_weight", 0.90))
    expensive_ief_w = float(params.get("expensive_ief_weight", 0.50))
    expensive_gld_w = float(params.get("expensive_gld_weight", 0.20))
    expensive_shy_w = float(params.get("expensive_shy_weight", 0.20))
    neutral_spy_w = float(params.get("neutral_spy_weight", 0.60))
    neutral_ief_w = float(params.get("neutral_ief_weight", 0.20))
    ...
    for i in range(WARMUP, n):
        ...
        # SPY and IEF prices at close i-1 and close i-1-lookback (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        spy_now = sym_data["SPY"].get(d_now, 0.0)
        spy_lb = sym_data["SPY"].get(d_lb, 0.0)
        ief_now = sym_data["IEF"].get(d_now, 0.0)
        ief_lb = sym_data["IEF"].get(d_lb, 0.0)

        if spy_now <= 0 or spy_lb <= 0 or ief_now <= 0 or ief_lb <= 0:
            daily_returns.append(0.0)
            continue

        # Compute relative return: SPY 300-day return minus IEF 300-day return
        spy_ret_300 = spy_now / spy_lb - 1
        ief_ret_300 = ief_now / ief_lb - 1
        relative_return = spy_ret_300 - ief_ret_300

        # Determine regime based on relative return
        if relative_return < cheap_thresh:
            # Equities cheap vs bonds (SPY crashed relative to IEF): go long SPY
            regime = "cheap"
            day_ret = spy_rets[i] * cheap_spy_w
        elif relative_return > expensive_thresh:
            # Equities expensive vs bonds (SPY surged relative to IEF): defensive
            regime = "expensive"
            day_ret = (
                asset_ret("IEF", i) * expensive_ief_w
                + asset_ret("GLD", i) * expensive_gld_w
                + asset_ret("SHY", i) * expensive_shy_w
            )
        else:
            # Neutral zone: balanced allocation
            regime = "neutral"
            day_ret = spy_rets[i] * neutral_spy_w + asset_ret("IEF", i) * neutral_ief_w
...
# ==============================================================================
perturbations = [
    ("lookback=250", {**BASE_PARAMS, "lookback": 250}),
    ("lookback=350", {**BASE_PARAMS, "lookback": 350}),
    ("cheap_thresh=-0.10", {**BASE_PARAMS, "cheap_thresh": -0.10}),
    ("cheap_thresh=-0.20", {**BASE_PARAMS, "cheap_thresh": -0.20}),
    ("expensive_thresh=0.15", {**BASE_PARAMS, "expensive_thresh": 0.15}),
    ("expensive_thresh=0.25", {**BASE_PARAMS, "expensive_thresh": 0.25}),
]
