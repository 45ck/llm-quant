#!/usr/bin/env python3
"""Robustness analysis for europe-us-momentum-v1 (F19, Europe-US Relative Momentum).

Hypothesis: VGK/SPY ratio 30-day momentum captures transatlantic capital flow.
  - ratio_mom > 0 (Europe outperforming): 40% SPY + 30% VGK + 10% GLD
  - ratio_mom < 0 (US outperforming): 80% SPY

This is DIFFERENT from F17 (global yield flow, which uses TLT/EFA ratio as a rate
signal). This uses VGK/SPY (pure equity relative momentum, not bond-equity ratio).

Base parameters: lookback=30, spy_us_weight=0.80,
                 spy_global_weight=0.40, vgk_weight=0.30, gld_weight=0.10.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_europe_us_momentum_robustness.py
"""
...
def run_single(params):
    ...
    spy_us_w = float(params.get("spy_us_weight", 0.80))
    spy_global_w = float(params.get("spy_global_weight", 0.40))
    vgk_w = float(params.get("vgk_weight", 0.30))
    gld_w = float(params.get("gld_weight", 0.10))
    ...
    for i in range(WARMUP, n):
        ...
        # VGK/SPY ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        vgk_now = sym_data["VGK"].get(d_now, 0.0)
        spy_now = sym_data["SPY"].get(d_now, 0.0)
        vgk_lb = sym_data["VGK"].get(d_lb, 0.0)
        spy_lb = sym_data["SPY"].get(d_lb, 0.0)

        if vgk_now <= 0 or spy_now <= 0 or vgk_lb <= 0 or spy_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = vgk_now / spy_now
        ratio_lb = vgk_lb / spy_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on VGK/SPY ratio momentum
        if ratio_mom > 0:
            # Europe outperforming: diversify into Europe + gold
            regime = "europe_strong"
            day_ret = (
                spy_rets[i] * spy_global_w
                + asset_ret("VGK", i) * vgk_w
                + asset_ret("GLD", i) * gld_w
            )
        else:
            # US outperforming: concentrate in US equity
            regime = "us_strong"
            day_ret = spy_rets[i] * spy_us_w
...
perturbations = [
    ...
    ("spy_us_weight=0.70", {**BASE_PARAMS, "spy_us_weight": 0.70}),
    ("spy_us_weight=0.90", {**BASE_PARAMS, "spy_us_weight": 0.90}),
    ("vgk_weight=0.20", {**BASE_PARAMS, "vgk_weight": 0.20}),
    ("vgk_weight=0.40", {**BASE_PARAMS, "vgk_weight": 0.40}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "europe_us_momentum",
