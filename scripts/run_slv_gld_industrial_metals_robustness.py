#!/usr/bin/env python3
"""Robustness analysis for slv-gld-industrial-metals-v1.

Mechanism: SLV/GLD price ratio 30-day momentum captures industrial vs safe haven demand.
  - ratio_mom > 0 (silver outperforming gold, industrial): 80% SPY
  - ratio_mom < 0 (gold outperforming silver, safe haven): 50% GLD + 20% SHY

DIFFERENT from GLD-SLV v4 (F2) which is z-score mean-reversion on the SAME pair.
This is MOMENTUM on the ratio -- opposite hypothesis.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_slv_gld_industrial_metals_robustness.py
"""
...
SLUG = "slv-gld-industrial-metals-v1"
SYMBOLS = ["SLV", "GLD", "SPY", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
BASE_PARAMS = {
    "lookback": 30,
    "spy_industrial_weight": 0.80,
    "gld_safe_haven_weight": 0.50,
    "shy_weight": 0.20,
}
...
def run_single(params):
    ...
    spy_industrial_w = float(params.get("spy_industrial_weight", 0.80))
    gld_safe_haven_w = float(params.get("gld_safe_haven_weight", 0.50))
    ...
    for i in range(WARMUP, n):
        ...
        # SLV/GLD ratio momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        slv_now = sym_data["SLV"].get(d_now, 0.0)
        gld_now = sym_data["GLD"].get(d_now, 0.0)
        slv_lb = sym_data["SLV"].get(d_lb, 0.0)
        gld_lb = sym_data["GLD"].get(d_lb, 0.0)

        if slv_now <= 0 or gld_now <= 0 or slv_lb <= 0 or gld_lb <= 0:
            daily_returns.append(0.0)
            continue

        ratio_now = slv_now / gld_now
        ratio_lb = slv_lb / gld_lb
        ratio_mom = ratio_now / ratio_lb - 1

        # Determine regime based on SLV/GLD ratio momentum
        if ratio_mom > 0:
            # Industrial regime: silver outperforming gold, industrial demand strong
            # 80% SPY
            regime = "industrial"
            day_ret = spy_rets[i] * spy_industrial_w
        else:
            # Safe haven regime: gold outperforming silver, defensive positioning
            # 50% GLD + 20% SHY
            regime = "safe_haven"
            day_ret = (
                asset_ret("GLD", i) * gld_safe_haven_w + asset_ret("SHY", i) * shy_w
            )
...
perturbations = [
    ...
    ("spy_industrial_weight=0.70", {**BASE_PARAMS, "spy_industrial_weight": 0.70}),
    ("spy_industrial_weight=0.90", {**BASE_PARAMS, "spy_industrial_weight": 0.90}),
    ("gld_safe_haven_weight=0.40", {**BASE_PARAMS, "gld_safe_haven_weight": 0.40}),
    ("gld_safe_haven_weight=0.60", {**BASE_PARAMS, "gld_safe_haven_weight": 0.60}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "slv_gld_industrial_metals",
