#!/usr/bin/env python3
"""Robustness analysis for F50: High-Beta / Low-Beta Rotation.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f50_beta_rotation.py
"""
...
from backtest_f50_beta_rotation import (  # noqa: E402
    ALL_SYMBOLS,
    INITIAL_CAPITAL,
    LOOKBACK_DAYS,
    SLUG,
...
def run_shuffled_signal_test(
    ...
    """Test whether the beta rotation signal is genuine by shuffling.

    At each rebalance point, instead of using the real regime, we
    randomly assign one of {RISK_SEEKING, NEUTRAL, RISK_AVERSE}.
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    regimes = ["RISK_SEEKING", "NEUTRAL", "RISK_AVERSE"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    # Momentum lookback (+/- 25%: 15 and 25)
    ("mom_lookback=15", {"mom_lookback": 15}),
    ("mom_lookback=25", {"mom_lookback": 25}),
    # SMA period (+/- 25%: 30 and 50)
    ("sma_period=30", {"sma_period": 30}),
    ("sma_period=50", {"sma_period": 50}),
    # Risk-seeking regime SPY weight
    ("spy_risk_seeking=0.60", {"spy_risk_seeking": 0.60, "shy_risk_seeking": 0.25}),
    ("spy_risk_seeking=0.80", {"spy_risk_seeking": 0.80, "shy_risk_seeking": 0.05}),
    # Risk-seeking regime QQQ weight
    ("qqq_risk_seeking=0.10", {"qqq_risk_seeking": 0.10, "shy_risk_seeking": 0.20}),
    ("qqq_risk_seeking=0.25", {"qqq_risk_seeking": 0.25, "shy_risk_seeking": 0.05}),
    # Risk-averse regime GLD weight
    ("gld_risk_averse=0.25", {"gld_risk_averse": 0.25, "shy_risk_averse": 0.35}),
    ("gld_risk_averse=0.40", {"gld_risk_averse": 0.40, "shy_risk_averse": 0.20}),
    # Risk-averse regime TLT weight
    ("tlt_risk_averse=0.15", {"tlt_risk_averse": 0.15, "shy_risk_averse": 0.35}),
    ("tlt_risk_averse=0.30", {"tlt_risk_averse": 0.30, "shy_risk_averse": 0.20}),
    # Rebalance frequency
...
def _adjust_weights_for_perturbation(params: BacktestParams) -> BacktestParams:
    ...
    """
    # Check and fix risk-seeking regime weights
    seeking_sum = (
        params.spy_risk_seeking
        + params.qqq_risk_seeking
        + params.gld_risk_seeking
        + params.tlt_risk_seeking
    )
    if abs(seeking_sum + params.shy_risk_seeking - 1.0) > 0.001:
        params.shy_risk_seeking = max(0.0, 1.0 - seeking_sum)

    # Check and fix risk-averse regime weights
    averse_sum = (
        params.spy_risk_averse
        + params.qqq_risk_averse
        + params.gld_risk_averse
        + params.tlt_risk_averse
    )
    if abs(averse_sum + params.shy_risk_averse - 1.0) > 0.001:
        params.shy_risk_averse = max(0.0, 1.0 - averse_sum)
...
def main() -> None:
    """Run full robustness analysis for F50."""
    import yaml
    ...
    # Check if Sharpe >= 0.70 to proceed with robustness
    if base_sharpe < 0.70:
        print(f"\n  Sharpe {base_sharpe:.4f} < 0.70 -- SKIPPING robustness.")
        print("  Strategy does not meet minimum threshold for robustness testing.")

        # Still save minimal robustness.yaml with FAIL verdict
        robustness_yaml: dict = {
            "strategy_slug": SLUG,
            "strategy_type": "high_low_beta_rotation",
            "mechanism_family": "F50",
    ...
    # ── Final gate summary ───────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("  F50 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
    ...
    robustness_yaml_full: dict = {
        ...
        "shuffled_signal": {
            ...
            "shuffled_95th": round(
                float(np.percentile([shuffle_result["shuffled_mean"]], 95)),
                4,
            )
            if shuffle_result["n_shuffles"] > 0
            else 0.0,
            "shuffled_99th": round(
                float(np.percentile([shuffle_result["shuffled_mean"]], 99)),
                4,
            )
            if shuffle_result["n_shuffles"] > 0
            else 0.0,
