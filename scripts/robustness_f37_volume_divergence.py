#!/usr/bin/env python3
"""Robustness analysis for F37: Volume-Price Divergence Regime.
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_f37_volume_divergence.py
"""
...
from backtest_f37_volume_divergence import (  # noqa: E402
    ALL_SYMBOLS,
    LOOKBACK_DAYS,
    SLUG,
    TRADEABLE_SYMBOLS,
...
def run_shuffled_signal_test(
    ...
    we randomly assign one of {hollow_rally, exhausted_selling, neutral}. If the
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    from backtest_f37_volume_divergence import INITIAL_CAPITAL
    ...
    regimes = ["hollow_rally", "exhausted_selling", "neutral"]
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    # price_lookback: 5, 15 (vs base 10)
    ("price_lookback=5", {"price_lookback": 5}),
    ("price_lookback=15", {"price_lookback": 15}),
    # vol_short: 5, 15 (vs base 10)
    ("vol_short=5", {"vol_short": 5}),
    ("vol_short=15", {"vol_short": 15}),
    # vol_long: 30, 60 (vs base 40)
    ("vol_long=30", {"vol_long": 30}),
    ("vol_long=60", {"vol_long": 60}),
    # entry_threshold: 1.0, 2.0 (vs base 1.5)
    ("entry_threshold=1.0", {"entry_threshold": 1.0}),
    ("entry_threshold=2.0", {"entry_threshold": 2.0}),
    # spy_hollow: 0.20, 0.40 (vs base 0.30)
    ("spy_hollow=0.20", {"spy_hollow": 0.20}),
    ("spy_hollow=0.40", {"spy_hollow": 0.40}),
    # spy_exhausted: 0.70, 0.90 (vs base 0.80)
    ("spy_exhausted=0.70", {"spy_exhausted": 0.70}),
    ("spy_exhausted=0.90", {"spy_exhausted": 0.90}),
]
...
def main() -> None:
    """Run full robustness analysis for F37."""
    import yaml
    ...
    # -- Final gate summary --
    print("\n" + "=" * 60)
    print("  F37 ROBUSTNESS GATE SUMMARY")
    print("=" * 60)
