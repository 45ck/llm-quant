#!/usr/bin/env python3
"""Robustness analysis for D11: SOXX -> SOXL Semiconductor Lead-Lag.
...
5. Gate check against Track D thresholds
...
Usage:
    cd E:/llm-quant && PYTHONPATH=src python scripts/robustness_d11_soxx_soxl.py
"""
...
from backtest_d11_soxx_soxl import (  # noqa: E402
    ALL_SYMBOLS,
    LOOKBACK_DAYS,
    SLUG,
    TRADEABLE_SYMBOLS,
...
def run_shuffled_signal_test(
    ...
    """Test whether the SOXX momentum signal is genuine by shuffling.

    At each rebalance point, instead of using the real SOXX return signal,
    we randomly assign RISK_ON or RISK_OFF. If the real signal is genuine,
    the shuffled Sharpe distribution should be significantly lower.
...
def _run_shuffled_backtest(  # noqa: PLR0912
    ...
    We override the regime classification by randomly choosing RISK_ON or
    RISK_OFF at each rebalance, while keeping everything else identical.
    """
    ...
    from llm_quant.backtest.metrics import compute_sharpe as _cs
    ...
    for i, date_str in enumerate(common_dates):
        ...
        if nav <= 0:
            nav = daily_navs[-1] if daily_navs else 100_000.0
        daily_navs.append(nav)
        if len(daily_navs) >= 2:
            prev_nav = daily_navs[-2]
    ...
    return _cs(strategy_returns, annualize=True)
...
PERTURBATION_CONFIGS: list[tuple[str, dict]] = [
    # Signal window
    ("signal_window=3", {"signal_window": 3}),
    ("signal_window=7", {"signal_window": 7}),
    ("signal_window=10", {"signal_window": 10}),
    # Entry threshold
    ("entry_threshold=0.01", {"entry_threshold": 0.01}),
    ("entry_threshold=0.015", {"entry_threshold": 0.015}),
    ("entry_threshold=0.03", {"entry_threshold": 0.03}),
    # Exit threshold
    ("exit_threshold=-0.005", {"exit_threshold": -0.005}),
    ("exit_threshold=-0.015", {"exit_threshold": -0.015}),
    ("exit_threshold=-0.02", {"exit_threshold": -0.02}),
    # SOXL weight
    ("soxl_weight=0.30", {"soxl_weight": 0.30, "shy_weight_risk_on": 0.20}),
    ("soxl_weight=0.50", {"soxl_weight": 0.50, "shy_weight_risk_on": 0.00}),
    # VIX threshold
    ("vix_crash=25", {"vix_crash_threshold": 25.0}),
    ("vix_crash=35", {"vix_crash_threshold": 35.0}),
    # Rebalance frequency
    ("rebalance=3", {"rebalance_frequency_days": 3}),
    ("rebalance=7", {"rebalance_frequency_days": 7}),
...
def run_perturbation_tests(
    ...
    return {
        ...
        "results": results,
        "passed": stability_pct >= 40.0,  # Track D: relaxed to 40%
    }
...
def main() -> None:
    """Run full robustness analysis for D11."""
    import yaml
    ...
    # ── Final gate summary ───────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("  D11 ROBUSTNESS GATE SUMMARY (Track D)")
    print("=" * 60)
