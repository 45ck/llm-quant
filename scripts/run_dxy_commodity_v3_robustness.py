#!/usr/bin/env python3
"""Robustness analysis for dxy-commodity-v3 (A3, DXY-Commodity Rotation).

Hypothesis: DXY strength predicts commodity underperformance over 30 days.
Mechanism: UUP (dollar bull ETF) 30-day momentum.
  - UUP momentum > 0 (dollar strengthening): 80% SPY (avoid commodities)
  - UUP momentum < 0 (dollar weakening): 30% GLD + 20% DBA + 40% SPY
Base parameters: lookback=30, spy_weight_strong=0.80,
                 gld_weight_weak=0.30, dba_weight_weak=0.20, spy_weight_weak=0.40.

Changes from v2:
  - gld_weight_weak: 0.40 -> 0.30 (reduce commodity exposure for MaxDD control)
  - spy_weight_weak: 0.30 -> 0.40 (increase SPY buffer in weak-dollar regime)
  - Total dollar-weak exposure stays at 90% (0.30 + 0.20 + 0.40)

Rationale: v2 passed 5/6 gates but failed MaxDD (16.9% vs 15%). Signal is confirmed
real (p=0.000, CPCV=1.10, perturbation 100%). Structural diagnosis: dollar-weak
commodity allocation drives drawdown. Reducing GLD allocation and increasing SPY
buffer should lower MaxDD while preserving the signal.

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_dxy_commodity_v3_robustness.py
"""
...
SLUG = "dxy-commodity-v3"
SYMBOLS = ["UUP", "DBA", "GLD", "SPY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60
...
BASE_PARAMS = {
    "lookback": 30,
    "spy_weight_strong": 0.80,
    "gld_weight_weak": 0.30,
    "dba_weight_weak": 0.20,
    "spy_weight_weak": 0.40,
}
...
def run_single(params):
    ...
    spy_w_strong = float(params.get("spy_weight_strong", 0.80))
    gld_w_weak = float(params.get("gld_weight_weak", 0.30))
    dba_w_weak = float(params.get("dba_weight_weak", 0.20))
    spy_w_weak = float(params.get("spy_weight_weak", 0.40))
    ...
    for i in range(WARMUP, n):
        ...
        # UUP momentum at close i-1 (lag-1, no look-ahead)
        d_now = dates[i - 1]
        d_lb = dates[i - 1 - lookback]

        uup_now = sym_data["UUP"].get(d_now, 0.0)
        uup_lb = sym_data["UUP"].get(d_lb, 0.0)
        ...
        # Determine regime based on UUP momentum
        if uup_mom > 0:
            # Dollar strengthening: avoid commodities, hold SPY
            regime = "dollar_strong"
            day_ret = spy_rets[i] * spy_w_strong
        else:
            # Dollar weakening: rotate into commodities
            regime = "dollar_weak"
            day_ret = (
                asset_ret("GLD", i) * gld_w_weak
                + asset_ret("DBA", i) * dba_w_weak
                + asset_ret("SPY", i) * spy_w_weak
            )
...
perturbations = [
    ...
    ("spy_weight_strong=0.70", {**BASE_PARAMS, "spy_weight_strong": 0.70}),
    ("spy_weight_strong=0.90", {**BASE_PARAMS, "spy_weight_strong": 0.90}),
    ("gld_weight_weak=0.20", {**BASE_PARAMS, "gld_weight_weak": 0.20}),
    ("gld_weight_weak=0.40", {**BASE_PARAMS, "gld_weight_weak": 0.40}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "dxy_commodity_rotation",
