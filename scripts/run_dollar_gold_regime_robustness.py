#!/usr/bin/env python3
"""Robustness analysis for dollar-gold-regime-v1 (F26 candidate).

Mechanism: UUP/GLD ratio as real purchasing power proxy.
Rising UUP/GLD (dollar strengthening vs gold) = risk-on -> SPY.
Falling UUP/GLD (gold outperforming dollar) = inflation hedge -> GLD+DBA.

This is DIFFERENT from F19 TLT/GLD disinflation: F19 uses TLT/GLD ratio
(nominal bonds vs gold), this uses UUP/GLD (dollar vs gold -- a purchasing
power signal, not a yield signal).

Run: cd E:/llm-quant && PYTHONPATH=src python scripts/run_dollar_gold_regime_robustness.py
"""
...
SYMBOLS = ["SPY", "GLD", "DBA", "UUP", "SHY"]
DD_THRESHOLD = 0.15
LOOKBACK_DAYS = 5 * 365
WARMUP = 60

BASE_PARAMS = {
    "lookback": 30,  # Momentum lookback for UUP/GLD ratio
    "spy_weight": 0.60,  # SPY weight during dollar strength (reduced from 0.80 for MaxDD)
    "gld_weight": 0.40,  # GLD weight during gold strength
    "dba_weight": 0.15,  # DBA weight during gold strength
}
...
def run_single(params):
    ...
    spy_weight = float(params.get("spy_weight", 0.80))
    gld_weight = float(params.get("gld_weight", 0.50))
    dba_weight = float(params.get("dba_weight", 0.20))
    ...
    for i in range(WARMUP, n):
        ...
        # UUP/GLD ratio momentum at close i-1 (lag-1, no look-ahead)
        ...
        # Determine regime
        gld_r = asset_ret("GLD", i)
        dba_r = asset_ret("DBA", i)

        if ratio_mom > 0:
            # Dollar strengthening vs gold -> risk-on SPY
            regime = "dollar_strength"
            day_ret = spy_rets[i] * spy_weight
        else:
            # Gold strengthening vs dollar -> inflation hedge
            regime = "gold_strength"
            day_ret = gld_r * gld_weight + dba_r * dba_weight
...
perturbations = [
    ...
    ("spy_weight=0.50", {**BASE_PARAMS, "spy_weight": 0.50}),
]
...
output = {
    "strategy_slug": SLUG,
    "strategy_type": "dollar_gold_regime",
    "mechanism_family": "F26",
